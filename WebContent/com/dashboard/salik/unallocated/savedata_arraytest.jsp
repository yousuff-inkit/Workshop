...
if(!(regno.equalsIgnoreCase(""))){
	...
	ArrayList<String> rowValues= new ArrayList<String>();
 	...
 	String sql="select s.tagno,s.salik_date,s.trans,va.authid auth,veh.fleet_no from gl_salik s left join gl_vehauth va on va.authname=s.source left join gl_vehmaster veh " 
	+ "on veh.salik_tag=s.tagno where  s.isallocated=0 and s.status in (0,3) "+sqltest; 
...
ArrayList<String> temp=new ArrayList<String>();
        //            System.out.println("-----2--"+sqlup1);  
				     ...
				     while(rs1.next())
				     {
				    	temp.add(rs1.getString("rdtype")+"::"+rs1.getString("rdocno")+"::"+rs1.getString("trancode")+"::"+rs1.getString("emp_id")+"::"+rs1.getString("emp_type")+"::"+rs1.getString("fleet_no"));
				      } 
				     stmt1.close();
			//	     System.out.println("-----2--"+temp);
						for(int a=0;a<temp.size();a++){
							
						String[] test=temp.get(a).split("::");
				//		System.out.println("-----2--"+temp.get(a));
			    	 if(test[0].equalsIgnoreCase("RAG") || test[0].equalsIgnoreCase("MOV") || test[0].equalsIgnoreCase("LAG")  || test[0].equalsIgnoreCase("VCU") )
			    	 {
			    		 
			    		 saveval="11";
			    		 Statement stmt2 = conn.createStatement ();
			    		 String sqlup2="update gl_salik set isallocated=1,reason='Allocated',ra_no='"+test[1]+"',rtype='"+test[2]+"',emp_id='"+test[3]+"',emp_type='"+test[4]+"', fleetno='"+test[5]+"' where tagno='"+subarray[1].trim()+"' and trans='"+subarray[3].trim()+"'";
			    	// 	  System.out.println(i+"=== a-----3--"+sqlup2);   
