...
<%ClsBalanceSheetSymbol cbs=new ClsBalanceSheetSymbol(); %>
...
<style type="text/css">
 ...
 <script type="text/javascript">
 		...
 		if(temp!='NA'){
	    		data = '<%=cbs.balanceSheetGrid(branchval, fromDate, toDate, level1, level2, level3, level4, check)%>';
	    		dataExcelExport = '<%=cbs.balanceSheetExcelExport(branchval, fromDate, toDate, level1, level2, level3, level4, check)%>';
 		}
            ...
            $('#balanceSheetGrid').on('rowDoubleClick', function (event) {
                ...
                var accdocno="";
          	    if(row.ordno == 6)
          	    {
					...
					var path="com/dashboard/accounts/balancesheetsymbol/accountDetails.jsp";
