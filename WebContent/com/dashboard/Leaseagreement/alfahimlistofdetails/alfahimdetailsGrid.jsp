<%@ page import="com.dashboard.leaseagreement.ClsleaseagreementDAO" %>
 ...
 var aa;
  ...
  if(temp4!='NA')
 { 
	  dataildatas='<%=clad.alfahimdetailsgrids(barchval,fromdate,todate,cldocno,status,fleet,group,brand,model,outchk,inchk)%>';
 
	  datasssss='<%=clad.alfahimdetailsgrid(barchval,fromdate,todate,cldocno,status,fleet,group,brand,model,outchk,inchk)%>';
            ...
            { 
                ...
                columns: [
                          ...
                          { text: 'SL#', sortable: false, filterable: false, editable: false,
					        ...
					        { text: ' ', datafield: 'btnprint', width: '6%',columntype: 'button',editable:false, filterable: false},
					]
            });
     	  ...
     	  $("#detailsgrid").on('cellclick', function (event) 
          		{
          		  ...
          		  var doc_no=$('#detailsgrid').jqxGrid('getcellvalue',rowBoundIndex,'doc_no');
          		var brhid=$('#detailsgrid').jqxGrid('getcellvalue',rowBoundIndex,'brhid');
          		  //alert(doc_no);
          		//alert(brhid);
          		  
          		    if(datafield=="btnprint"){
          		    	 
          		    	funPrintBtn(doc_no,brhid);
          		    	
          		    }
          		
          		    	
          		    function funPrintBtn(doc_no,temp4)
          		    {
          			var docno=doc_no;
          			var branch=temp4;
          			...
          			// alert(url);
          		    var reurl=url.split("Leaseagreement");
          		...
          		// var win= window.open(reurl[0]+"/epicraprint1.jsp","_blank","top=250,left=310,Width=800,Height=800,location=no,scrollbars=no,toolbar=yes");
          		    var win= window.open(reurl[0]+"leaseagreement/Leasemainprints?docno="+docno+"&branchval="+branch+"&formdetailcode=LAG","_blank","top=250,left=310,Width=800,Height=800,location=no,scrollbars=no,toolbar=yes");
          		    win.focus(); 
          			   } 
