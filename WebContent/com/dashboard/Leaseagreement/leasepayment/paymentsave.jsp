...
try{
	...
	if(!(odate.equalsIgnoreCase("undefined"))&&!(odate.equalsIgnoreCase(""))&&!(odate.equalsIgnoreCase("0")&&!(odate.equalsIgnoreCase("NA"))))
		{
   ...
   //  odate,brhid
	...
	int update=0; // if payment auth already in payid then update else insert 
	  ...
	  String upsqld="select rdocno from gl_lpyt where rdocno='"+rano+"' ";
	//  System.out.println("--------3-----------"+upsqld);
	   ResultSet resultSet = stmt.executeQuery(upsqld);
	    ...
	    {
	    String upsqlss="select rdocno from gl_lpyt where rdocno='"+rano+"' and  payid=3";
	    
	 //  System.out.println("--------4----------"+upsqlss);
		   ResultSet resultSet1 = stmt.executeQuery(upsqlss);
	    ...
	    { // rdocno, payment, mode, amount, acode, cardno, expdate, recieptno, brhid, payid, doc_no, card, paytype, cardtype, reldate, relstatus
	    	 ...
	    	 upsql="update gl_lpyt set payment='"+payment+"',mode='"+mode+"',amount='"+amount+"',acode='"+acode+"' ,cardno='"+cardno+"',expdate='"+expdate+"',recieptno=0,brhid='"+brhid+"',payid=3,card='"+card+"',paytype='"+paytype+"',cardtype='"+cardtype+"'  where rdocno='"+rano+"' and  payid=3 ";
	    ...
	    String	upsql11="update gl_lpyt set reldate='"+sqloutDate+"'  where rdocno='"+rano+"' and  payid=3 ";
	    	// System.out.println("--------6---------"+upsql11);	
	    	 int temp1=   stmt.executeUpdate(upsql11);
	    	  if(temp1<=0)
	 	     {
	 	    	 conn.close();
	    ...
	    else
	    {
	    	...
	    	// System.out.println("--------7---------"+sql);
		     ...
		     if(!(odate.equalsIgnoreCase("undefined"))&&!(odate.equalsIgnoreCase(""))&&!(odate.equalsIgnoreCase("0")&&!(odate.equalsIgnoreCase("NA"))))
			    ...
			    String	upsql1="update gl_lpyt set reldate='"+sqloutDate+"'  where rdocno='"+rano+"' and  payid=3 ";
			    	// System.out.println("--------8---------"+upsql1);
			    	 int temp1=   stmt.executeUpdate(upsql1);
			    	  if(temp1<=0)
			 	     {
			 	    	 conn.close();
