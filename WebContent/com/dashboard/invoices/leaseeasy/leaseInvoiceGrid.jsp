...
var type='<%=type%>';
if(id=="1"){
	invoicedata='<%=leasedao.getLeaseInvoice(branch, date, client, type, id)%>';
}
else
{
	invoicedata;
...
$('#leaseInvoiceGrid').on('rowdoubleclick', function (event) 
    		{
    		    ...
    		    $('#leaseagmtdiv').load('agmtDetailGrid.jsp?cldocno='+cldocno+'&id=1&date='+date+'&branch='+branch+'&type='+type);
