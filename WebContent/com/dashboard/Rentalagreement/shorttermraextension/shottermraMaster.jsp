...
function funExportBtn(){
	  // $("#detailsgrid").jqxGrid('exportdata', 'xls', 'RAG-Short Term RA Extension');
	   
	   if(parseInt(window.parent.chkexportdata.value)=="1")
	    {
	    JSONToCSVCon(shotterm, 'RAG-Short Term RA Extension', true);
	    }
	   else
	    {
		   $("#detailsgrid").jqxGrid('exportdata', 'xls', 'RAG-Short Term RA Extension');
    ...
    $.messager.confirm('Message', 'Do you want to Update?', function(r){
     ...
     $('#gridlength').val(rows.length); 
	   ...
	   for(var i=0 ; i < rows.length ; i++){
		   ...
		   ajaxcall(rows[i].rentaltype,rows[i].rate,rows[i].cdw,rows[i].pai,rows[i].cdw1,rows[i].pai1,rows[i].gps,rows[i].babyseater,rows[i].cooler,rows[i].kmrest,
				   rows[i].exkmrte,rows[i].oinschg,rows[i].exhrchg,rows[i].chaufchg,rows[i].chaufexchg,rstatus,docnos,excessinsur,jqxDateOut,saveval,branchid,invoicevalue,advance);
...
function ajaxcall(rentaltype,rate,cdw,pai,cdw1,pai1,gps,baby,cooler,kmrest,exkm,insuchg,exhrchg,chafchg,chaufexchg,rstatus,docnos,excessinsur,jqxDateOut,saveval,branchid,invoicevalue,advance){
	...
	x.open("GET","savedata.jsp?rentaltype="+rentaltype+"&rate="+rate+"&cdw="+cdw+"&pai="+pai+"&cdw1="+cdw1+"&pai1="+pai1+"&gps="+gps+"&baby="+baby+"&cooler="+cooler+"&kmrest="+kmrest+"&exkm="+exkm+"&insuchg="+insuchg+"&exhrchg="+exhrchg+"&chafchg"+chafchg+"&chaufexchg="+chaufexchg+"&rstatus="+rstatus+"&docno="+docnos+"&excessinsur="+excessinsur+"&jqxDateOut="+jqxDateOut+"&saveval="+saveval+"&branchid="+branchid+"&invoicevalue="+invoicevalue+"&advance="+advance,true); 
	x.send();
}
...
<tr><td  ><label class="branch">Invoice</label></td><td><select name="invoice" id="invoice" style="width:70%;"  value='<s:property value="invoice"/>'  >
  ...
  <option value="2">Period</option>
	 ...
	 <tr><td colspan="2" align="center"><input type="button" class="myButton" name="update" id="update"  value="Update" onclick="funupdatera()"></td></tr>
...
<td width="80%">
 ...
 <tr>
		...
		<tr>
			 <td><div id="tariff"><jsp:include page="tariff.jsp"></jsp:include></div></td>
		</tr>
