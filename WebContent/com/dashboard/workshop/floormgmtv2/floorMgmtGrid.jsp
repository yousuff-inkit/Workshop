...
var rendererstring=function (aggregates){
        ...
        $("#floorMgmtGrid").jqxGrid(
                {
                    ...
                    columns: [
    					...
    					{ text: 'Est Doc No',datafield: 'estdocno', width: '3%',hidden:true,cellclassname: cellclassname},
    					{ text: 'Gate Doc No',datafield: 'gatedocno', width: '3%',hidden:true,cellclassname: cellclassname},
    					{ text: 'Branch Id',datafield: 'brhid', width: '3%',hidden:true,cellclassname: cellclassname},
    					{ text: 'Branch',datafield: 'branchname', width: '7%',cellclassname: cellclassname,pinned:true},
						...
						{ text: 'Total Loss Status', datafield: 'totalloss', width: '5%',cellclassname: cellclassname,cellsformat:'d2',hidden:true},
						{ text: 'Delay SMS Status', datafield: 'delaysms', width: '5%',cellclassname: cellclassname,cellsformat:'d2',hidden:true},
				...
				{ 
				    ...
				    getComments();
				    var job=$('#jobcarddocno').val();
				    $('#selectedtechgriddiv').load('selectedTechGrid.jsp?id=1&jobdocno='+job);
				    $('#baygriddiv').load('bayGrid.jsp?id=1&jobno='+job);
					...
					$('.jobvocno').text($('#jobcardvocno').val());
					if($('#delaysmsconfig').val()=='1'){
				 		var delaysms=$('#floorMgmtGrid').jqxGrid('getcellvalue',boundIndex,'delaysms');
				 		if(delaysms=="1"){
				 			$('#btndelaysms').attr('disabled',true);
				 			$('#btndelaysms').addClass('disabled');
				 		}
				 		else{
				 			$('#btndelaysms').attr('disabled',false);
				 			$('#btndelaysms').removeClass('disabled');
				 			var jobdocno=$('#jobcarddocno').val();
				 			getDelaySMSData(jobdocno);
				 		}
	...
	function getDelaySMSData(jobdocno){
		$.get('getDelaySMSDetails.jsp',{'jobdocno':jobdocno},function(data){
			data=JSON.parse(data);
			var msg=data.msg;
			msg=msg.replace('<reason>','<span class="delayreason"></span>');
			msg=msg.replace('<deliverdate>','<span class="deliverdate"></span>');
			$('.hiddelaysmstext').html($.parseHTML(msg));
			var deliverdate=$('#delaydate').jqxDateTimeInput('val');
			$('#modaldelaysms').find('.hiddelaysmstext').find('.deliverdate').text(deliverdate);
			$('#delaysmstext').text($('.hiddelaysmstext').text());
		});
	}
</script>
