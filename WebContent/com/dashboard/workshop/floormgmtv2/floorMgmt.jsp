...
<body>
	...
	<div class="container-fluid">
    	<div class="row rowgap">
      		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        		...
        		<div class="actionpanel custompanel">
          			...
          			<button type="button" class="btn btn-default" id="btntechselection" data-target="#modaltechselection"><i class="fa fa-users " aria-hidden="true" data-toggle="tooltip" title="Technician Selection" data-placement="bottom"></i></button>
        			...
        			<button type="button" class="btn btn-default" id="btnsendsms" onclick="funSendSMS();"><i class="fa fa-envelope" aria-hidden="true" data-toggle="tooltip" title="Send SMS" data-placement="bottom"></i></button>
        			<button type="button" class="btn btn-default" id="btndelaysms" data-target="#modaldelaysms"><i class="fa fa-commenting" aria-hidden="true" data-toggle="tooltip" title="Send Delay SMS" data-placement="bottom"></i></button>        	
        		</div>
		...
		<div id="modaldelaysms" class="modal fade " role="dialog">
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal">&times;</button>
						<h4 class="modal-title">Delay SMS for JOB #<span class="jobvocno"></span></h4>
					</div>
					<div class="modal-body">
						<div class="container-fluid form-horizontal">
    						<div class="form-group">
		    					<label class="control-label col-sm-2" for="delayreason">Reason:</label>
		    					<div class="col-sm-10">
		      						<input type="text" class="form-control" id="delayreason">
		    					</div>
		  					</div>
		  					<div class="form-group">
		    					<label class="control-label col-sm-2" for="delaydate">Date:</label>
		    					<div class="col-sm-10">
		      						<div id="delaydate"></div>
		    					</div>
		  					</div>
		  					<div class="form-group">
		  						<div class="hiddelaysmstext" hidden="true"></div>
		  						<textarea class="form-control" id="delaysmstext" style="height:130px;"></textarea>
		  					</div>	
    					</div>
					</div>
					<div class="modal-footer text-right">
	            		<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
	            		<button type="button" class="btn btn-default btn-primary" id="btnsenddelaysms">Send SMS</button>
	          		</div>
				</div>
			</div>
		</div>
	...
	<!-- Techinician Selection -->
	
	<div id="modaltechselection" class="modal fade " role="dialog">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal">&times;</button>
					<h4 class="modal-title">Technician Selection</h4>
				</div>
				<div class="modal-body custom-nav-pills">
					<ul class="nav nav-pills nav-justified">
						<li class="active"><a data-toggle="pill" href="#pilltechtoselect">To Be Selected</a></li>
				    	<li><a data-toggle="pill" href="#pilltechselected">Selected Technicians</a></li>
					</ul>	
					<div class="tab-content" style="margin-top:10px;">
    					<div id="pilltechtoselect" class="tab-pane fade in active">
    						<div class="container-fluid form-horizontal">
    							<div class="form-group">
		    						<label class="control-label col-sm-2" for="cmbtechnician">Technician:</label>
		    						<div class="col-sm-10">
		      							<select name="cmbtechnician" id="cmbtechnician" class="form-control" style="width:100%;">
											<option value="">--Select--</option>
										</select>
		    						</div>
		  						</div>
		  						<div class="form-group">
		    						<label class="control-label col-sm-2" for="techremarks">Remarks:</label>
		    						<div class="col-sm-10">
		      							<input type="text" name="techremarks" id="techremarks" class="form-control" placeholder="Remarks">
		    						</div>
		  						</div>
		  						<div class="form-group">
		    						<label class="control-label col-sm-2" for="techhrs">Est.Hours:</label>
		    						<div class="col-sm-10">
		      							<div id="techhrs" name="techhrs"></div>
		    						</div>
		  						</div>		
    						</div>
    					</div>
    					<div id="pilltechselected" class="tab-pane fade">
    						<div id="selectedtechgriddiv"><jsp:include page="selectedTechGrid.jsp"></jsp:include></div>
    					</div>
    				</div>
				</div>
				<div class="modal-footer text-right">
	            	<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
	            	<button type="button" class="btn btn-default btn-primary" id="btntechsave">Update</button>
	          	</div>
			</div>
		</div>
	</div>
...
<!--           <div class="modal-footer">
  ...
  <input type="hidden" name="delaysmsconfig" id="delaysmsconfig">
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $("#techhrs").jqxDateTimeInput({ width: '80px', height: '15px', formatString:"HH:mm",showCalendarButton:false});
        
        $("#delaydate").jqxDateTimeInput({ width: '125px', height: '15px', formatString:"dd/MM/yyyy"});
        
        $('#btnsendsms').hide();
        $('.load-wrapp').hide();
        
        $('#delaydate').on('change', function (event){  
            var jsDate = event.args.date; 
            var type = event.args.type; // keyboard, mouse or null depending on how the date was selected.
         	var deliverdate=$('#delaydate').jqxDateTimeInput('val');
            funSetDelaySMSText(deliverdate,'date');
        });
        
        $("#delayreason").keyup(function(){
        	var delivertext=$(this).val();
        	funSetDelaySMSText(delivertext,'text');
        });
        
        $('#btnsenddelaysms').click(function(){
        	var jobdocno=$('#jobcarddocno').val();
        	var msg=$('#delaysmstext').text();
        	$.post('sendDelaySMS.jsp',{'message':msg,'jobdocno':jobdocno},function(data,status){
        ...
        $('#btntechsave').click(function(){
        	...
        	var techdocno=$('#cmbtechnician').val();
        	if(techdocno==null || techdocno=="" || techdocno=="undefined" || typeof(techdocno)=="undefined"){
        		Swal.fire({
					type: 'error',
					title: 'Warning',
        	...
        	var techremarks=$('#techremarks').val();
        	var techhrs=$('#techhrs').jqxDateTimeInput('getDate');
        	if(techhrs==null || techhrs=="undefined" || typeof(techhrs)=="undefined" || techhrs==""){
        		Swal.fire({
					type: 'error',
					title: 'Warning',
					text: 'Please select a technician'
        	...
        	techhrs=$('#techhrs').jqxDateTimeInput('val');
        	Swal.fire({
	  			title: 'Are you sure?',
	  			text: "Do you want to update technician for Job Card "+jobvocno,
	  			icon: 'warning',
	  			...
	  			if (result.isConfirmed) {
	  				$.post('saveTechData.jsp',{'jobdocno':jobdocno,'techdocno':techdocno,'techhrs':techhrs,'remarks':techremarks},function(data,status){
	  	        		data=JSON.parse(data);
	  	        		if(data.errorstatus=="0"){
	  	        			Swal.fire({
	  	    					type: 'success',
	  	    					title: 'Success',
	  	    					text: 'Technician Allocated'
	  	    				});	
        ...
        function funSetDelaySMSText(delivervalue,delivertype){
        	if(delivertype=='date'){
        		$('#modaldelaysms').find('.hiddelaysmstext').find('.deliverdate').text(delivervalue);
        	}
        	else if(delivertype=='text'){
        		$('#modaldelaysms').find('.hiddelaysmstext').find('.delayreason').text(delivervalue);
        	}
        	$('#delaysmstext').text($('#modaldelaysms').find('.hiddelaysmstext').text());
        }
        function bayUpdate(){
        	$('.load-wrapp').show();
        	var jobcarddocno=$('#jobcarddocno').val();
    ...
    function getInitData(){
		...
		x.onreadystatechange=function(){
			if (x.readyState==4 && x.status==200)
			{
				...
				if(items.techconfig=="1"){
					$('#btnteamselection').hide();
					$('#btntechselection').show();
					
				}
				else{
					$('#btnteamselection').show();
					$('#btntechselection').hide();
				}
				$('#delaysmsconfig').val(items.delaysms);
				if(items.delaysms=="1"){
					$('#btndelaysms').show();
				}
				else{
    ...
    function funGetCountData(){
		...
		x.onreadystatechange=function(){
			if (x.readyState==4 && x.status==200)
			{
				...
				var data=JSON.parse(items[21]);
				$.each(data.technicianarray,function(key,value){
					htmldata+='<option value="'+value.docno+'">'+value.name+'</option>';
				});
				$('#cmbtechnician').html($.parseHTML(htmldata));
				$('#cmbtechnician').select2({
...
function funSendSMS(){
	if($('#jobcarddocno').val()=='' || $('#jobcarddocno').val()=='0'){
		...
		Swal.fire({
  			title: 'Are you sure?',
  			text: "Do you want to send SMS for jobcard "+jobcardvocno+"?",
  			icon: 'warning',
  			showCancelButton: true,
  			confirmButtonColor: '#3085d6',
  			cancelButtonColor: '#d33',
  			...
  			if (result.isConfirmed) {
    			var jobdocno=$('#jobcarddocno').val();
				$.post('sendSMS.jsp',{'jobdocno':jobdocno,'dtype':'FLR'},function(data,status){
					data=JSON.parse(data);					
					if(data.errorstatus=="0"){
						Swal.fire({
							icon: 'success',
