...
//		System.out.println(cnt +"  "+rsgetbranch.getString("permission"));
	...
	String strconfig="select 3 srno,field_nme,method,coalesce(value,0) value from gl_config where field_nme='printEstDropdownOrder'";
	ResultSet rsconfig=stmt.executeQuery(strconfig);
	int printestorder=0;
	while(rsconfig.next()){
		printestorder=rsconfig.getInt("method");
	}
	objdata.put("branchdata",brancharray);
	objdata.put("printestorder",printestorder);
}
catch(Exception e){
	e.printStackTrace();
}
