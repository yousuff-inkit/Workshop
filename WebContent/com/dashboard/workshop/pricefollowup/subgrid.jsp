...
<script type="text/javascript">
	...
	if(temp1!='NA')
{
		vehdatas= '<%=DAO.subDetails(fromdate,cldocno,check,"") %>';
...
$(document).ready(function () {
    ...
    $('#userdetails').on('rowdoubleclick', function (event) 
    		{ 
    		    ...
    		    if(rds=='A'){
    		    	var check=1;
    		    $("#fleetdiv").load("detailsgrid.jsp?rds="+rds+"&check="+check+"&froms="+'<%=fromdate%>'+"&cldoc="+'<%=cldocno%>'+"&process=1");
    		    }
    		    if(rds=='B'){
    		    	var check=1;
    		    $("#fleetdiv").load("detailsgrid.jsp?rds="+rds+"&check="+check+"&froms="+'<%=fromdate%>'+"&cldoc="+'<%=cldocno%>'+"&process=2");
    		    }
    		    if(rds=='C'){
    		    	var check=1;
    		    $("#fleetdiv").load("detailsgrid.jsp?rds="+rds+"&check="+check+"&froms="+'<%=fromdate%>'+"&cldoc="+'<%=cldocno%>'+"&process=4");
    		    }
    		    if(rds=='D'){
    		    	var check=1;
    		    $("#fleetdiv").load("detailsgrid.jsp?rds="+rds+"&check="+check+"&froms="+'<%=fromdate%>'+"&cldoc="+'<%=cldocno%>'+"&process=5");
    		    }
    		});
});
