...
$(document).ready(function () {
	...
	var detsparedata=[];
	if(id=="1"){
		detsparedata='<%=dao.getDetailSpareData(regno,pltid,fromdate,todate,id)%>';
	}
	 ...
	 $("#detailSpareGrid").jqxGrid(
            {
                ...
                handlekeyboardnavigation: function (event) {
                    /* var cell1 = $('#detailSpareGrid').jqxGrid('getselectedcell');
                    if (cell1 != undefined && cell1.datafield == 'amount') {
                        var key = event.charCode ? event.charCode : event.keyCode ? event.keyCode : 0; 
                        if ((key == 9)) {
                        	$("#detailSpareGrid").jqxGrid('addrow',null,{});
							return true;						
						}
                    } */
				},
