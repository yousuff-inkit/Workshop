...
try{
	...
	for(int i=0;i<invarray.size();i++){
		...
		String estaddition="";
		String strgetaddition="select addition from ws_invcalctemp where rowno="+rowno;
		System.out.println("addition:"+strgetaddition);
		ResultSet rsgetadd=stmt.executeQuery(strgetaddition);
		while(rsgetadd.next()){
			estaddition=rsgetadd.getString("addition");
		}
		
		vat=processdao.getVATAmount(jobdocno, estaddition, conn);
...
" where card.doc_no="+jobcarddocno+" and card.status=3  and jcc.addition in ("+detaddition+") and jcc.lumsumstatus=0  and jcc.dettype='esp' and jcc.chkcomplete=1 "+ 
...
" where card.doc_no="+jobcarddocno+" and card.status=3  and jcc.addition in ("+detaddition+") and jcc.lumsumstatus=0 and jcc.dettype='gis' and jcc.chkcomplete=1 union all "+
...
" where card.doc_no="+jobcarddocno+" and card.status=3  and jcc.addition in ("+detaddition+") and jcc.lumsumstatus>0)a";
			System.out.println("ws_investdetail ===== "+strsql);
			...
			for(int detindex=0;detindex<detailarray.size();detindex++){
				...
				if(Double.parseDouble(jobvatamount)==0.0){
					jobnetamount=jobtotal;
				}
