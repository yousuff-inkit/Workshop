...
try{
	...
	while(rs.next()){
		...
		if(tax>0 && clienttax>0){
			vatamt=processdao.getVATAmount(jobdocno, rs.getString("addition"), conn);
		}
		double total=nettotal+vatamt;
		double nontaxamt=rs.getDouble("nontaxamt");
		double netbill=total+nontaxamt;
		calcarray.add(rs.getInt("insurstatus")+" :: "+rs.getInt("acno")+" :: "+rs.getString("claimno")+" :: "+rs.getInt("jobdocno")+" :: "+rs.getDouble("amount")+" :: "+nettotal+" :: "+vatamt+" :: "+total+" :: "+rs.getDouble("excess")+" :: "+netbill+" :: "+rs.getString("pono")+" :: "+rs.getString("description")+" :: "+rs.getString("addition")+" :: "+rs.getString("regno")+" :: "+nontaxamt);  
	}
	...
	for(int i=0;i<calcarray.size();i++){
		...
		" vatamount, totalamount,excessamount, netbill,insurstatus,description,pono,addition,regno,nontaxamt)values("+
		" "+temp[3].trim()+","+temp[1].trim()+",'"+temp[2].trim()+"',"+temp[4].trim()+","+temp[5].trim()+","+temp[6].trim()+","+temp[7].trim()+","+temp[8].trim()+","+temp[9].trim()+","+temp[0].trim()+",'"+desc1+"','"+temp[10].trim()+"','"+temp[12].trim()+"','"+temp[13].trim()+"',"+temp[14]+")";
