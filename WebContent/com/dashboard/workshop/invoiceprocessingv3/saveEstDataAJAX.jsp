		...
		//	System.out.println("===== "+estarray.get(i).split("::")[9]+"====="+estarray.get(i).split("::")[9].equalsIgnoreCase("Insur.Company"));
			...
			if(sqlpodatenew!=null){
				strsql="insert into ws_investdata(jobdocno, chkmultiple, estno, labourtotal, sparetotal, nettotal, chkclaim, claimno, excess, pono, "+
						" podate, vattype, status,addition,insurtypedocno,nontaxamt)values("+jobcarddocno+","+chkmultiple+",'"+estno+"',"+labourtotal+","+sparetotal+","+nettotal+","+chkclaim+","+
						" '"+claimno+"',"+excess+",'"+pono+"','"+sqlpodatenew+"',"+vattype+",3,"+addition+","+insurtypedocno+","+nontaxamt+")";
	...
	if(chkmultiple.equalsIgnoreCase("0")){
		...
		if(sqlpodate!=null){
			strinsert="insert into ws_investdata(jobdocno, chkmultiple, estno, labourtotal, sparetotal, nettotal, chkclaim, claimno, excess, pono, "+
					" podate, vattype, status,addition,nontaxamt)values("+jobcarddocno+","+chkmultiple+",'"+estno+"',"+labourtotal+","+sparetotal+","+nettotal+","+(Double.parseDouble(strexcess)>0.0 || insurancecomp==1 ? 1 : 0)+","+
					" '"+strclaimno+"',"+strexcess+",'"+strpono+"','"+sqlpodate+"',"+strvattype+",3,0,"+nontaxamt+")";	
		}
		else{
			strinsert="insert into ws_investdata(jobdocno, chkmultiple, estno, labourtotal, sparetotal, nettotal, chkclaim, claimno, excess, pono, "+
					" podate, vattype, status,addition,nontaxamt)values("+jobcarddocno+","+chkmultiple+",'"+estno+"',"+labourtotal+","+sparetotal+","+nettotal+","+(Double.parseDouble(strexcess)>0.0 || insurancecomp==1 ? 1 : 0)+","+
