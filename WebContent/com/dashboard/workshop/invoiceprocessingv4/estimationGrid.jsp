<%@page import="com.dashboard.workshop.invoiceprocessingv4.*"%>
<%
ClsInvProcessingV4DAO jobsdao=new ClsInvProcessingV4DAO();
...
String clientinsurance=request.getParameter("clientinsurance")==null?"":request.getParameter("clientinsurance");
%>
<style>
.redClass
   		{
...
var list = ['Shared', 'Insur.Company'];
 if(id=='1'){
	  estimatedata='<%=jobsdao.getEstimateData(jobcard,id,seccldocno)%>';
...
$(document).ready(function () {
    ...
    $("#estimationGrid").on("bindingcomplete", function (event) {
    	...
    	var clientinsurance='<%=clientinsurance%>';
		for(var i=0;i<rows.length;i++){
    		var chkmultiple=$('#estimationGrid').jqxGrid('getcellvalue',i,'chkmultiple');
    		console.log('Value Check'+chkmultiple+"::"+clientinsurance);
    		if(chkmultiple=='0'){
    		...
    		if(clientinsurance=='1'){
    			$('#estimationGrid').jqxGrid('setcellvalue', i, 'chkclaim',true);
    		}
    	}
        ...
        columns: [
             ...
             cellsrenderer: function (row, column, value) {
              return "<center><div style='margin:4px;'>" + (value + 1) + "</div></center>";
       				...
       				{ text: 'Est No',datafield:'estno',width:'7%', editable: false},
       				...
       				{ text: 'Non Taxable', datafield: 'nontaxamt', width:"7%",cellsformat:'d2',align:'right',cellsalign:'right', editable: false,aggregates: ['sum'],aggregatesrenderer:rendererstring},
       				{ text: 'Claim', datafield: 'chkclaim', width:"6%",columntype:'checkbox',cellbeginedit:isEditableMain},
       				{ text: 'Claim No',datafield:'claimno',width:'11%',cellbeginedit:isEditable},
       				{ text: 'Excess',datafield:'excess',width:'7%',cellbeginedit:isEditable,cellsformat:'d2',cellsalign:'right',align:'right'},
       				{ text: 'PO No',datafield:'pono',width:'8%',cellbeginedit:isEditable},
       				{ text: 'PO Date', datafield: 'podate', width:'9%',columntype:'datetimeinput',cellbeginedit:isEditable,cellsformat:'dd.MM.yyyy'},
       				{ text: 'VAT Type', datafield: 'vattype', width:'9%',cellbeginedit:isEditable,columntype:'dropdownlist',
