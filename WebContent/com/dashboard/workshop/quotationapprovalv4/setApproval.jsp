...
String strlabourrows=request.getParameter("labourrows")==null?"":request.getParameter("labourrows");
String strsparerows=request.getParameter("sparerows")==null?"":request.getParameter("sparerows");
System.out.println("Labour:"+strlabourrows);
System.out.println("Spare:"+strsparerows);
...
try{
	 ...
	 conn.setAutoCommit(false);
		...
		if(!strlabourrows.trim().equalsIgnoreCase("")){
			for(int i=0;i<strlabourrows.split(",").length;i++){
				labourarray.add(strlabourrows.split(",")[i]);
			}	
		}
		
		ArrayList<String> sparearray=new ArrayList();
		if(!strsparerows.trim().equalsIgnoreCase("")){
			for(int i=0;i<strsparerows.split(",").length;i++){
				sparearray.add(strsparerows.split(",")[i]);
			}	
		}
		...
		int seccldocno=0;
		String strgate= "select gate.cldocno from ws_estm est left join ws_gateinpass gate on est.gipno=gate.doc_no where est.doc_no="+estDocno;
		ResultSet rsgate=stmt.executeQuery(strgate);
		while(rsgate.next()){
			seccldocno=rsgate.getInt("cldocno");
		}
		String strupdateappr="";
		if(Integer.parseInt(addition)==0){
			strupdateappr="update ws_estm set approved="+action+" where doc_no="+estDocno;
		}
		else if(Integer.parseInt(addition)>0){
			strupdateappr="update ws_estmadd set approved="+action+" where doc_no="+estDocno+" and addition="+addition;
		}
		int updateappr=conn.createStatement().executeUpdate(strupdateappr);
		if(updateappr<=0){
			System.out.println("Estimation("+addition+") Update Error");
			System.out.println(strupdateappr);
			errorstatus=1;
		}
		for(int i=0;i<labourarray.size();i++){
			int rowno=Integer.parseInt(labourarray.get(i));
			String strSql1 = "update ws_estlabour set seccldocno="+seccldocno+",approved="+action+" where rdocno="+estDocno+" and confirmed=1 and approved=0 and addition="+addition+" and rowno="+rowno;	
			//System.out.println(strSql1);
			int labour=stmt.executeUpdate(strSql1);
			if(labour<=0){
				System.out.println("Est Labour Update Error");
				System.out.println(strSql1);
				errorstatus=1;
			}
		}
		for(int i=0;i<sparearray.size();i++){
			int rowno=Integer.parseInt(sparearray.get(i));
			String strSql2 = "update ws_estspare set seccldocno="+seccldocno+",approved="+action+" where rdocno="+estDocno+" and confirmed=1 and approved=0 and addition="+addition+" and rowno="+rowno;	
			int spare=stmt.executeUpdate(strSql2);
			
			if(spare<=0){
				System.out.println("Est Spare Update Error");
		...
		if((z<=0)||(p<=0)||(q<=0)){
			if(z<=0){
				System.out.println("Est Approve Master Insert Error");
				System.out.println(strSql3);
			}
			if(p<=0){
