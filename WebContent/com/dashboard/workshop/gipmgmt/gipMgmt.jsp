    ...
    <div class="row rowgap">
      <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        ...
        <div class="detailpanel custompanel">
        	...
        	<button type="button" class="btn btn-default" id="btnsms" data-target="#modalsms"><i class="fa fa-commenting" aria-hidden="true" data-toggle="tooltip" title="Send SMS" data-placement="bottom"></i></button>
    	...
    	<div class="modal-dialog">
        	<div class="modal-content">
          		<div class="modal-header">
            		...
            		<h4 class="modal-title">SMS for GIP#<span class="gipvocno"></span></h4>
          		</div>
          		<div class="modal-body">
					<textarea class="form-control" id="smstext" style="height:130px;"></textarea>    				       	
            	</div>
            	<div class="modal-footer">
            		<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
          			<button type="button" name="btnsendsms" id="btnsendsms" class="btn btn-default btn-primary">Send SMS</button>
          		</div>
          	</div>
		</div>
	</div>
  ...
  <input type="hidden" name="refno" id="refno">
   <input type="hidden" name="fileattachconfig" id="fileattachconfig">
     ...
     <input type="hidden" name="checklistconfig" id="checklistconfig">
       ...
       <input type="hidden" name="smsconfig" id="smsconfig">
    ...
    $(document).ready(function(){
	   ...
	   $('#cmbbranch,.cmbbaystatus,.cmbbaystatusupdate,.cmbteamupdatebay,#cmbgopfuel').select2();
        ...
        $('#btnsendsms').click(function(){
        	...
        	var smstext=$('#smstext').text();
        	var rowindex=$('#rowindex').val();
        	var mobile=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'mobile');
        	var clientname=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'refname');
        	var brhid=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'brhid');
        	
        	$.post('sendSMS.jsp',{'gatedocno':gatedocno,'smstext':smstext,'mobile':mobile,'clientname':clientname,'brhid':brhid},function(data,status){
        ...
        $('#btnattach').click(function(){
        	...
        	var rowindex=$('#rowindex').val();
        	var docno=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'docno');
        	var vocno=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'vocno');
			var brhid=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'brhid');
        	var processstatus=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'processstatus');
        ...
        $('#btnclosegip').click(function(){
        	...
        	var jobdocno=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'jobdocno');
        	var invalid=0;
        	if(jobdocno!="" && jobdocno!="undefined" && jobdocno!=null && typeof(jobdocno)!="undefined" && parseInt(jobdocno)>0){
        		//alert(processstatus);
        		if(parseInt(processstatus)==10){
        			Swal.fire({
						...
						type: 'error',
						title: 'Warning',
						text: 'Vehicle Release,not able to close'
					});
					invalid=1;
					return false;	
        		}
        		if(parseInt(processstatus)<6){
        			Swal.fire({
						icon:'warning',
						type: 'error',
						title: 'Warning',
						text: 'Please complete jobcard'
					});
					invalid=1;
					return false;
        		}
        	...
        	if(invalid==0){
        		$('#modalclosegip').find('span.gipvocno').text(vocno);
        		$('#modalclosegip').modal('show');
        	}
        	
        });
        
        $('#btnclosegipsave').click(function(){
        	var rowindex=$('#rowindex').val();
        	var docno=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'docno');
        	var vocno=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'vocno');
			var brhid=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'brhid');
        	...
        	var closegipreason=$('#closegipreason').val();
        	if($('#closegipreason').val()==''){
        		$('#closegipreason').closest('.form-group').find('span.help-block').remove();
        		$('#closegipreason').closest('.form-group').append($.parseHTML('<span class="help-block" style="color:red;"><small>Mandatory</small></span>'));
        		return false;
        	}
        	else{
        		$('#closegipreason').closest('.form-group').find('span.help-block').remove();
        	...
        	Swal.fire({
	  				title: 'Are you sure?',
	  				text: "Do you want to close GIP#"+vocno,
	  				icon: 'warning',
						...
						function(data,status){
							...
							if(data.errorstatus=="0"){
								Swal.fire({
									...
									type: 'success',
									title: 'Success',
									text: 'GIP Closed Successfully'
								});
								$('.textpanel p').text('');
								$('#modalclosegip').find('input').val('');
								$('#modalclosegip').modal('hide');
        ...
        $('#btnprintopenjobcard').click(function(){
        	...
        	var reurl=url.split("com/dashboard/");
 			var gatedocno=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'docno');
 			var estdocno=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'estdocno');
 			var jobdocno=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'jobdocno');
            ...
            if(brhid=='' || brhid==null || brhid=="undefined" || typeof(brhid)=="undefined"){
 			...
 			var path= "com/dashboard/workshop/gipmgmt/printgipmgmt.action?branch="+brhid;  
        ...
        $('#btnprintgip,#btnprintest,#btnprintjobcard').click(function(){
        	...
        	if (targetid=="btnprintgip") {
				var reurl=url.split("com/dashboard/");
				//var docno=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'vocno');
				var win= window.open(reurl[0]+"com/workshop/gateinpassmaster/Gateinpassprint?docno="+docno+"&brhid="+brhid+"&formdetailcode=GIP","_blank","top=250,left=310,Width=800,Height=800,location=no,scrollbars=no,toolbar=yes");
				...
				var reurl=url.split("com/dashboard/");
				var docno=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'estdocno');
				var estvocno=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'estvocno');
				var gatedoc=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'docno');
				var brhid=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'brhid');      
	        	...
	        	var jobvocno=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'jobvocno');
        ...
        $('.card-item').click(function(){
			...
			if($(this).hasClass('active')){
				...
				$('#floorMgmtGrid').jqxGrid('removefilter','gipprocess', true);
			}
	    					...
	    					function(data,status){
	    						...
	    						if(data.errorstatus=='0'){
	    							Swal.fire({
				          			...
				          			$('#floormgmtgriddiv').load('floorMgmtGrid.jsp?id=1&brhid='+$("#cmbbranch").val());
	    						}
	    						else{
	    							Swal.fire({
										icon:'warning',
        ...
        $('#btnqotapproval').click(function(){
        	...
        	var vocno=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'vocno');
        	var processstatus=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'processstatus');
	        if($('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'customertype')=='New'){
	        	Swal.fire({
					icon:'warning',
					type: 'error',
					title: 'Warning',
        ...
        $('#btnchangebillto').click(function(){
        	$.get("getCountData.jsp", function(data, status){
				...
				$.each(data.clientdata,function(index,value){
	  			...
	  			var rowindex=$('#rowindex').val();
	  			var cldocno=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'cldocno');
			 	var insurcompcldocno=$('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'insurcldocno');
		        ...
		        /*else if($('#floorMgmtGrid').jqxGrid('getcellvalue',rowindex,'customertype')=='New'){
		        	Swal.fire({
						icon:'warning',
						type: 'error',
						title: 'Warning',
        ...
        $('#btnexcel').click(function(){
        	$("#floorMgmtGrid").excelexportjs({
				...
				worksheetName: "GIP Management Data"
			});
        });
		...
		x.onreadystatechange=function(){
			if (x.readyState==4 && x.status==200)
			{
				...
				if(parseInt(items.split("::")[0])>0){
				 ...
				 $('#smsconfig').val(items.split("::")[3]);
				 if(parseInt(items.split("::")[3])>0){
					 $('#btnsms').show();
				 }else{
					 $('#btnsms').hide();
				 }
