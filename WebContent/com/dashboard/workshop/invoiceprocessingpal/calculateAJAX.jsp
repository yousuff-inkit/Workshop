...
try{
	...
	if(claimconfig==1){
		...
		" ((sum(m.nettotal)-sum(m.excess))+((sum(m.nettotal))*("+vatpercent+"/100))) netbill from ws_investdata m left join ws_jobcard job on job.doc_no=m.jobdocno left"+
		...
		" sum(m.excess) excess,(sum(m.nettotal)-sum(m.excess))+((sum(m.nettotal)-sum(m.excess))*("+vatpercent+"/100)) netbill from ws_investdata m left join ws_jobcard job on"+
		...
		" if(m.vattype=1,sum((m.excess))*("+vatpercent+"/100),0)) netbill from ws_investdata m left join ws_jobcard job on job.doc_no=m.jobdocno left join"+
		...
		" nettotal,sum((m.nettotal))*("+vatpercent+"/100) vatamt,sum((m.nettotal))+sum(((m.nettotal))*("+vatpercent+"/100)) total,0.0 excess,sum((m.nettotal))+sum(((m.nettotal))*("+vatpercent+"/100)) netbill"+
		...
		strsql="select  concat(coalesce(gate.regno,''),' ',coalesce(gate.pltid,'')) regno,m.addition,concat('CL - ',coalesce(m.claimno,''),' PO - ',coalesce(m.pono,''),' JCB - ',coalesce(job.voc_no,''),' Regn - ',concat(coalesce(gate.regno,''),' ',coalesce(gate.pltid,''))) description,1 insurstatus,ac.acno,m.claimno,m.pono,m.jobdocno,m.nettotal-m.excess amount,0 discount,m.nettotal-m.excess nettotal,(m.nettotal)*("+vatpercent+"/100) vatamt,(m.nettotal-m.excess)+((m.nettotal)*("+vatpercent+"/100)) total, m.excess,((m.nettotal-m.excess)+((m.nettotal)*("+vatpercent+"/100))) netbill from ws_investdata m left join ws_jobcard job on job.doc_no=m.jobdocno left join ws_estm est on (job.reftype='EST' and job.refno=est.doc_no) left join ws_gateinpass gate on (if(job.reftype='GIP',job.refno=gate.doc_no,est.gipno=gate.doc_no)) left join my_acbook ac on (gate.insurcldocno=ac.cldocno and ac.dtype='CRM') where chkclaim=1 and m.vattype=2 and m.jobdocno="+jobdocno+" and (m.nettotal-m.excess)!=0 union all"+
		" select  concat(coalesce(gate.regno,''),' ',coalesce(gate.pltid,'')) regno,m.addition,concat('CL - ',coalesce(m.claimno,''),' PO - ',coalesce(m.pono,''),' JCB - ',coalesce(job.voc_no,''),' Regn - ',concat(coalesce(gate.regno,''),' ',coalesce(gate.pltid,''))) description,1 insurstatus,ac.acno,m.claimno,m.pono,m.jobdocno,m.nettotal-m.excess amount,0 discount,m.nettotal-m.excess nettotal,(m.nettotal-m.excess)*("+vatpercent+"/100) vatamt,(m.nettotal-m.excess)+((m.nettotal-m.excess)*("+vatpercent+"/100)) total, m.excess,(m.nettotal-m.excess)+((m.nettotal-m.excess)*("+vatpercent+"/100)) netbill from ws_investdata m left join ws_jobcard job on job.doc_no=m.jobdocno left join ws_estm est on (job.reftype='EST' and job.refno=est.doc_no) left join ws_gateinpass gate on (if(job.reftype='GIP',job.refno=gate.doc_no,est.gipno=gate.doc_no)) left join my_acbook ac on (gate.insurcldocno=ac.cldocno and ac.dtype='CRM') where chkclaim=1 and m.vattype=1  and m.jobdocno="+jobdocno+" and (m.nettotal-m.excess)!=0 union all "+
		" select  concat(coalesce(gate.regno,''),' ',coalesce(gate.pltid,'')) regno,m.addition,concat('CL - ',coalesce(m.claimno,''),' PO - ',coalesce(m.pono,''),' JCB - ',coalesce(job.voc_no,''),' Regn - ',concat(coalesce(gate.regno,''),' ',coalesce(gate.pltid,''))) description,2 insurstatus,ac.acno,m.claimno,m.pono,m.jobdocno,m.excess amount,0 discount,m.excess nettotal,if(m.vattype=1,(m.excess)*("+vatpercent+"/100),0) vatamt,(m.excess)+(if(m.vattype=1,(m.excess)*("+vatpercent+"/100),0)) total,0 excess,(m.excess)+(if(m.vattype=1,(m.excess)*("+vatpercent+"/100),0)) netbill from ws_investdata m left join ws_jobcard job on job.doc_no=m.jobdocno left join ws_estm est on (job.reftype='EST' and job.refno=est.doc_no) left join ws_gateinpass gate on (if(job.reftype='GIP',job.refno=gate.doc_no,est.gipno=gate.doc_no)) left join my_acbook ac on (gate.cldocno=ac.cldocno and ac.dtype='CRM') where chkclaim=1 and m.excess!=0  and m.jobdocno="+jobdocno+" union all"+
		" select  concat(coalesce(gate.regno,''),' ',coalesce(gate.pltid,'')) regno,m.addition,concat('CL - ',coalesce(m.claimno,''),' PO - ',coalesce(m.pono,''),' JCB - ',coalesce(job.voc_no,''),' Regn - ',concat(coalesce(gate.regno,''),' ',coalesce(gate.pltid,''))) description,0 insurstatus,ac.acno,m.claimno,m.pono,m.jobdocno,m.nettotal amount,0 discount,m.nettotal nettotal,(m.nettotal)*("+vatpercent+"/100) vatamt,(m.nettotal)+((m.nettotal)*("+vatpercent+"/100)) total,0.0 excess,(m.nettotal)+((m.nettotal)*("+vatpercent+"/100)) netbill from ws_investdata m left join ws_jobcard job on job.doc_no=m.jobdocno left join ws_estm est on (job.reftype='EST' and job.refno=est.doc_no) left join ws_gateinpass gate on (if(job.reftype='GIP',job.refno=gate.doc_no,est.gipno=gate.doc_no)) left join my_acbook ac on (gate.cldocno=ac.cldocno and ac.dtype='CRM') where chkclaim=0 and m.excess=0 and m.jobdocno="+jobdocno;
	}
	...
	while(rs.next()){
		calcarray.add(rs.getInt("insurstatus")+" :: "+rs.getInt("acno")+" :: "+rs.getString("claimno")+" :: "+rs.getInt("jobdocno")+" :: "+rs.getDouble("amount")+" :: "+rs.getDouble("nettotal")+" :: "+rs.getDouble("vatamt")+" :: "+rs.getDouble("total")+" :: "+rs.getDouble("excess")+" :: "+rs.getDouble("netbill")+" :: "+rs.getString("pono")+" :: "+rs.getString("description")+" :: "+rs.getString("addition")+" :: "+rs.getString("regno"));  
	}
	...
	for(int i=0;i<calcarray.size();i++){
		...
		" vatamount, totalamount,excessamount, netbill,insurstatus,description,pono,addition,regno)values("+
		" "+temp[3].trim()+","+temp[1].trim()+",'"+temp[2].trim()+"',"+temp[4].trim()+","+temp[5].trim()+","+temp[6].trim()+","+temp[7].trim()+","+temp[8].trim()+","+temp[9].trim()+","+temp[0].trim()+",'"+desc1+"','"+temp[10].trim()+"','"+temp[12].trim()+"','"+temp[13].trim()+"')";
