...
<script type="text/javascript">
       var data5;
        $(document).ready(function () { 	
            ...
            $("#jqxApplyCashReceiptInvoicing").jqxGrid(
            {
                width: '100%',
                height: 150,
                source: dataAdapter,
            ...
            if(tempToAcc>0){
        	 ...
        	 var applied1="";
          	 $("#jqxApplyCashReceiptInvoicing").on('cellvaluechanged', function (event){
                 var rowindex1=event.args.rowindex;
                 var value = $('#jqxApplyCashReceiptInvoicing').jqxGrid('getcellvalue', rowindex1, "tramt");
                 var value1=$("#jqxApplyCashReceiptInvoicing").jqxGrid('getcellvalue', rowindex1, "applying");
                 var balance= value - value1;
                 $('#jqxApplyCashReceiptInvoicing').jqxGrid('setcellvalue', rowindex1, "balance",balance);
                
                var applied=$('#jqxApplyCashReceiptInvoicing').jqxGrid('getcolumnaggregateddata', 'applying', ['sum'], true);
          	...
          	$("#jqxApplyCashReceiptInvoicing").on('cellvaluechanged', function (event) {
                ...
                var value = $('#jqxApplyCashReceiptInvoicing').jqxGrid('getcellvalue', rowIndex, "tramt");
                var value1=$("#jqxApplyCashReceiptInvoicing").jqxGrid('getcellvalue', rowIndex, "applying");
          		var amount = document.getElementById("txtapplyinvoiceamt").value;
          		var applied=$('#jqxApplyCashReceiptInvoicing').jqxGrid('getcolumnaggregateddata', 'applying', ['sum'], true);
          		applied1=parseFloat(applied.sum); 
          		if(dataField=="applying"){
          			if(applied1>amount){ 
          		        $("#jqxApplyCashReceiptInvoicing").jqxGrid('showvalidationpopup', rowIndex, "applying", "Limit Already Reached,Invalid Amount.");
          		        $('#txtvalidation').val(1);
          		         return true;  
          		        }
          		    else if(value1>value){
          		    	$("#jqxApplyCashReceiptInvoicing").jqxGrid('showvalidationpopup', rowIndex, "applying", "Invalid Amount.");
          		    	$('#txtvalidation').val(1);
         		         return true; 
          		    }  
          		    else{  
          		        $("#jqxApplyCashReceiptInvoicing").jqxGrid('hidevalidationpopups');
          		         $('#txtvalidation').val(0);
          		        return false;  
          		        }
          		}      		
    ...
    <div id="jqxApplyCashReceiptInvoicing"></div>
