...
System.out.println("-----list-------"+list);
 ...
 for(int i=0;i<aa.length;i++){
	...
	//  String prdids=""+serarray[0].equalsIgnoreCase("undefined") || serarray[0].equalsIgnoreCase("") || serarray[0].trim().equalsIgnoreCase("NaN")|| serarray[0].isEmpty()+"";						 
	 ...
	 {
		 sql="select (sum(op_qty)-sum(out_qty+rsv_qty+del_qty)) acqty from my_prddin where brhid='"+branch+"'   and specno='"+specno+"' and prdid='"+prdid+"'  "; 
	 }
	 else
	 {
		 sql="select (sum(op_qty)-sum(out_qty+rsv_qty+del_qty))+"+oldqty+" acqty from my_prddin where brhid='"+branch+"'   and specno='"+specno+"' and prdid='"+prdid+"'  ";
	 }
