        ...
        $(document).ready(function () { 	
            ...
            $("#attendanceGridID").on("bindingcomplete", function (event) {
					 ...
					 for(var k=0 ; k < leavetype ; k++){
						 ...
						 if($('#cmbleavetype option').eq(k).text().trim()=='Compassionate Leave'){
						 ...
						 if($('#cmbleavetype option').eq(k).text().trim()=='Paid Leave'){
            ...
            $("#attendanceGridID").jqxGrid(
            {
                ...
                columns: [
							...
							{ text: 'Emergency Leave', datafield: 'leave1total', width: '10%', cellsalign: 'center', align: 'center', cellbeginedit: function (row) {
									if ($('#attendanceGridID').jqxGrid('getcellvalue', row, "leave1total")>=0) { 
								         if(parseInt($('#txtattendanceleaveseditgrid').val())==0) {
								              return false;
								         }}}
							},
							{ text: 'Sick Leave', datafield: 'leave2total', width: '7%', cellsalign: 'center', align: 'center', cellbeginedit: function (row) {
						         	if ($('#attendanceGridID').jqxGrid('getcellvalue', row, "leave2total")>=0) { 
							         	if(parseInt($('#txtattendanceleaveseditgrid').val())==0) {
							              	return false;
							         	}}}
							},
							{ text: 'Compassionate Leave', datafield: 'leave3total', width: '10%', cellsalign: 'center', align: 'center', cellbeginedit: function (row) {
									if ($('#attendanceGridID').jqxGrid('getcellvalue', row, "leave3total")>=0) { 
							         	if(parseInt($('#txtattendanceleaveseditgrid').val())==0) {
							              	return false;
							         	}}}
							},
							{ text: 'Annual Leave', datafield: 'leave4total', width: '8%', cellsalign: 'center', align: 'center', cellbeginedit: function (row) {
									if ($('#attendanceGridID').jqxGrid('getcellvalue', row, "leave4total")>=0) { 
							         	if(parseInt($('#txtattendanceleaveseditgrid').val())==0) {
							              	return false;
							         	}}}
						    },
							{ text: 'Paid Leave', datafield: 'leave5total', width: '8%', cellsalign: 'center', align: 'center', cellbeginedit: function (row) {
									if ($('#attendanceGridID').jqxGrid('getcellvalue', row, "leave5total")>=0) { 
							         	if(parseInt($('#txtattendanceleaveseditgrid').val())==0) {
							            	  return false;
							         	}}}
						    },
							{ text: 'Unpaid Leave', datafield: 'leave6total', width: '8%', cellsalign: 'center', align: 'center', cellbeginedit: function (row) {
									if ($('#attendanceGridID').jqxGrid('getcellvalue', row, "leave6total")>=0) { 
							        	if(parseInt($('#txtattendanceleaveseditgrid').val())==0) {
							            	  return false;
							         	}}}
						    },
							{ text: 'Total OT', datafield: 'totalovertime', editable: false, width: '7%', hidden: true },
							{ text: 'Total HOT', datafield: 'totalholidayovertime', editable: false, width: '7%', hidden: true },
                ...
                for(var i=1;i<=parseInt(totdays);i++)
         		{
                	...
                	if(value=='T'){leave1total=leave1total+1;} 
                	if(value=='S'){leave2total=leave2total+1;}
                	if(value=='C'){leave3total=leave3total+1;}
                	if(value=='A'){leave4total=leave4total+1;}
                	if(value=='M'){leave5total=leave5total+1;}
                	if(value=='U'){leave6total=leave6total+1;}
                	if(value=='T2'){leave1total=leave1total+(0.5);} 
                	if(value=='S2'){leave2total=leave2total+(0.5);}
                	if(value=='C2'){leave3total=leave3total+(0.5);}
                	if(value=='A2'){leave4total=leave4total+(0.5);}
                	if(value=='M2'){leave5total=leave5total+(0.5);}
                	if(value=='U2'){leave6total=leave6total+(0.5);}
                	if(value.indexOf(':')>0){ 
