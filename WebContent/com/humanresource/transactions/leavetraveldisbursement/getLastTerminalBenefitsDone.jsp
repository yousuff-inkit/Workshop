...
<%	
	...
	try{
		...
		/*String sqlSalaryPaid = "select count(*) salaryPaid from hr_empm where active=1 and status=3 and dtjoin<='"+sqlDate+"' and salary_paid<'"+sqlDate+"' and doc_no="+empid+"";*/
		
		/* String sqlSalaryPaid = "select (MONTH('"+sqlDate+"')-MONTH(salary_paid)) salaryPaid from hr_empm where active=1 and status=3 and dtjoin<='"+sqlDate+"' and doc_no="+empid+"";
		ResultSet rsSalaryPaid = stmt.executeQuery(sqlSalaryPaid);
		
		while(rsSalaryPaid.next()) {
			salaryPaid=rsSalaryPaid.getString("salaryPaid").equalsIgnoreCase("1")?"0":rsSalaryPaid.getString("salaryPaid");
		} */ 
		
		/* String sqlSalaryPaid = "select TIMESTAMPDIFF(MONTH,salary_paid,'"+sqlDate+"') salaryPaid from hr_empm where active=1 and status=3 and dtjoin<='"+sqlDate+"' and doc_no="+empid+"";
		ResultSet rsSalaryPaid = stmt.executeQuery(sqlSalaryPaid);
		
		while(rsSalaryPaid.next()) {
			salaryPaid=rsSalaryPaid.getInt("salaryPaid")<=1?"0":rsSalaryPaid.getString("salaryPaid");
		} */ 
		
		//System.out.println(deprDate+"***"+dateDiff+"***"+deprDates+"***"+salaryPaid+"***"+terminaldate);
