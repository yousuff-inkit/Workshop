	   	                ...
	   	                var disper=rows[0].disper;
			          	    ...
			          	    for(var i=0;i<rows1.length;i++){
			  	          		   ...
			  	          		   if(parseInt(rows1[i].prodoc)==parseInt(prdid))
			  	           		   ...
			  	           		   if(parseFloat(rows1[i].disper)==parseFloat(disper))
			  	           			  {
...
/*     	if(document.getElementById("reftype").value=="PO")
          ...
          /*  $('#serviecGrid').on('celldoubleclick', function (event) {
           ...
           { 
           	...
           	var per=$('#serviecGrid').jqxGrid('getcellvalue', rowBoundIndex, "disper");	
           	var total=parseFloat(qty)*parseFloat(unitprice);
           	
           
           
           	
        	var totaldiscount=((parseFloat(total)*parseFloat(per))/100);
        	
         
          		
   		    $('#serviecGrid').jqxGrid('setcellvalue', rowBoundIndex, "discount",totaldiscount);
