...
%>
	...
	var aa="yes";
		getdatas(docnoss,refnosss,datess,aa,cmbreftype);
 

	}
	function getdatas(docnoss,refnosss,datess,aa,cmbreftype){
		var acno=document.getElementById("accdocno").value;
		 $("#refsearch").load('subrefnosearch.jsp?docnoss='+docnoss+'&datess='+datess+'&refnosss='+refnosss+'&aa='+aa+'&cmbreftype='+cmbreftype+'&acno='+acno);
	...
	function searchdata()
	{
       ...
       if(rows[i].chk==true)
    	   ...
    	   if(mm==0)
		   {// itemtype itemdocno itemname costtr_no
		...
		// importsearchcontent('importoption.jsp');	
	       $.messager.confirm('Message', 'Do you want to Import?', function(r){
     	 		...
     	 		var puraccid=document.getElementById("puraccid").value;
		       		  ...
		       		  $("#sevdesc").load("serviecgrid.jsp?reqdoc="+ document.getElementById("reqmasterdocno").value+"&chk="+chk+"&from="+from+"&cmbreftype="+cmbreftype+"&acno="+acno+"&dates="+dates+"&cmbbilltype="+cmbbilltype+"&puraccid="+puraccid);	
