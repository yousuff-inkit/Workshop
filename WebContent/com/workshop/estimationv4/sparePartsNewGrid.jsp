...
$(document).ready(function () { 
	var param_docno='<%=docno%>';
	var param_contractdocno='<%=contractdocno%>';
	var spareurl='getSpareGridData.jsp?type=SPARE&docno='+param_docno+'&id='+spareid+'&contractdocno='+param_contractdocno;
	// alert(spareurl);
	...
	if(spareid=="1"){
		...
		sparepartsdata='<%=gatedao.getPackageSpareData(docno,id,contractdocno)%>';
	}
	var renderertotal=function (aggregates){
     	var value=aggregates['sum'];
     	if(value=="undefined" || typeof(value)=="undefined"){
     		value="0.00";
     		//$('#sparetotal,#sparediscount').val(0.00);
     	}
     	if(value!="" && value!=null && value!="undefined"){
	     	...
	     	//$('#netesttotal').val(parseFloat($('#netestsparetotal').val())+parseFloat($('#netestservicetotal').val()));
     ...
     var source =
     {
         ...
         datafields: [
				...
				{name : 'contractdetdocno',type:'number'},
				{name : 'pkgqty',type:'number'}
     ...
     var dataAdapter = new $.jqx.dataAdapter(source,
            ...
            $("#sparePartsNewGrid").jqxGrid(
            {
               ...
               handlekeyboardnavigation: function (event) {
                ...
                columns: [
							...
							{ text: 'Dist.Discount', datafield: 'distsparediscount', width: '6%',editable:false,align:'right',cellsalign:'right',cellsformat:'d2',aggregates: ['sum']},
							...
							{ text: 'Total', datafield: 'approvedvalue', width: '6%',editable:true,align:'right',cellsalign:'right',cellsformat:'d2',aggregates: ['sum']},
			              	...
			              	{ text: 'Contract Detail Docno', datafield: 'contractdetdocno', width: '4%',hidden:true },
							{ text: 'Pkg Qty', datafield: 'pkgqty', width: '4%',hidden:true }
            ...
            $("#sparePartsNewGrid").on("cellvaluechanged", function (event) {
            	...
            	if(datafield=="qty" || datafield=="sprate" || datafield=="spdiscount" || datafield=="spvatpercent"){
            		...
            		var pkgqty=parseFloat($('#sparePartsNewGrid').jqxGrid('getcellvalue',rowindex,'pkgqty'));
            		var pkgdocno=$('#packagedocno').val();
            		if(qty>pkgqty && pkgdocno!=''){
            			document.getElementById("errormsg").innerText="";
            			document.getElementById("errormsg").innerText="Maximum Allowed "+pkgqty+" Only";
            			$('#sparePartsNewGrid').jqxGrid('setcellvalue',rowindex,'qty',oldvalue);
            			return false;
            		}
            		else{
            			//document.getElementById("errormsg").innerText="";
