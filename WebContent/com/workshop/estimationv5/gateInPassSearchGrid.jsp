...
$(document).ready(function () { 
     ...
     var dataAdapter = new $.jqx.dataAdapter(source,
            ...
            $("#gateInPassSearchGrid").on("rowdoubleclick", function (event) {
                ...
                funChangeEntity();
            	funCheckAttach($('#gatedocno').val());    
            });
        });
        
        function funCheckAttach(gatedocno){
        	$.get('getGIPAttachData.jsp',{'gatedocno':gatedocno},function(data){
        		data=JSON.parse(data);
        		if(data.estimaterestrictconfig=="1" && data.errorstatus=="1"){
        			document.getElementById("errormsg").innerText="";
        			document.getElementById("errormsg").innerText=data.errortype+" attachment not found";
        			document.getElementById("attacherror").value=data.errorstatus;
        			document.getElementById("attacherror").setAttribute("data-errorname",data.errortype+" attachment not found");
        		}
        		else{
        			document.getElementById("attacherror").value="0";
        		}
        	});
        }
    </script>
    <div id="gateInPassSearchGrid"></div>
    <input type="hidden" id="attacherror" name="attacherror" value="0">