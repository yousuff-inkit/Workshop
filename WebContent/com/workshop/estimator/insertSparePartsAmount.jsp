...
try{
	...
	String strchecktax="select (select method from gl_config where field_nme='tax') taxmethod";
	...
	String strdelete="delete from ws_estimationspareamt where rdocno="+docno;  
	int updateval=stmt.executeUpdate(strdelete);  
	
	vatnettotal=nettotal+(vatvalue*nettotal);
	ArrayList<String> amtarray=new ArrayList();
	if(chkrandomlumsum.equalsIgnoreCase("0")){
		amtarray.add(docno+"::"+"Total Parts Cost::"+null+"::"+partstotal);
		amtarray.add(docno+"::"+"Total Services Cost::"+null+"::"+servicetotal);	
	}
	else{
		amtarray.add(docno+"::"+"Total Parts Cost::"+null+"::"+0.0);
		amtarray.add(docno+"::"+"Total Services Cost::"+null+"::"+0.0);	
	}
	
	amtarray.add(docno+"::"+"Sub Total::"+null+"::"+nettotal);
	amtarray.add(docno+"::"+"VAT::"+vatpercent+"::"+(vatvalue*nettotal));
	amtarray.add(docno+"::"+"Net Total with VAT::"+null+"::"+vatnettotal);      

	for(int i=0;i<amtarray.size();i++){
		//System.out.println("///////////////");
		String temp[]=amtarray.get(i).split("::");
		String stramtinsert="insert into ws_estimationspareamt(rdocno,description,taxpercent,approved,status)values("+temp[0]+",'"+temp[1]+"',"+temp[2]+","+temp[3]+",3)";
