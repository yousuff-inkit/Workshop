...
try{
	...
	String strsql="select coalesce(insur.refname,'') gipinsurcomp,coalesce(gate.claim,'') gipclaimno,date_format(dat.edate,'%d.%m.%Y %H:%i') gipdatetime,convert(concat(coalesce(brd.brand_name,''),' ',coalesce(model.vtype,''),' Reg No: ',coalesce(gate.regno,''),' Plate Code: ',"+
			...
			" gate.yom=yom.doc_no left join my_acbook insur on (gate.insurcldocno=insur.cldocno and insur.dtype='CRM') inner join datalog dat on (dat.brhid=gate.brhid and dat.doc_no=gate.doc_no and dat.dtype='GIP' and dat.entry='A') where gate.status=3 and gate.processstatus=1 and approvalreq=1 "+sqltest;
