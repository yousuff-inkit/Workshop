...
function funReadOnly() {	
	$('#frmPackageMaster input').attr('readonly',true);
	$('#frmPackageMaster select').attr('disabled',true);
}
function funRemoveReadOnly() {
	$('#frmPackageMaster input').attr('readonly',false);
	$('#frmPackageMaster select').attr('disabled',false);
	if($('#mode').val()=='A'){
...
function funNotify(){
	...
	for(var i=0;i<labourrows.length;i++){
		if(labourrows[i].jobid!="" && labourrows[i].jobid!=null && labourrows[i].jobid!="undefined" && typeof(labourrows[i].jobid)!="undefined"){
			...
			newTextBox.val(labourrows[i].jobid+" :: "+labourrows[i].hrs+" :: "+labourrows[j].remarks+" :: "+labourrows[j].jobtype+" :: "+labourrows[j].jobdesc+" :: "+labourrows[j].seqno);
...
function funInitData(){
	$.get('getInitData.jsp',function(data){
		...
		$.each(data.branddata,function(index,value){
...
<div>
<form id="frmPackageMaster" action="savePackageMaster" autocomplete="off">
