	...
	if(docno.equalsIgnoreCase("")){
		if(chklumsum.equalsIgnoreCase("1") && Double.parseDouble(lumsumamount)>0){
			...
			for(int i=0;i<amtarray.size();i++){
				...
				String stramtinsert="insert into ws_estspareamt(gatedocno,description,taxpercent,genuine,market,used,approved,status)values("+temp[0]+",'"+temp[1]+"',"+temp[2]+","+temp[3]+","+temp[4]+","+temp[5]+","+temp[6]+",3)";
			...
			for(int i=0;i<amtarray.size();i++){
				//System.out.println("??????????????????");
				String temp[]=amtarray.get(i).split("::");
				String stramtinsert="insert into ws_estspareamt(gatedocno,description,taxpercent,genuine,market,used,status)values("+temp[0]+",'"+temp[1]+"',"+temp[2]+","+temp[3]+","+temp[4]+","+temp[5]+",3)";
