<%@page import="com.workshop.estimationadditionfancy.ClsEstimationAdditionFancyDAO"%>
<%@page import="com.workshop.estimationaddition.*" %>
<%
ClsEstimationAdditionFancyDAO gatedao=new ClsEstimationAdditionFancyDAO();
...
var id='<%=id%>';
if(id=="1"){
	gatedata='<%=gatedao.getGateInPassData(gatedocno,cldocno,clientname,jobdocno,date,id)%>';
}
else{
	gatedata=[];
}
$(document).ready(function () { 
     ...
     var dataAdapter = new $.jqx.dataAdapter(source,
            ...
            $("#gateInPassSearchGrid").on("rowdoubleclick", function (event) {
                ...
                $('#sparemarkup').val($('#gateInPassSearchGrid').jqxGrid('getcellvalue',row1,'sparemarkup'));
                $('#labdiscount').val($('#gateInPassSearchGrid').jqxGrid('getcellvalue',row1,'labdiscount'));
