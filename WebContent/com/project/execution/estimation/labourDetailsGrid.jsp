...
var loadid=parseInt('<%=loadid%>');
	...
	if(loadid==1){
		...
		labdata='<%=DAO.labourGridReLoad(session,trno,aid)%>'; 
		}
        $(document).ready(function () { 
            ...
            $("#labourDetailsGridID").on("bindingcomplete", function (event) {
            	
            	 var rowlength=$("#labourDetailsGridID").jqxGrid('rows').records.length;
            ...
            $("#labourDetailsGridID").jqxGrid(
            {
                ...
                columns: [
						  { text: 'Sr. No.', sortable: false, filterable: false, editable: false,
							...
							{ text: 'Activity', datafield: 'activity', width: '15%',editable:false },
							{ text: 'Code No', datafield: 'codeno', width: '10%',editable:false },	
							...
							{ text: 'Mins', datafield: 'min', width: '5%' },	
							{ text: 'Rate/Hr', datafield: 'rate', width: '10%', cellsformat: 'd2', cellsalign: 'right',editable:false,aggregates: ['sum1'],aggregatesrenderer:rendererstring1 },
 ...
 */    		    
            ...
            // create context menu
               ...
               $("#labourDetailsGridID").on('contextmenu', function () {
                   return false;
               });
               
               $("#Menu1").on('itemclick', function (event) {
            	   var args = event.args;
                   var rowindex = $("#labourDetailsGridID").jqxGrid('getselectedrowindex');
                   if ($.trim($(args).text()) == "Edit Selected Row") {
                       editrow = rowindex;
                       var offset = $("#labourDetailsGridID").offset();
                       $("#popupWindow1").jqxWindow({ position: { x: parseInt(offset.left) + 60, y: parseInt(offset.top) + 60} });
                       // get the clicked row's data and initialize the input fields.
                       var dataRecord = $("#labourDetailsGridID").jqxGrid('getrowdata', editrow);
                       // show the popup window.
                       $("#popupWindow1").jqxWindow('show');
                   }
                   else {
                       var rowid = $("#labourDetailsGridID").jqxGrid('getrowid', rowindex);
                       $("#labourDetailsGridID").jqxGrid('deleterow', rowid);
                   }
               });
               
               $("#labourDetailsGridID").on('rowclick', function (event) {
                   if (event.args.rightclick) {
        		   if(document.getElementById("mode").value=="A" || document.getElementById("mode").value=="E"){
                       $("#labourDetailsGridID").jqxGrid('selectrow', event.args.rowindex);
