...
ClsCommon ClsCommon=new ClsCommon();
	...
	try{
		...
		if(multi==0){
			...
			strSql = "select c.doc_no,c.code,round(cb.rate,2) c_rate,cb.type from my_brch b inner join my_curr c on(c.doc_no=b.curId) inner join my_curbook cb on (b.curid=cb.curid) "
			...
			/* strSql = "select doc_no,code,c_rate,type from my_curr where status <> 7"; */
			   ...
			   strSql = "select a.curid doc_no,c.code,round(a.rate,2) c_rate,a.type from my_curbook a inner join (select max(cb.doc_no) doc_no,cb.curid curid,cb.toDate,cb.frmDate "
