...
<%ClsReplacementDAO repdao=new ClsReplacementDAO(); %>
 <script type="text/javascript">
       var driverdata='<%=repdao.getDriverData(session)%>';	
       $(document).ready(function () { 	
    	   var id='<%=request.getParameter("id")%>';
            ...
            $('#driverSearch').on('rowdoubleclick', function (event) {
            	...
            	if(id=="1"){
            		document.getElementById("hidcollectdriver").value=$("#driverSearch").jqxGrid('getcellvalue', rowindex1, "dr_id");
            		document.getElementById("collectdriver").value=$("#driverSearch").jqxGrid('getcellvalue', rowindex1, "name");
            	}
            	if(id=="2"){
