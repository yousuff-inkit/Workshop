...
.hidden-scrollbar {
	...
	$("#searchdate").jqxDateTimeInput({ width: '125px', height: '15px',formatString:"dd.MM.yyyy",value:null});
 	...
 	function loadSearch() {
 		...
 		if(document.getElementById("docnosearch").value!=""){
 ...
 if(docnosearch1==null && fleetsearch==null && regnosearch==null && clientsearch==null && agmttype==null && mobilesearch==null && searchdate==null){
		...
		getdata(docnosearch1,fleetsearch,regnosearch,clientsearch,searchdate,mobilesearch,agmttype,agmtbranch);
 }
	}
 	
	function getdata(docnosearch,fleetsearch,regnosearch,clientsearch,searchdate,mobilesearch,agmttype,agmtbranch){
		
				$("#loadAgmtSearch").load("gridAgmtSearch.jsp?agmttype="+agmttype+"&docno="+docnosearch+"&fleet="+fleetsearch+"&regno="+regnosearch+"&client="+clientsearch+"&date="+searchdate+"&mobile="+mobilesearch+"&date="+searchdate+'&agmtbranch='+agmtbranch);
