...
function getGroup() {
	...
	function getColor() {
 	...
 	//	var reftype=document.getElementById("cmbsearchrtype").value;
		...
		getdata(searchdate,fleetno,docno,regno,color,group,branchid,id);
 

	}

	 function getdata(searchdate,fleetno,docno,regno,color,group,branchid,id){
		
		// $("#tariffDivId").load('rateDescription.jsp?txtrentaldocno='+indexVal1+'&revehGroup='+revehGroup);
		//alert(id);
		 $("#srefreshdiv").load('outFleetSearch.jsp?searchdate='+searchdate+'&fleetno='+fleetno+'&docno='+docno+'&regno='+regno+'&color='+color+'&group='+group+'&branch='+branchid+'&id1='+id);
