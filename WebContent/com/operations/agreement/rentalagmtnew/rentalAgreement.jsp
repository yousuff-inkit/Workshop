											  ...
											  if(out > del){
											   ...
											   $('#rasales_Agent').dblclick(function(){
										        ...
										        $('#ratariffbutton').click(function(){
										        	  if(($('#txtfleetno').val()!="") && ($('#txtcusid').val()!=""))
													      ...
													      if(document.getElementById("weekend").checked==true) 
													    	  ...
													    	  {
													    	  var tarifnew=$('#cmbtarifnew').val();
													    	  tariffbtnSearchContent('gettariffbtn.jsp?vehgpid='+document.getElementById("veh_fleetgrouptariff").value+"&cldocno="+document.getElementById("txtcusid").value+"&tarifnew="+tarifnew);
										        ...
										        $('#searchuser').click(function(){
										 		  ...
										 		  searchuserContent('searchotheruser.jsp?vehgpid='+document.getElementById("veh_fleetgrouptariff").value+"&tarifdoc="+document.getElementById("ratariffdocno1").value+"&fleetno="+document.getElementById("txtfleetno").value); 
				   ...
				   return false;
                            	...
                            	var x = new XMLHttpRequest();
                         		x.onreadystatechange = function() {
                         			if (x.readyState == 4 && x.status == 200) {
                         				...
                         				var TarifIdItems = items[1].split(",");
                         				var optionstarif = '<option value="">--Select--</option>';
                         				for (var i = 0; i < TarifItems.length; i++) {
                         					optionstarif += '<option value="' + TarifIdItems[i] + '">'
                         							+ TarifItems[i] + '</option>';
                         				}
                         				$("select#cmbtarifnew").html(optionstarif);
                         			 	 if ($('#hidcmbtarifnew').val() != null) {
                         					$('#cmbtarifnew').val($('#hidcmbtarifnew').val());
                         				}
                         			} else {
                         			}
                         		}
                         		x.open("GET", "getTarifNew.jsp", true);
                         		x.send();
                             }
                             ...
                             function getPriorClose(){
                            	...
                            	x.onreadystatechange = function() {
                            		...
                            		x.open("GET", "../getPriorClose.jsp?status=create", true);
                            		x.send();
                             }
				  ...
				  // tariff grid
						 ...
						 var rows = $("#jqxgridtarif").jqxGrid('getrows');
						   ...
						   //alert($('#gridlength').val());
							...
							var rowlgt= rows.length-1; 
							 ...
							 if(i==rowlgt)
								...
								var tarifnew=document.getElementById("cmbtarifnew").value;
								if(tarifnew.trim()!="Slab" && tarifnew.trim()!="Package"){
						   ...
						   newTextBox.val(rows[i].rentaltype+"::"+rows[i].rate+" :: "+rows[i].cdw+" :: "
								   ...
								   +rows[i].startday+" :: "+rows[i].starttime+" :: "+rows[i].endday+" :: "+rows[i].endtime+" :: "+rows[i].slabfromday+" :: "+rows[i].slabtoday+" :: "+rows[i].slabrateperday+" :: "+rows[i].packageblockday+" :: "+rows[i].packageblocktarif+" :: "+rows[i].packageextradaytarif);
                           //alert(rows[i].slabfromday+" :: "+rows[i].slabtoday+" :: "+rows[i].slabrateperday+" :: "+rows[i].packageblockday+" :: "+rows[i].packageblocktarif+" :: "+rows[i].packageextradaytarif);
						   newTextBox.appendTo('form'); 
   ...
   */
      ...
      /*     function funsalagentreqs()
        ...
        //	alert(document.getElementById("delchkvalue").value);
	      ...
	      function setValues() {
	        	 var tarifstat=document.getElementById("cmbtarifnew").disabled;
	        	 if(tarifstat==true){
	        	 	document.getElementById("cmbtarifnew").disabled=false;
	        	 	getTarifNew();
	        	 	document.getElementById("cmbtarifnew").value=document.getElementById("hidcmbtarifnew").value;
	        	 }
	        	 else{
	        		 getTarifNew();
	        		 document.getElementById("cmbtarifnew").value=document.getElementById("hidcmbtarifnew").value;	 
	        	 ...
	        	 var tarifnew=document.getElementById("cmbtarifnew").value;
	        	 document.getElementById("cmbtarifnew").disabled=true;
              ...
              $("#tariffDivId").load('rateDescription.jsp?txtrentaldocno='+indexVal1+'&revehGroup='+revehGroup+'&tarifnew='+document.getElementById("hidcmbtarifnew").value);
	...
	/*       function funchknumber()
          {
	       ...
	       function funPrintBtn(){
	           ...
	           var win_voucher= window.open(reurl[0]+"printVoucherWindow.jsp?docno="+document.getElementById("masterdoc_no").value+"&formdetailcode=RAG","_blank","top=250,left=310,Width=700,Height=400,location=no,scrollbars=no,toolbar=yes");
...
<form id="frmRentalAgreement" action="saveRentalAgmtNew" name="rentalform" method="post"  autocomplete="off">
...
<select name="cmbtarifnew" id="cmbtarifnew" onchange="setTarifNew(this.value);"><option value="">--Select--</option></select>&nbsp;
<button type="button"  title="Search Tariff"  class="icon" id="ratariffbutton"  value='<s:property value="ratariffbutton"/>'>					 
    ...
    <input type="hidden" name="systemval" id="systemval" value='<s:property value="systemval"/>'  /> 
     <input type="hidden" name="invoval" id="invoval" value='<s:property value="invoval"/>'  />
      ...
      <input type="hidden" name="supercdwinsu" id="supercdwinsu" value='<s:property value="supercdwinsu"/>'  />   <!-- set ex.insu by ratedescription grid cdw super cdw click else normal value set in quary -->
       ...
       <input type="hidden" name="hidcmbtarifnew" id="hidcmbtarifnew" value='<s:property value="hidcmbtarifnew"/>' />
       <input type="checkbox" id="weekend"  hidden="true" name="weekend" value="0" onclick="$(this).attr('value', this.checked ? 1 : 0)" style="display:none;visibility:hidden;">
