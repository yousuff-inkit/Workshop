 ...
 <%
 ClsLeaseCloseDAO viewDAO= new ClsLeaseCloseDAO();
...
if(tempss>0)
{
accountsdata='<%=viewDAO.accountstatementSearch(docnovalss)%>';
                   ...
                   if ($.trim($(args).text()) == "Edit Selected Row") {
                       ...
                       if(doctype=="INV" || doctype=="INS" || doctype=="INT"){
                         ...
                         var reurl=url.split("operations");
  						var win= window.open(reurl[0]+"finance/transactions/bankpayment/printBankPayment?docno="+document.getElementById("printinvno").value+"&branch="+document.getElementById("brchName").value+"&header=1","_blank","top=150,left=250,Width=1020,Height=500,location=no,scrollbars=no,toolbar=yes");
                        win.focus(); 
                       
                       }
                       else if(doctype=="BRV"){
                      	 var reurl=url.split("operations");
                      	 var win= window.open(reurl[0]+"finance/transactions/bankreceipt/printBankReciept?docno="+document.getElementById("printinvno").value+"&branch="+document.getElementById("brchName").value+"&header=1","_blank","top=150,left=250,Width=1020,Height=500,location=no,scrollbars=no,toolbar=yes");
                           win.focus();
                           }
                       else if(doctype=="CNO"){
    	                 	  var reurl=url.split("operations");
                      	  var win= window.open(reurl[0]+"finance/transactions/creditnote/printCreditNote?docno="+document.getElementById("printinvno").value+"&branch="+document.getElementById("brchName").value+"&header=1","_blank","top=150,left=250,Width=1020,Height=500,location=no,scrollbars=no,toolbar=yes");
                            win.focus(); 
                       }
                       else if(doctype=="CRV"){
                       	 var reurl=url.split("operations");
                      	 var win= window.open(reurl[0]+"finance/transactions/cashreciept/printCashReceipt?docno="+document.getElementById("printinvno").value+"&branch="+document.getElementById("brchName").value+"&header=1","_blank","top=150,left=250,Width=1020,Height=500,location=no,scrollbars=no,toolbar=yes");
                           win.focus(); 
                       }
                   }
               });
