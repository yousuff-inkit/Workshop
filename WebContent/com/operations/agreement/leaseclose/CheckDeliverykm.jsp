...
stmt.close();
	...
	//System.out.println("KM2: "+km2+"      KM1: "+km1);
	finalkm=(km2-km1)+tkm;
	...
	if(finaltime<0){
		...
		//Overriding totalkm-on 02/05/2017
	String sqldelcal="";
	if(delcal==0){
	...
	String strtotalkm="select (select sum(kmin-kmout) from gl_vmove where rdocno="+agmtno+" and rdtype='LAG'"+sqldelcal+" and km_invno=0 and status='IN') movtotalkm,"+
	" (select sum(fin-fout) from gl_vmove where rdocno="+agmtno+" and rdtype='LAG'"+sqldelcal+" and fuel_invno=0  and status='IN') movtotalfuel,"+
	" (select kmout from gl_vmove where rdocno="+agmtno+" and rdtype='LAG'  and status='OUT') movoutkm,"+
	" (select fout from gl_vmove where rdocno="+agmtno+" and rdtype='LAG'  and status='OUT') movoutfuel";
	double movtotalkm=0.0;
	double movoutkm=0.0;
	double currentinkm=0.0;
	double movtotalfuel=0.0,movoutfuel=0.0,currentinfuel=0.0;
	ResultSet rsmovkm=stmt.executeQuery(strtotalkm);
	while(rsmovkm.next()){
		movtotalkm=rsmovkm.getDouble("movtotalkm");
		movoutkm=rsmovkm.getDouble("movoutkm");
		currentinkm=km2;
		movtotalfuel=rsmovkm.getDouble("movtotalfuel");
		movoutfuel=rsmovkm.getDouble("movoutfuel");
		currentinfuel=fuel2;
	}
	System.out.println("Mov Total Km: "+movtotalkm+"---Current In Km: "+currentinkm+"----Mov Out Km: "+movoutkm);
	finalkm=movtotalkm+(currentinkm-movoutkm);
	finalfuel=movtotalfuel+(currentinfuel-movoutfuel);
	finalfuel=finalfuel*-1;
	if(finalfuel<0){
		finalfuel=0;
	}
	if(finalkm<0){
...
//Date and time validation ends
	...
	response.getWriter().write(datediff2+"***"+finaltime1+"***"+finalkm+"***"+finalfuel+"***"+outkm+"***"+kmstatus+"***"+datestatus+"***"+timestatus+"***"+date1+"***"+invstatus+"***"+invtmpdate+"***"+collectkmstatus+"***"+outdatestatus+"***"+outtimestatus+"***"+delcheckstatus+"***"+invtostatus+"***"+date2+"***"+closecalflag+"***"+closeinvdate+"***"+firstinvstatus+"***"+latest+"***"+fuel2);
