...
$(document).ready(function () {
 ...
 function getPriorClose(){
	...
	x.onreadystatechange = function() {
		...
		x.open("GET", "../getPriorClose.jsp?status=close", true);
		x.send();
 }
...
/* 		getCheckin();
	...
	/* 	if(typeof($("#totalgrid").jqxGrid("getcellvalue",0,"rate"))=="undefined" || $("#totalgrid").jqxGrid("getcellvalue",0,"rate")==""){
    	...
    	$('#gridlength').val(rows.length);
			...
			//	var myvar = rows[i].tarif; 
			    ...
			    .attr("name", "test"+i)
			    .attr("hidden", "true");
				//alert(rows[i].rentaltype+"::"+rows[i].rate+"::"+rows[i].cdw+"::"+rows[i].pai+"::"+rows[i].cdw1+"::"+rows[i].pai1+"::"+rows[i].gps+"::"+rows[i].babyseater+"::"+rows[i].cooler+"::"+rows[i].exkmrte+"::"+rows[i].exhrchg+"::"+rows[i].chaufchg);
	...
	function funProcess(){
		...
		$("#totaldiv").load("totalGrid.jsp?useddays="+useddays+"&tarifnew="+document.getElementById("tarifnew").value+"&usedhours="+usedhours+"&totalkm="+totalkm+"&excesskm="+excesskm+"&agmtno="+agmtno+"&rentaltype="+rentaltype.replace(/\s/g, "%20")+"&agmtdate="+agmtdate+"&closedate="+closedate+"&temp=1&closecalflag="+$("#closecalflag").val()+"&closeinvdate="+closeinvdate+"&idnew=1");
	/* } */
	}
		...
		if(document.getElementById("mode").value!="A"){
		 ...
		 if($('#hidintime').val()){
			...
			if((document.getElementById("agreementno").value!='')&&(document.getElementById("docno").value!='')){
				...
				$("#agmttarifdiv").load("agreementTarifGrid.jsp?agmt="+document.getElementById("agreementno").value+"&tarifnew="+document.getElementById("tarifnew").value);
				...
				$('#referencetarifdiv').load("referenceTarifGrid.jsp?id="+document.getElementById("agreementno").value+"&tarifnew="+document.getElementById("tarifnew").value);
			}
...
<form id="frmRentalClose" action="saveRentalCloseNew" autocomplete="off">
	<%-- <script>
...
<input type="text" name="tarifnew" id="tarifnew" value='<s:property value="tarifnew"/>' >
</div>
</form>
<div id="agmtnowindow">
   <div ></div>
