...
if(document.getElementById("rentalagent").value==""){
	...
	function funCalculate(){
		...
		var useddays=document.getElementById("useddays").value;
		var rentaltype=$('#totalgrid').jqxGrid('getcellvalue', 0, "rentaltype");
		//alert("===="+agmtno+":"+tarif+":"+cdwtotal+":"+":"+acc+":"+chauffer+":"+excesskmchg+":"+excesshrchg+"===");
	$('#calcdiv').load("calculationGrid.jsp?agmtno="+agmtno+"&tarif="+tarif+"&cdwtotal="+cdwtotal+"&acctotal="+acc+"&chauffer="+chauffer+"&excesskmchg="+excesskmchg+"&excesshrchg="+excesshrchg+"&temp=1&usedhours="+document.getElementById("usedhours").value+"&clientid="+clientid+"&fuel="+fuel+"&deliverychg="+document.getElementById("deliverychg").value+"&collectchg="+document.getElementById("collectchg").value+"&outdate="+agmtdate+"&indate="+closedate+"&cmbinfuel="+document.getElementById("fuel2").value+"&exkm="+document.getElementById("excesskm").value+"&closeinvdate="+closeinvdate+"&closecalflag="+$('#closecalflag').val()+"&calctype="+calctype+"&useddays="+useddays+"&rentaltype="+rentaltype);
	}
	...
	function funSearchLoad(){
		changeContent('masterSearch.jsp', $('#window'));
			...
			function funPrintBtn() {
		    	...
		    	var url=document.URL;
		    	  var reurl=url.split("saveRentalCloseSayara");
		    	    	var win= window.open(reurl[0]+"printRASayara?docno="+document.getElementById("agreementno").value+"&formdetailcode=RAC","_blank","top=250,left=310,Width=800,Height=800,location=no,scrollbars=no,toolbar=yes");
		    	win.focus();
		    	 }
...
<form id="frmRentalClose" action="saveRentalCloseSayara" autocomplete="off">
	<%-- <script>
