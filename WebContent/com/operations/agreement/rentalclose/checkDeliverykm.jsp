...
try{
	...
	*/
	...
	/** (delivery - collectoin) date or (out- in) date calculation STARTS 	*/
		...
		String sqlkmsum="select sum(coalesce(tkm,0)) tkm from gl_vmove where rdtype='RAG' and km_invno=0 and rdocno="+agmtno+""+sqltest;
		String sqlfuelsum="select sum(coalesce(tfuel,0)) tfuel from gl_vmove where rdtype='RAG' and km_invno=0 and rdocno="+agmtno+""+sqltest;
		//System.out.println("sqlsum:"+sqlsum);
		ResultSet rskmsum=stmt.executeQuery(sqlkmsum);
		while(rskmsum.next()){
			tkm=rskmsum.getDouble("tkm");
		}
		ResultSet rsfuelsum=stmt.executeQuery(sqlfuelsum);
		while(rsfuelsum.next()){
			tfuel=rsfuelsum.getDouble("tfuel");
		}
		...
		String strtotalkm="select (select sum(kmin-kmout) from gl_vmove where rdocno="+agmtno+" and rdtype='RAG'"+sqldelcal+" and km_invno=0 and status='IN') movtotalkm,"+
		" (select sum(fin-fout) from gl_vmove where rdocno="+agmtno+" and rdtype='RAG'"+sqldelcal+" and fuel_invno=0  and status='IN') movtotalfuel,"+
		" (select kmout from gl_vmove where rdocno="+agmtno+" and rdtype='RAG'  and status='OUT') movoutkm,"+
		" (select fout from gl_vmove where rdocno="+agmtno+" and rdtype='RAG'  and status='OUT') movoutfuel";
