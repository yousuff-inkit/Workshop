...
try{
	...
	*/
	//For Alfahim Starts here
	
	int pytschedulestatus=-1;
	
	//For Alfahim Ends here
int delstatus=0;
String finaltime1="";
...
stmt.close();
	...
	/** (delivery - collection) date or (out- in) date calculation  ENDS */
		...
		//			System.out.println("Date1 valid:"+date1valid+" Date2valid :"+date2valid+"   Km1valid: "+km1valid+"::::Km2valid: "+km2valid+"   TIME1valid: "+time1valid+"::::time2valid: "+time2valid);
			...
			Statement stmtvalid=conn.createStatement();
				String strvalid="select dout,tout,kmout,fout from gl_vmove where rdocno="+agmtno+" and rdtype='LAG' and status='OUT'";
	...
	String strtotalkm="select (select sum(kmin-kmout) from gl_vmove where rdocno="+agmtno+" and rdtype='LAG'"+sqldelcal+") movtotalkm,"+
	" (select kmout from gl_vmove where rdocno="+agmtno+" and rdtype='LAG'  and status='OUT') movoutkm";
	...
	// System.out.println(movtotalkm+"::"+currentinkm+"::"+movoutkm);
...
//Date and Time Validation
	 ...
	 //System.out.println("Date check1:"+date2.compareTo(invtmpdate));
	  // System.out.println("Date check2:"+date2valid.compareTo(date1valid));
	 if(date2valid.compareTo(date1valid)==0){
...
//Date and time validation ends
	...
	// System.out.println("InvTempDate:"+invtmpdate.compareTo(date2));
	 ...
	 int closecalflag=0;
		...
		// System.out.println("InvTempDate:"+invtmpdate.compareTo(date1));
		...
		String strcontractenddate="select per_value,per_name,if(per_name=1,date_add('"+date1+"',interval per_value*12 month),date_add('"+date1+"',interval per_value month))"+
		" contractenddate from gl_lagmt where doc_no="+agmtno;
		ResultSet rscontractend=stmt.executeQuery(strcontractenddate);
		java.sql.Date contractenddate=null;
		while(rscontractend.next()){
			contractenddate=rscontractend.getDate("contractenddate");
		}
		/* String strlastpytdate="select date from gl_leasepytcalc where srno=(select max(srno) from gl_leasepytcalc where rdocno="+agmtno+" and markstatus=0)";
		ResultSet rslastpytdate=stmt.executeQuery(strlastpytdate);
		java.sql.Date lastpytdate=null;
		while(rslastpytdate.next()){
			lastpytdate=rslastpytdate.getDate("date");
		}
		// System.out.println("InvTempDate: 111  "+lastpytdate);
		if(date2.compareTo(contractenddate)<0){
			if(lastpytdate!=null){
			if(date2.compareTo(lastpytdate)>0){
				pytschedulestatus=0;
				/*  String strcheckpytschedule="select markstatus from gl_leasepytcalc where rdocno="+agmtno+" and month(date)=month('"+date2+"')-1";
				System.out.println(strcheckpytschedule);
				ResultSet rscheckpytshedule=stmt.executeQuery(strcheckpytschedule);				
				while(rscheckpytshedule.next()){
					pytschedulestatus=rscheckpytshedule.getInt("markstatus");
				} 
				if(date2.compareTo(lastpytdate)>0){
					pytschedulestatus=0;
				}
		...
		String strlastpytdate="select date from gl_leasepytcalc where date<='"+date2+"' and rdocno="+agmtno+" and markstatus=0 and status=3";
		ResultSet rslastpytdate=stmt.executeQuery(strlastpytdate);
		java.sql.Date lastpytdate=null;
		while(rslastpytdate.next()){
			lastpytdate=rslastpytdate.getDate("date");
	...
	response.getWriter().write(datediff2+"***"+finaltime1+"***"+finalkm+"***"+finalfuel+"***"+outkm+"***"+kmstatus+"***"+datestatus+"***"+timestatus+"***"+date1+"***"+invstatus+"***"+invtmpdate+"***"+collectkmstatus+"***"+outdatestatus+"***"+outtimestatus+"***"+delcheckstatus+"***"+invtostatus+"***"+date2+"***"+closecalflag+"***"+closeinvdate+"***"+firstinvstatus+"***"+latest+"***"+fuel2+"***"+pytschedulestatus);
