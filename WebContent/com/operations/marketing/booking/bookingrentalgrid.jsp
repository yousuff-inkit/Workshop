...
<%@page import="com.operations.agreement.rentalagreement.ClsRentalAgreementDAO" %>
 	...
 	//slno
 		String indexrdocno=request.getParameter("indexrdocno")==null?"0":request.getParameter("indexrdocno").trim();
  		...
  		String groupid=request.getParameter("groupid")==null?"0":request.getParameter("groupid").trim();
...
var datatariff;
  ...
  {
	 // groupid
	  datatariff='<%=viewDAO.tariffslrelode(session,indexrdocno,qotslno,groupid)%>';  
  }
	...
	{ 
      ...
      datatariff='<%=viewDAO1.tariffType()%>'; 
  ...
  if(temp2<=0)
    ...
    var cellsrenderer = function (row, column, value, defaultHtml) {
    			    ...
    			    if ( row == row1 || row == row2) {
    			        ...
    			        element.css({ 'background-color': '#7fffd4', 'width': '100%', 'height': '100%', 'margin': '0px' }); //#7fffd4
    			    ...
    			    if (row == row3) {
    			    	//alert(row3);
    			        var element = $(defaultHtml);
    			        element.css({ 'background-color': '#ffc0cb', 'width': '100%', 'height': '100%', 'margin': '0px' });
    			        return element[0].outerHTML;
    			    }
    			    return defaultHtml;
    			}
   ...
   $("#jqxgridtarif").jqxGrid(
   {
      ...
      theme: theme,
       ...
       columns: [  
					...
					{ text: ''+document.getElementById("mainchaufchg").value,        datafield: 'chaufchg',  editable:true,cellsformat: 'd2', cellsalign: 'right', align:'right',cellbeginedit: rowEdit,cellsrenderer: cellsrenderer,hidden:true},
					{ text: ''+document.getElementById("mainchaufexchg").value,      datafield: 'chaufexchg', editable:true,cellsformat: 'd2', cellsalign: 'right', align:'right',cellbeginedit: rowEdit,cellsrenderer: cellsrenderer,hidden:true},
           ...
           var rentypevalid=$('#jqxgridtarif').jqxGrid('getcellvalue', rentalrow, "rentaltype");
            ...
            if(columnindex1>0)
			   ...
			   if(rowindex1<rentalrow)
			   {
				   ...
				   document.getElementById("renttype").value=$('#jqxgridtarif').jqxGrid('getcellvalue', rentalrow, "rentaltype");
