...
$(document).ready(function () {
	if($('#docno').val()==''){
     ...
     $('#yomsearchwindow').jqxWindow({ width: '20%', height: '55%',  maxHeight: '62%' ,maxWidth: '60%' , title: 'Yom Search' ,position: { x: 500, y: 120 }, keyboardCloseKey: 27});
     $('#yomsearchwindow').jqxWindow('close');
     $('#kmsearchwindow').jqxWindow({ width: '20%', height: '55%',  maxHeight: '62%' ,maxWidth: '60%' , title: 'KM Search' ,position: { x: 500, y: 120 }, keyboardCloseKey: 27});
   ...
   $('#EnquiryDate').on('change', function (event) {
	...
	function funReadOnly(){
		...
		$('#frmLeasePriceRequest input').prop('readonly', true );
		
		$('#frmLeasePriceRequest textarea').attr('readonly', true );
		$('#frmLeasePriceRequest select').attr('disabled', true);
	...
	function funRemoveReadOnly(){
		...
		$('#frmLeasePriceRequest input').attr('readonly', false );
		
		$('#frmLeasePriceRequest textarea').attr('readonly', false );
		$('#frmLeasePriceRequest select').attr('disabled', false);
	...
	function getEnqsrc(event){
	 ...
	 function getclinfo(event){
    	 ...
    	 clientSearchContent('clientINgridsearch.jsp?', $('#clientsearch1'));    }
	       ...
	       function kmSearchContent(url) {
               $.get(url).done(function (data) {
	          		 $('#kmsearchwindow').jqxWindow('setContent', data);
    			}); 
         	}
	...
	function funNotify(){	
		...
		if(valid=="")
			...
			var reqrows=$('#jqxEnquiry').jqxGrid('getrows');
		var yomerror=0;
		for(var z=0;z<reqrows.length;z++){
			if(reqrows[z].brdid!="undefined" && reqrows[z].brdid!="" && reqrows[z].brdid!=null && reqrows[z].modid!="undefined" && reqrows[z].modid!="" && reqrows[z].modid!=null){
				if(reqrows[z].yomid=="undefined" || reqrows[z].yomid=="" || reqrows[z].yomid==null || typeof(reqrows[z].yomid)=="undefined"){
					yomerror=1;	
				}
			}
		}
		if(yomerror==1){
			document.getElementById("errormsg").innerText="";
			document.getElementById("errormsg").innerText="Please select YOM";
			return 0;
		}
		}
		...
		if(parseInt(toval)==1)
		 ...
		 var rows = $("#jqxEnquiry").jqxGrid('getrows');
		   ...
		   newTextBox.val(rows[i].sr_no+"::"+rows[i].brdid+" :: "+rows[i].modid+" :: "
				   +rows[i].yomid+" :: "+rows[i].clrid+" :: "+rows[i].ldur+" :: "+rows[i].hidfromdate+" :: "+rows[i].kmusage+" :: "+rows[i].qty+" :: "+rows[i].vsb);
	...
	function reqdata()
	{
 		 if((document.getElementById("r1").checked=="")&&(document.getElementById("r2").checked==""))
    		...
    		{ 
     	      ...
     	      $('#frmLeasePriceRequest input').attr('readonly', true );
     			$('#frmLeasePriceRequest textarea').attr('readonly', true );
    ...
    function funPrintBtn(){
        ...
        var reurl=url.split("saveLeasePriceRequest");
        ...
        if (x.readyState==4 && x.status==200) 	 
         {
	         ...
	         if(items=="1"){
		    	 ...
		    	 var path= path1+"?masterdoc_no="+masterdoc_no+"&checkval="+checkval+"&branchvals="+branchvals+"&currencyvals="+currencyvals;
		  		 top.addTab( "Lease Calculator / Quote",reurl[0]+""+path);
	         }
	         else{
	        	 $.messager.alert('warning','Lease Calculator Document Already Created');
	         }
	         
           }
        }
        x.open("GET","checkLeaseCalculator.jsp?masterdocno="+masterdoc_no,true);
      	x.send();
