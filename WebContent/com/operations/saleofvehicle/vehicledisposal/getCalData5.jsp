...
try{
	...
	double purcost=0.0,depr=0.0;
	String strfleet="select depr_date,depr from gl_vehmaster where fleet_no="+fleet;
	ResultSet rsfleet=stmtfleet.executeQuery(strfleet);
	if(rsfleet.next()){
		deprdate=rsfleet.getDate("depr_date");
		depr=rsfleet.getDouble("depr");
	...
	/* String strasset="select (select sum(dramount) from gc_assettran where fleet_no="+fleet+") sumamount,datediff('"+docdate+"','"+deprdate+"') daysdiff"; */
	 String strasset="select datediff('"+docdate+"','"+deprdate+"') datediff,round((("+purcost+" * "+depr+"/100)/365)*(select datediff) ,2) currentdepr";	
	...
	double netamt=salesprice-(netbook<0?netbook*-1:netbook);
