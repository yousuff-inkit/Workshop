...
<script type="text/javascript">
      $(document).ready(function () {
    	 ...
    	 $('#stateWindow').jqxWindow({width: '25%', height: '58%',  maxHeight: '70%' ,maxWidth: '25%' , title: 'State Search',position: { x: 420, y: 87 } , theme: 'energyblue', showCloseButton: true, keyboardCloseKey: 27});
 		 ...
 		 $('#btnLeasePriceRequest').hide();	
 		 getGroup();getSalesman();getCategory();getNationality();getSalutation();getContractDate();getContract();getIDPDetails();getLanguage();getNonTaxableEntity();getTax();
      }); 
      ...
      function getLanguage() {
    		...
    		x.onreadystatechange = function() {
    			if (x.readyState == 4 && x.status == 200) {
    				var items = x.responseText;
    				items = items.split('####');
    				var languageItems = items[0].split(",");
    				var languageIdItems = items[1].split(",");
    				var optionslanguage = '<option value="0">--Select--</option>';
    				for (var i = 0; i < languageItems.length; i++) {
    					optionslanguage += '<option value="' + languageIdItems[i] + '">'
    							+ languageItems[i] + '</option>';
    				}
    				$("select#cmblanguage").html(optionslanguage);
    				if ($('#hidcmblanguage').val() != null) {
    					$('#cmblanguage').val($('#hidcmblanguage').val());
    				}
    			} else {
    			}
    		}
    		x.open("GET", "getLanguage.jsp", true);
    		x.send();
    	}
      ...
      function getNonTaxableEntity(){
	  		...
	  		x.onreadystatechange = function() {
	  			if (x.readyState == 4 && x.status == 200) {
	  				var items = x.responseText.trim();
	  			    if(parseInt(items)==1) {
	  			    	document.getElementById("lblnontaxableentity").style.display = 'inline-block';
    ...
    function getPassportNoAlreadyExists(passportno,docno,mode){
	  ...
	  function getDefaultService(b) {
    		...
    		x.onreadystatechange = function() {
    			if (x.readyState == 4 && x.status == 200) {
    				...
    				var items = x.responseText;
      				items = items.split('####');
      				var salikRateItems = items[0];
      				var trafficRateItems  = items[1];
      			
      			    $('#txtsalik').val(salikRateItems);	
      			    $('#txttraffic').val(trafficRateItems);
    				
    			} else {
    			}
    		}
    		x.open("GET", "getDefaultServiceChargeCategory.jsp?categoryId="+b, true);
    		x.send();
    	}
     ...
     function defaultcheck(){
 		 if(document.getElementById("chckdefault").checked){
			 ...
			 $('#txttraffic').val("0.0");
 			 ...
 			 getDefaultService($('#cmbcategory').val());
	 ...
	 function funOpenLeasePriceRequestBtn() {
	 	 var checkval="open"; 
 		 var masterdoc_no=document.getElementById("docno").value; 
 		 var branchvals=document.getElementById("brchName").value; 
 		 var currencyvals=document.getElementById("currency").value;
         var url=document.URL;
	     var reurl=url.split("com");
	     var path1='com/operations/marketing/leasepricerequestalfahim/leasepricerequest.jsp';
  	     var path= path1+"?masterdoc_no="+masterdoc_no+"&checkval="+checkval+"&branchvals="+branchvals+"&currencyvals="+currencyvals;
		 top.addTab( "Lease Price Request",reurl[0]+""+path);
     }
	 ...
	 function funReadOnly(){
			...
			if ($("#mode").val() == "view" && parseInt($('#docno').val())>0) {
				$('#btnLeasePriceRequest').show();
			} else {
				$('#btnLeasePriceRequest').hide();
			}
	 }
	 ...
	 function funRemoveReadOnly(){
			...
			if ($("#mode").val() == "A") {
					 ...
					 $('#cmblanguage').prop('selectedIndex',1);
	 ...
	 function funNotify(){	
		 ...
		 var clientname=document.getElementById("txtclient_name").value;
		 if(clientname.trim()==''){
			 document.getElementById("errormsg").innerText="Client Name is Mandatory.";
			 return 0;
		 }
		 ...
		 var personaltel=document.getElementById("personal_tel2").value;
		 if(personaltel.trim()==''){
			 document.getElementById("errormsg").innerText="Communication Mobile Number is Mandatory.";
			 return 0;
		 }
		 
		 var personalemail=document.getElementById("txtpersonal_email").value;
		 if(personalemail.trim()==''){
			 document.getElementById("errormsg").innerText="Communication Email is Mandatory.";
			 return 0;
		 }
		 ...
		 if(!mobileValid($("#personal_tel2").val())){
			 return 0;
		 }
		
		 if(!mobileValid($("#office_tel2").val())){
			 return 0;
		 }
		 
		 if(!mobileValid($("#residence_tel2").val())){
			 return 0;
		 }
		 if(!mobileValid($("#home_tel2").val())){
			 return 0;
		 }
	 ...
	 function setValues(){
		    ...
		    /* Enquiry Form dtype */
		    document.getElementById("formdetail").value="New Customer";
...
<table width="100%">
  <tr>
    ...
    <td width="10%"><input type="text" id="txtcode" name="txtcode" style="width:60%;" tabindex="-1" value='<s:property value="txtcode"/>'/></td>
    <td width="6%" align="right">Name</td>
    <td width="29%"><input type="text" id="txtclient_name" name="txtclient_name" onfocus="getCurrencyIds();" style="width:97%;" value='<s:property value="txtclient_name"/>'/></td>
    <td width="9%"><select id="cmbsalutation" name="cmbsalutation" style="width:75%;" value='<s:property value="cmbsalutation"/>'>
    ...
    <td width="11%"><input type="text" id="docno" name="txtclientdocno" style="width:75%;" tabindex="-1" value='<s:property value="txtclientdocno"/>'/></td>
  </tr>
  <tr>
    <td align="right">Currency</td>
    <td><select id="cmbcurrency" name="cmbcurrency" value='<s:property value="cmbcurrency"/>'>
      <option value="">--Select--</option></select>
      <input type="hidden" id="hidcmbcurrency" name="hidcmbcurrency" value='<s:property value="hidcmbcurrency"/>'/></td>
    <td align="right">Category</td>
    <td><select id="cmbcategory" name="cmbcategory" style="width:90%;" onchange="getCategoryAccountGroup(this.value);getDefaultService(this.value);" value='<s:property value="cmbcategory"/>'>
      <option value="">--Select--</option></select>
    ...
    <td><select id="cmbsalesman" name="cmbsalesman" style="width:70%;" value='<s:property value="cmbsalesman"/>'>
      <option value="">--Select--</option></select>
      <input type="hidden" id="hidcmbsalesman" name="hidcmbsalesman" value='<s:property value="hidcmbsalesman"/>'/></td>
    <td align="center"><input type="checkbox" id="chckadvance" name="chckadvance" value="" onchange="advancecheck();" onclick="$(this).attr('value', this.checked ? 1 : 0)">Advance
                                 <input type="hidden" id="hidchckadvance" name="hidchckadvance" value='<s:property value="hidchckadvance"/>'/></td>
    <td align="right">Invoicing Method</td>
    <td><select id="cmbinvoicing_method" name="cmbinvoicing_method"  style="width:90%;" value='<s:property value="cmbinvoicing_method"/>'>
  ...
  <tr>
    ...
    <td><select id="cmbdel_charges" name="cmbdel_charges" style="width:90%;" value='<s:property value="cmbdel_charges"/>'>
      <option value="">--Select--</option>
    ...
    <td><select id="cmblanguage" name="cmblanguage" style="width:95%;" value='<s:property value="cmblanguage"/>'>
      <option value="">--Select--</option></select>
      <input type="hidden" id="hidcmblanguage" name="hidcmblanguage" value='<s:property value="hidcmblanguage"/>'/></td>
    <td align="right"><label id="lblnontaxableentity">Tax</label></td>
    <td><select id="cmbtax" name="cmbtax" style="width:40%;" onchange="taxcheck();" value='<s:property value="cmbtax"/>'>
      <option value="">--Select--</option></select>
    ...
    <td><input type="text" id="txtregisteredtrnno" name="txtregisteredtrnno" style="width:95%;" value='<s:property value="txtregisteredtrnno"/>'/></td>
    <td><input type="checkbox" id="chcknontaxableentity" name="chcknontaxableentity" style="display: none;" value="" onchange="nontaxableentitycheck();" onclick="$(this).attr('value', this.checked ? 1 : 0)">
...
<table width="100%">
  <tr>
    ...
    <td  colspan="3"><select id="cmbgroup1" name="cmbgroup1" style="width:83%;" value='<s:property value="cmbgroup1"/>'>
      <option value="">--Select--</option></select>
       <input type="hidden" id="hidcmbgroup1" name="hidcmbgroup1" value='<s:property value="hidcmbgroup1"/>'/></td>
   <td align="right">Account</td>
    <td width="26%"><input type="text" id="txtaccount" name="txtaccount" style="width:70%;" tabindex="-1" value='<s:property value="txtaccount"/>'/></td>
    <td width="8%" rowspan="2" align="center"><button type="button" class="icon" id="btnLeasePriceRequest" title="Lease Price Request" onclick="funOpenLeasePriceRequestBtn()">
									<img alt="Lease Price Request" src="<%=contextPath%>/icons/leasepricerequest.png">
								</button></td>
  </tr>
  <tr>
    <td align="right">Credit Period-Min(Days)</td>
    <td width="14%"><input type="text" id="txtcredit_period_min" name="txtcredit_period_min" style="width:80%;text-align: right;" value='<s:property value="txtcredit_period_min"/>'/></td>
    <td width="8%" align="right">Max(Days)</td>
    <td width="21%"><input type="text" id="txtcredit_period_max" name="txtcredit_period_max" style="width:60%;text-align: right;" value='<s:property value="txtcredit_period_max"/>'/></td>
    <td width="8%" align="right">Credit Limit</td>
    <td><input type="text" id="txtcredit_limit" name="txtcredit_limit" style="width:70%;text-align: right;" value='<s:property value="txtcredit_limit"/>'/></td>
  </tr>
</table>
</fieldset></td>
<td width="40%">
