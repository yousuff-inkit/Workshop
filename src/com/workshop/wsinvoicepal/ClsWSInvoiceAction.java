package com.workshop.wsinvoicepal;  
			...
			if (objcommon.getPrintPath(dtype).contains(".jrxml") == true) {
				...
				try {
                    ...
                    String strsql2="select imgpath from my_brch where doc_no='"+brhid+"'";          
		    	     ...
		    	     Double servicetot=0.0,partstot=0.0,nettotal=0.0,vatamt=0.0,vatamtpal=0.0;
		    	     String netamt="";
		    	     ...
		    	     String addition="0";    
		 			 String straddition="select coalesce(group_concat(distinct t.addition),0) addition from ws_invm m left join ws_invcalctemp t on m.doc_no=t.invno where m.doc_no="+doc;
		    	     ...
		    	     String detailsql="",team21detailsql="",liwadetailsql="";
		    	     if(proformaconfig==1){  
...
//		 		System.out.println(team21detailsql);
		 		...
		 		//System.out.println(strservie+"==="+strservie);
		 			...
		 			Double subletstot=0.00,partssublet=0.00;
		 			String strpartssublet="select round(sum(coalesce(lab.invoiceamt,0)),2) subletstot,est.doc_no estdocno from ws_jobcard card inner join ws_estm est on (card.reftype='EST' and card.refno=est.doc_no) inner join ws_estlabour lab on est.doc_no=lab.rdocno  where card.doc_no="+jobcarddocno+" and lab.addition in ("+addition+") and lab.chkcomplete=1 and strjobtype='SUBLET'";
			 		//System.out.println(strparts+"==="+strparts);  
		 			ResultSet rspartssublet=stmt.executeQuery(strpartssublet);
		 			while(rspartssublet.next()){
		 				subletstot=rspartssublet.getDouble("subletstot");  
		 			}
		 			partssublet=servicetot-subletstot;
		 			
		 		Double sublettot=0.00,nettotalpal=0.0;
		 			String strsqls1="select sum(round(lab.invoiceamt,2)) amount2 from ws_jobcard card left join ws_estm est on  (card.reftype='EST' and card.refno=est.doc_no) left join ws_estlabour lab on est.doc_no=lab.rdocno left join ws_jobmaster m  on lab.jobid=m.doc_no left join ws_jobtype t on m.jobid=t.doc_no where card.doc_no="+jobcarddocno+" and lab.addition in ("+addition+") and card.status=3 and lab.confirmed=1 and lab.approved=1 and lab.chkcomplete=1 and upper(lab.strjobtype)='SUBLET'";
		    	     ResultSet rss1=stmt.executeQuery(strsqls1);            
		    	     while(rss1.next()){         
		    	    	 sublettot=rss1.getDouble("amount2");    
		    	    	//System.out.println("sublettot=="+strsqls1);;
		    	     }
		    	     vatamtpal= objcommon.Round((partstot+servicetot+sublettot)*0.05,2);
		    	     nettotalpal=partstot+servicetot+sublettot+vatamtpal;
		 			/**/
					/* TEAM 21 PRINT*/ 
		 			...
		 			if(objcommon.getPrintPath2(dtype).contains("linediscount") ){
			 			...
			 			String strservice1li="select round(sum(coalesce(lab.invoiceamt,0)),2) servicetot from ws_jobcard card inner join ws_estm est on (card.reftype='EST' and card.refno=est.doc_no) inner join ws_estlabour lab on est.doc_no=lab.rdocno  where card.doc_no="+jobcarddocno+" and lab.addition in ("+addition+") and lab.chkcomplete=1 and upper(lab.strjobtype)!='SUBLET'";
			 			//System.out.println(strservice1li+"==="+strservice1li);
			 			...
			 			String strservice2li="select round(sum(coalesce(lab.invoiceamt,0)),2) servicetot from ws_jobcard card inner join ws_estm est on (card.reftype='EST' and card.refno=est.doc_no) inner join ws_estlabour lab on est.doc_no=lab.rdocno  where card.doc_no="+jobcarddocno+" and lab.addition in ("+addition+") and lab.chkcomplete=1 and upper(lab.strjobtype)='SUBLET'";
			 		   ...
			 		   // param.put("nettotalli",nettotalli);
			    	     ...
			    	     String strsqls4="select sum(amount) amount,sum(partsli) partsli from(select rdocno,if(upper(description)='CONSUMABLES',lab.approvedvalue+lab.spdiscount,0) amount,if(upper(description)!='CONSUMABLES',lab.approvedvalue+lab.spdiscount,0) partsli,upper(description) type from ws_estspare lab where lab.addition="+addition+" and lab.rdocno='"+estdocno+"' )a group by a.rdocno";
			    	    //System.out.println("strsql4="+strsql4); 
		 			...
		 			//Payment Status for PAL
		 			String strpyt="select round(sum(jv.dramount-jv.out_amount),2) diff,round(sum(jv.dramount),2) dramt from ws_invm inv left join my_jvtran jv on (inv.tr_no=jv.tr_no and inv.invoicetoacno=jv.acno) where inv.doc_no="+doc;
		 			ResultSet rspyt=stmt.executeQuery(strpyt);
		 			double balance=0.0,dramt=0.0;
		 			String paymentstatus="";
		 			while(rspyt.next()){
		 				balance=rspyt.getDouble("diff");
		 				dramt=rspyt.getDouble("dramt");
		 			}
		 			if(balance==0.0){
		 				paymentstatus="Fully Paid";
		 			}
		 			else if(balance>0.0 && balance!=dramt){
		 				paymentstatus="Partially Paid, Balance: "+balance;
		 			}
		 			else if(balance>0.0 && balance==dramt){
		 				paymentstatus="";
		 			}
		 			...
		 			param.put("type", type);   
		 			param.put("lumsumlabour", partssublet);
		 			param.put("lumsumparts", partstot);
		 			param.put("totallumsum", partstot+partssublet); 
		 			param.put("lumsumsublet", sublettot);
		 			param.put("totallumsumpal", partstot+partssublet+sublettot); 
		 			param.put("vatamt", vatamt);
		 			param.put("vatamtpal", vatamt);
		 			param.put("nettotal", nettotal);
		 			//System.out.println("nettotal==="+nettotal);
                    param.put("nettotalpal",nettotal);
					...
					System.out.println(partstot+"=="+servicetot+"=="+sublettot+"=="+vatamtpal);
		 			param.put("amountinwordspal", objamount.convertAmountToWords( partstot+servicetot+sublettot+vatamtpal+"")); 
		    	    ...
		    	    param.put("detailsql", detailsql);
		    	    	 ...
		    	    	 if(!addition.equals("") && addition!=null) {
		    	    		 String[] add1=addition.split(",");
		    	    	...
		    	    	param.put("addition1",addition1);
		    	    	param.put("addition2", addition2);
		    	    
		    	    	
		    	    //System.out.println("addition1==="+addition1+"==addition2=="+addition2);
		    	    param.put("addition",addition);
		    	    //System.out.println("addition2==="+addition2);
		    	    param.put("team21detailsql", team21detailsql);
		    	   
		    	    
		    	    String strpyt2="select type,desc1,format(amount,2) amount,qty,rate from(select '' qty,'' rate,'Services' type,convert(concat(a.voc_no,'-S',@i:=@i+1),char(25)) srno,a.description desc1,a.amount from ( select @i:=0 count,lab.strjobdesc description,round(lab.invoiceamt,2) amount,est.voc_no from ws_jobcard card left join ws_estm est on  (card.reftype='EST' and card.refno=est.doc_no) left join ws_estlabour lab on est.doc_no=lab.rdocno left join ws_jobmaster m  on lab.jobid=m.doc_no left join ws_jobtype t on m.jobid=t.doc_no where card.doc_no="+jobcarddocno+" and lab.addition in ("+addition+") and card.status=3 and lab.confirmed=1 and lab.approved=1 and lab.chkcomplete=1 and upper(lab.strjobtype)<>'SUBLET' union all  select @i:=0 count,extra.description,round(extra.amount,2) amount,est.voc_no from ws_jobcard card left join ws_estm est on  (card.reftype='EST' and card.refno=est.doc_no) left join ws_jccextra extra  on card.doc_no=extra.jobcarddocno where card.doc_no="+docno+" and card.status=3  and coalesce(extra.jobcarddocno,0)<>0)a union all  select  qty, rate,'Sublet' type,convert(concat(a.voc_no,'-S',@i:=@i+1),char(25)) srno,a.description desc1,a.amount from ( select @i:=0 count,lab.strjobdesc description,round(lab.invoiceamt,2) amount,0 discount,0 vatamount,0 vatpercent,0 netamount,est.voc_no,round(lab.hrs,2) qty, round(lab.rate,2) rate from ws_jobcard card left join ws_estm est on  (card.reftype='EST' and card.refno=est.doc_no) left join ws_estlabour lab on est.doc_no=lab.rdocno left join ws_jobmaster m  on lab.jobid=m.doc_no left join ws_jobtype t on m.jobid=t.doc_no where card.doc_no="+jobcarddocno+" and lab.addition in ("+addition+") and card.status=3 and lab.confirmed=1 and lab.approved=1 and lab.chkcomplete=1 and upper(lab.strjobtype)='SUBLET') a union all select qty,rate,'Parts' type,convert(concat(a.voc_no,'-P',@j:=@j+1),char(25)) serialno,a.description,a.amount from ( select @j:=0 count,if(spare.qty=0,'',round(spare.qty,0)) qty,if(spare.qty=0,'',round(spare.customeramt/spare.qty,2)) rate,if(spare.description='',m.productname,spare.description) description,round(spare.customeramt,2) amount,est.voc_no from ws_jccspare spare left join ws_jobcard card on spare.jobcarddocno=card.doc_no left join  ws_estm est on (card.reftype='EST' and card.refno=est.doc_no)   left join my_main m on m.psrno=spare.psrno  where card.doc_no="+jobcarddocno+" and spare.addition in ("+addition+") and card.status=3 and spare.chkcomplete=1)a)b";
                    ResultSet rspyt2=stmt.executeQuery(strpyt2);
		 			
		 			//System.out.println("strpyt2=="+strpyt2);
		 			while(rspyt2.next()){
		 				//balance=rspyt.getDouble("diff");
		 				//dramt=rspyt.getDouble("dramt");
		 			}
                    ...
                    param.put("docno", doc);
                    //System.out.println("docno==="+doc);
                    param.put("doc_no", jobcarddocno);
                    //System.out.println("docno==="+jobcarddocno+"==addition=="+addition);
