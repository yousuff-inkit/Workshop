...
public class ClsWSInvoiceDAO {
	...
	public boolean edit(Date sqldate, String cmbreftype, String hidrefno,
		...
		//System.out.println("Sub Total: "+nettotal);
...
//								System.out.println("Excess JV 1:"+strjvparty);
				...
				if(insuracno>0){
					...
					//System.out.println("VAT Amount: "+vatval);
					//System.out.println("Before Subtracting VAT: "+remaintotal);
					...
					partyldramt=partydramt*currate;
					compldramt=compdramt*currate;
					//System.out.println("After Subtracting VAT: "+remaintotal);
					if(compdramt!=0.0){
						String strjvcomp="insert into my_jvtran(tr_no,acno,dramount,rate,curid,out_amount,id,ref_row,brhid,description,yrid,date,dtype,"+
				...
				if (docno > 0) {
					...
					while(rstestjv.next()){
						if(rstestjv.getDouble("dramount")==0.00){
							...
							String strjvtest="select coalesce(jv.dramount,0) dramount,jv.acno,coalesce(h.description,'') acname from my_jvtran jv left join my_head h on jv.acno=h.doc_no where jv.tr_no="+trno;
							ResultSet rsjvtest=conn.createStatement().executeQuery(strjvtest);
							while(rsjvtest.next()) {
								System.out.println(rsjvtest.getString("dramount")+"::"+rsjvtest.getString("acno")+"::"+rsjvtest.getString("acname"));
							}
...
//			System.out.println("AC Query: "+strsql);
	...
	public JSONArray getSearchData(String docno,String date,String jobcardno,String regno,String cldocno,
		...
		try{
			...
			" left join my_head invoiceto on invm.acno=invoiceto.doc_no left join my_head excess on invm.excessacno=excess.doc_no  where invm.status=3"+sqltest;
			//System.out.println(strsql);
			ResultSet rs=stmt.executeQuery(strsql);
			data=objcommon.convertToJSON(rs);
		}
