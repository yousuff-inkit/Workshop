...
public class ClsWSJobCardAction extends ActionSupport{
	...
	public String saveAction() throws ParseException, SQLException{
		...
		if(!mode.equalsIgnoreCase("view")){
			...
			if(mode.equalsIgnoreCase("A")){
				int insertval=jobcarddao.insert(getCmbreftype(),getHidrefno(),sqldate,session,request,
						promdate,getPromtime(),mode,getFormdetailcode(),getBrchName(),getJobdesc());
				if(insertval>0){
	...
	public String printAction() throws ParseException, SQLException,Exception{
			 ...
			 String jobdet=request.getParameter("jobdet");  
			 String printchk=request.getParameter("printchk"); 
			 String otherdetailconfig=request.getParameter("otherdetailconfig"); 
			 String detailarray=request.getParameter("detailarray")==null?"":request.getParameter("detailarray"); 
	               ...
	               Connection conn = null;
			    			 try {
							     ...
							     watermarkroyal=watermarkroyal.replace("\\", "\\\\");
							        ...
							        String printpath="";
							        if(jobdet.equalsIgnoreCase("1")) {
							        	  printpath=objcommon.getPrintPath2("JBC"); 
							        }else {
							        	  printpath=objcommon.getPrintPath("JBC");    
							        } 
							        String detsql="";
							        int i=0;  
									String temparray[]=detailarray.split(",");  
									for(i=0;i<temparray.length;i++){
                                        System.out.println("Array ="+temparray[i]);
                                        if(i==0) {  
                                        	detsql="select '"+temparray[i]+"' details";
                                        }else {
                                        	detsql+=" union all select '"+temparray[i]+"' details";
                                        }
									} 
									param.put("detsql", detsql);   
									param.put("otherdetailconfig", otherdetailconfig);
									//System.out.println("detsql-->"+detsql);   
