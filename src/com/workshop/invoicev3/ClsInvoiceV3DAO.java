...
public class ClsInvoiceV3DAO {

	ClsConnection objconn=new ClsConnection();
	ClsCommon objcommon=new ClsCommon();
	public  double sqlRound(double value,int deci,Connection conn) throws SQLException
    {
	...
	public JSONArray getDetailNewGridEditData(String jobdocno,String id,String invno)throws SQLException{
		...
		try{
			...
			//Removed Complete
			String strsql="select a.invno,a.jobdocno,a.estdocno,a.addition,a.jobtype type,convert(concat(a.voc_no,'-S',@i:=@i+1),char(25)) serialno,a.description,a.hrs qty,a.rate,a.jobdiscount discount,a.jobtotal amount,a.jobvatpercent vatpercent,a.jobvatamount vatamount,a.jobnetamount netamount,a.chkcomplete from ("+
	...
	public JSONArray getDetailNewGridData(String jobdocno,String id,String invno)throws SQLException{
		JSONArray data=new JSONArray();
		...
		try{
			conn=objconn.getMyConnection();
			String strsql="select rowno, type, description, qty, rate, discount, amount, vatpercent, vatamount, netamount, serialno, estdocno, addition, jobdocno, invno from ws_investdetail where invno="+invno;
			ResultSet rs=conn.createStatement().executeQuery(strsql);
			data=objcommon.convertToJSON(rs);
		}
		catch(Exception e){
	...
	public boolean edit(Date sqldate, String cmbreftype, String hidrefno,
			...
			String strroundamt,ArrayList<String> invdetarray) throws SQLException {
		// TODO Auto-generated method stub
		int vocno=0,trno=0;
		try{
			excessamountacno=excessamountacno.trim().equalsIgnoreCase("")?"0":excessamountacno.trim();
...
//				System.out.println(strchecktax);
				...
				if(taxstatus==1 && clienttaxmethod==1){
					...
					while(rsgettax.next()){
						...
						vatval=Double.parseDouble(taxamount);
					...
					if(setpercent>0.0 || vatpercent>0.0){
						...
						for(int j=0;j<temptaxarray.size();j++){
							...
							if(Double.parseDouble(tax[2])>0){
								...
								"'"+curid+"','5',1,'"+cldocno+"',3)";
								System.out.println("Company Tax Jv1:"+strtaxjv);
								tempsrno++;
...
//								System.out.println("Excess JV 1:"+strjvparty);
				...
				if(insuracno>0){
					...
					if(roundamt!=0.0){
						...
						if(roundamt!=0.0){
							String sqljvdisc="insert into my_jvtran(tr_no,acno,dramount,rate,curid,out_amount,id,ref_row,brhid,description,yrid,date,dtype,ldramount,"+
									...
									"'"+curid+"','5',1,"+cldocno+",3)";
							System.out.println("Discount/Round JV:"+sqljvdisc);
							i++;
					...
					System.out.println("1:"+remaintotal);
					remaintotal-=vatval;
					System.out.println("2:"+remaintotal+"::"+vatval);
					partydramt=remaintotal;
					compdramt=remaintotal*-1;
					compdramt=sqlRound(compdramt, 2,conn);
					System.out.println("3:"+compdramt);
					partyldramt=partydramt*currate;
					compldramt=compdramt*currate;
					if(compdramt!=0.0){
						String strjvcomp="insert into my_jvtran(tr_no,acno,dramount,rate,curid,out_amount,id,ref_row,brhid,description,yrid,date,dtype,"+
				...
				while(rsgetinsurtype.next()){
					double insurpercent=0.0;
					if(netamtbeforeedit>0.0){
						insurpercent=(rsgetinsurtype.getDouble("amount")/netamtbeforeedit)*100;
					}					
				...
				//Editing Detail
				String strdeleteinvdet="delete from ws_investdetail where invno="+docno;
				//System.out.println("Delete Details:"+strdeleteinvdet);
				int deleteinvdet=stmt.executeUpdate(strdeleteinvdet);
				
				for(int j=0;j<invdetarray.size();j++){
					String temp[]=invdetarray.get(j).split("::");
					...
					//String stredit="update ws_investdetail set qty="+temp[3]+",rate="+temp[4]+",discount="+temp[5]+",amount="+temp[6]+",vatpercent="+temp[7]+",vatamount="+temp[8]+",netamount="+temp[9]+" where rowno="+temp[0];
					String strinsertinvdet="insert into ws_investdetail(type, description, qty, rate, discount, amount, vatpercent, vatamount, netamount, estdocno, addition, serialno, jobdocno, invno)values("+
					"'"+temp[1]+"','"+temp[2]+"','"+temp[3]+"','"+temp[4]+"','"+temp[5]+"','"+temp[6]+"','"+temp[7]+"','"+temp[8]+"','"+temp[9]+"','"+temp[11]+"','"+temp[12]+"','"+temp[10]+"','"+temp[13]+"',"+docno+")";
					//System.out.println("Detail Insert:"+strinsertinvdet);
					int detupdate=stmt.executeUpdate(strinsertinvdet);
					if(detupdate<0){
						errorstatus=1;
						break;
					}
				...
				if (docno > 0) {
					...
					String testjv="select sum(coalesce(jv.dramount,0)) dramount from my_jvtran jv inner join my_head head on (jv.acno=head.doc_no) where jv.tr_no="+trno;
...
//			System.out.println("AC Query: "+strsql);
	...
	public JSONArray getSearchData(String docno,String date,String jobcardno,String regno,String cldocno,
		...
		try{
			...
			if(!cldocno.equalsIgnoreCase("")){
				sqltest+=" and invoiceto.cldocno like '%"+cldocno+"%'";
			}
			if(!clientname.equalsIgnoreCase("")){
				sqltest+=" and invoiceto.description like '%"+clientname+"%'";
			}
			/*if(!session.getAttribute("BRANCHID").toString().equalsIgnoreCase("")){
				sqltest+=" and invm.brhid="+session.getAttribute("BRANCHID").toString();
			}*/
			String strsql="select invm.invoicetoacno tempinvoicetoacno,invm.excessinvdocno,invoiceto.account billtoaccount,invoiceto.description billtoacname,invm.roundamt,invm.taxpercent,invm.taxamount,invm.taxtotal,est.nettotal,round(invm.total,2) total,round(invm.discount,2) discount,round(invm.excess,2)excessamt,round(invm.nettotal,2) "+
			...
			" (gate.cldocno=ac.cldocno and ac.dtype='CRM')   left join gl_vehbrand brd on gate.brdid=brd.doc_no left join gl_vehmodel model on"+
			...
			" left join my_head invoiceto on invm.acno=invoiceto.doc_no left join my_head excess on invm.excessacno=excess.doc_no  where invm.status=3"+sqltest +" order by invm.voc_no";
			//System.out.println(strsql);
			ResultSet rs=stmt.executeQuery(strsql);
			data=objcommon.convertToJSON(rs);
		}
		...
		try{
			...
			" (card.reftype='EST' and card.refno=est.doc_no) where card.doc_no="+jobcarddocno+" and card.status=3";
				...
				if(proformaconfig==1){
					...
					" where card.doc_no="+jobcarddocno+" and card.status=3 and spare.addition in ("+addition+") and spare.chkcomplete=1";
				}
				else{
					strtest="select @j:=0 count,spare.description,round(spare.approvedvalue,2) amount,est.voc_no from ws_jobcard card left join ws_estm est on"+
							...
							" and card.status=3  and spare.addition in ("+addition+") and spare.chkcomplete=1";
				}
			//}
			String strsql="";
			if(proformaconfig==1){
				strsql="select convert(concat(a.voc_no,'-S',@i:=@i+1),char(25)) serialno,a.description,a.amount from ("+
						...
						" on lab.jobid=m.doc_no left join ws_jobtype t on m.jobid=t.doc_no where card.doc_no="+jobcarddocno+" and card.status=3 and lab.confirmed=1 and lab.approved=1  and lab.addition in ("+addition+") and lab.chkcomplete=1 union all "+
	...
	public ClsInvoiceV3Bean printDetails(String doc, HttpServletRequest request) throws SQLException {
		// TODO Auto-generated method stub
