package com.workshop.estimationv4;
...
public class ClsEstimationV4DAO {
	...
	public String getMailPrint() throws ParseException{
		try{
			ClsEstimationV4Action action=new ClsEstimationV4Action();
	...
	public int insert(String gatedocno, String sparepartstotal,
			...
			String sparetotal, String sparediscount, String netspare,String cmbentitytype,String distservicediscount,String distsparediscount,ClsEstimationV4Action masteraction) throws SQLException {
			...
			if(docno<=0){
				...
				for(int i=0;i<sparepartsarray.size();i++){
					...
					if(!contractdetdocno.equalsIgnoreCase("0")){
						String strsql="insert into ws_estspare(rdocno, srno,description, qty, rate, approvedvalue, addition, confirmed, approved,sptotal,spdiscount,spvatpercent,spvatamount,spnetamount,psrno,distsparediscount,contractdetdocno)values("+
							""+docno+","+sparecount+",'"+temp[0]+"',"+temp[1]+","+temp[2]+","+temp[3]+",0,1,0,"+sptotal+","+spdiscount+","+spvatpercent+","+spvatamount+","+spnetamount+","+psrno+","+distsparediscountitem+","+contractdetdocno+")";
						System.out.println(strsql);
				...
				for(int i=0;i<labourcostarray.size();i++){
					...
					if(!contractdetdocno.equalsIgnoreCase("0")){
						String strsql="insert into ws_estlabour(rdocno, srno, jobid, hrs, rate, markupper, total, remarks, addition, confirmed, approved,strjobtype,strjobdesc,seqno,jobqty,jobdiscount,jobtotal,jobvatpercent,jobvatamount,jobnetamount,distservicediscount,contractdetdocno)values("+
						""+docno+","+labourcount+","+temp[0]+","+temp[1]+","+temp[2]+","+temp[3]+","+temp[4]+",'"+temp[5]+"',0,1,0,'"+strjobtype+"','"+strjobdesc+"',"+seqno+","+jobqty+","+jobdiscount+","+subtotal+","+jobvatpercent+","+jobvatamount+","+jobnetamount+","+distservicediscountitem+","+contractdetdocno+")";
						System.out.println(strsql);
						int gridinsert=stmt.executeUpdate(strsql);
						if(gridinsert<=0){
							errorstatus=1;
	...
	public ClsEstimationV4Bean viewdetails(int docno,int vocno) throws SQLException
	{
		ClsEstimationV4Bean been=new ClsEstimationV4Bean();
		Connection conn=null;
