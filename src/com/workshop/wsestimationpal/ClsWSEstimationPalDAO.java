...
public class ClsWSEstimationPalDAO {
	...
	public JSONArray getSparepartsData(String docno,String id) throws SQLException
	{
		...
		try{
			conn=objconn.getMyConnection();
			Statement stmt=conn.createStatement();
			String strsql="select description, rate sprate, qty, genuinerate, marketrate, usedrate, genuinetotal, markettotal, usedtotal, approval, approvedvalue from ws_estspare where addition=0 and rdocno="+docno+" order by srno";
			...
			String brhid,String regno) throws SQLException
	{
		...
		try{
			...
			strsql="select round(coalesce(m.sparetotal,0),2) sparetotal,round(coalesce(m.sparediscount,0),2) sparediscount,round(coalesce(m.sparenettotal,0),2) netspare,coalesce(insur.refname,'') gipinsurcomp,coalesce(m.claimno,'') gipclaimno,coalesce(m.gipdatetime,'') gipdatetime,coalesce(m.estimatedays,'') estimatedays,m.header,m.notes,m.internalremarks,m.chkservicelumsum,round(m.servicelumsumamt,2) servicelumsumamt,m.chkrandomlumsum,round(m.randomlumsumamt,2) randomlumsumamt,m.chklumsum,round(m.lumsumamount,2) lumsumamount,m.doc_no,m.voc_no,m.date,round(m.servicestotal,2) servicestotal, round(m.servicesdiscount,2) servicesdiscount, round(m.netservices,2) netservices,"+
	...
	public ClsWSEstimationPalBean viewdetails(int docno,int vocno) throws SQLException
	{
		...
		try{
			...
			strsql="select round(coalesce(m.sparetotal,0),2) sparetotal,round(coalesce(m.sparediscount,0),2) sparediscount,round(coalesce(m.sparenettotal,0),2) netspare,coalesce(m.brhid,0) estbrhid,coalesce(insur.refname,'') gipinsurcomp,coalesce(m.claimno,'') gipclaimno,coalesce(m.gipdatetime,'') gipdatetime,coalesce(m.estimatedays,'') estimatedays,m.header,m.notes,coalesce(m.internalremarks,'') internalremarks,"+
