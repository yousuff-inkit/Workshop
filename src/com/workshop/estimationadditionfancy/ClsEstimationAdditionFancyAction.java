...
public class ClsEstimationAdditionFancyAction extends ActionSupport{
	...
	ClsEstimationAdditionFancyDAO estimationdao=new ClsEstimationAdditionFancyDAO();
	ClsEstimationAdditionFancyBean been=new ClsEstimationAdditionFancyBean();
	private String jobcardvocno,jobcarddocno,addition,estdocno;
	private String sparemarkup;
	private String labdiscount;
	private String sparetotal,sparediscount,sparenettotal;
	private String genuinetotal,markettotal,usedtotal,approvedtotal;
	
	
	
	public String getGenuinetotal() {
		return genuinetotal;
	}
	public void setGenuinetotal(String genuinetotal) {
		this.genuinetotal = genuinetotal;
	}
	...
	public String getSparemarkup() {
		return sparemarkup;
	}
	public void setSparemarkup(String sparemarkup) {
		this.sparemarkup = sparemarkup;
	}
	public String getLabdiscount() {
		return labdiscount;
	}
	public void setLabdiscount(String labdiscount) {
	...
	public String getSparenettotal() {
		return sparenettotal;
	}
	public void setSparenettotal(String sparenettotal) {
		this.sparenettotal = sparenettotal;
	}
	...
	public void setData(String docno,String vocno,java.sql.Date sqldate,String addition){
		...
		setSparemarkup(getSparemarkup());
		setLabdiscount(getLabdiscount());
		setSparediscount(getSparediscount());
		setSparenettotal(getSparenettotal());
		setSparetotal(getSparetotal());
		setGenuinetotal(getGenuinetotal());
		setMarkettotal(getMarkettotal());
		setUsedtotal(getUsedtotal());
		setApprovedtotal(getApprovedtotal());
	...
	public String saveAction() throws ParseException, SQLException{
	    ...
	    int jc_no=(Integer.parseInt(jcno));
	 	been=estimationdao.viewdetails(doc_no,jc_no,add);
			...
			setServicestotal(been.getServicestotal());
			setSparediscount(been.getSparediscount());
			setSparemarkup(been.getSparemarkup());
			setSparenettotal(been.getSparenettotal());
			setSparetotal(been.getSparetotal());
			setLabdiscount(been.getLabdiscount());
			setGateuserdetails(been.getGateuserdetails());
			setGatevehicledetails(been.getGatevehicledetails());
			setServicesdiscount(been.getServicesdiscount());
			setNetservices(been.getNetservices());			//System.out.println("dsfdnfhd"+been.getNetservices());
			return "success";
		}
		...
		if(!mode.equalsIgnoreCase("view")){
			...
			if(mode.equalsIgnoreCase("A")){
				...
				int insertval=estimationdao.insert(getGatedocno(),getSparepartstotal(),getLabourtotal(),getDiscount(),getEsttotal(),sqldate,
						...
						getServicestotal(),getNetservices(),getHidchklumsum(),getLumsumamount(),getJobcarddocno(),getEstdocno(),getSparetotal(),
						getSparediscount(),getSparenettotal());
				/*,getGenuinetotal(),getMarkettotal(),getUsedtotal(),getApprovedtotal()*/
				if(insertval>0){
				...
				boolean status=estimationdao.edit(getGatedocno(),getSparepartstotal(),getLabourtotal(),getDiscount(),getEsttotal(),sqldate,
						...
						getServicesdiscount(),getServicestotal(),getNetservices(),getHidchklumsum(),getLumsumamount(),getJobcarddocno(),
						getEstdocno(),getAddition(),getSparetotal(),getSparediscount(),getSparenettotal());
				if(status){
