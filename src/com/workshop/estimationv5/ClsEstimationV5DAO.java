package com.workshop.estimationv5;
...
public class ClsEstimationV5DAO {
	...
	public JSONArray getPackageSpareData(String docno,String id,String contractdocno) throws SQLException
	{
		JSONArray data=new JSONArray();
		...
		try{
			conn=objconn.getMyConnection();
			Statement stmt=conn.createStatement();
			String strsql="select doc_no contractdetdocno,pkgqty,0.0 distsparediscount,spdesc description,0.0 sprate, qty,0.0 genuinerate,0.0 marketrate,0.0 usedrate,0.0 genuinetotal,0.0 markettotal,0.0 usedtotal,null approval,0.0 approvedvalue,psrno,0.0 spdiscount,0.0 spvatpercent,0.0 spvatamount,0.0 spnetamount,0.0 sptotal from ws_pkgcontractspare where contractdocno="+contractdocno+" order by seqno";
	...
	public JSONArray getPackageLabourData(String docno,String id,String contractdocno)throws SQLException{
		JSONArray data=new JSONArray();
		if(!id.equalsIgnoreCase("2")){
			return data;
		}
		...
		try{
			...
			String strsql="select lab.doc_no contractdetdocno,lab.pkgqty,1 taxable,0.0 distservicediscount,0.0 jobnetamount,0.0 jobvatamount,0.0 jobvatpercent,0.0 jobtotal,0.0 jobdiscount,lab.jobqty,lab.seqno,lab.jobdesc jobdesc,lab.jobqty hrs,0.0 rate,lab.strjobtype jobtype,0.0 markuppercent,0.0 total,lab.remarks, "+
			" lab.jobtypeid jobid from ws_pkgcontractlabour lab "+
			" where lab.contractdocno='"+contractdocno+"' order by seqno";
	...
	public JSONArray getPackage(String gatedocno,String date,String id) throws SQLException{
		...
		try{
			...
			/*String strsql="select m.doc_no,m.packagename,m.fromdate,m.todate,m.amount,m.maxusage-coalesce(estmax.packagecount,0) maxusage,cnt.doc_no contractdocno from ws_packagem m"+
			...
			" where m.status=3 and '"+sqldate+"' between m.fromdate and m.todate  and cnt.modeldocno="+modelid+" and cnt.doc_no is not null";*/
			String strsql="select m.doc_no,m.packagename,m.fromdate,m.todate,m.amount,m.maxusage-coalesce(estmax.packagecount,0) maxusage,cnt.doc_no contractdocno from ws_packagecontract cnt"+
			" left join ws_packagem m on cnt.packagedocno=m.doc_no"+
			" left join (select count(*) packagecount,pkgcontractdocno from ws_gateinpass gate"+
			" left join  ws_estm est on est.gipno=gate.doc_no where packagedocno<>0 group by cldocno,packagedocno) estmax on"+
			" (cnt.doc_no=estmax.pkgcontractdocno)"+
			" left join (select sum(pkgqty) pkglabourqty,0 pkgspareqty,contractdocno from ws_pkgcontractlabour group by contractdocno union all"+
			" select 0 pkglabourqty,sum(pkgqty) pkgspareqty,contractdocno from ws_pkgcontractspare group by contractdocno) maxpkg on (cnt.doc_no=maxpkg.contractdocno)"+
			" where cnt.cldocno="+cldocno+" and cnt.modeldocno="+modelid+" and '"+sqldate+"' between cnt.fromdate and cnt.todate and cnt.status=3 and (maxpkg.pkglabourqty+maxpkg.pkgspareqty)>0 group by cnt.doc_no";
			System.out.println(strsql);
			ResultSet rs=stmt.executeQuery(strsql);
			data=objcommon.convertToJSON(rs);
		}
	...
	public String getMailPrint() throws ParseException{
		try{
			ClsEstimationV5Action action=new ClsEstimationV5Action();
	...
	public JSONArray getLabourcostData(String docno,String id)throws SQLException{
		...
		try{
			...
			" lab.jobid from ws_estlabour lab left join ws_jobmaster m on (lab.jobid=m.doc_no ) left join ws_jobtype t on m.jobid=t.doc_no "+
			" where m.status=3 and lab.addition=0 and lab.rdocno='"+docno+"'";*/
			String strsql="select lab.contractdetdocno,coalesce(pkglab.pkgqty,0) pkgqty,t.taxable,lab.distservicediscount,lab.jobnetamount,lab.jobvatamount,lab.jobvatpercent,lab.jobtotal,lab.jobdiscount,lab.jobqty,lab.seqno,lab.strjobdesc jobdesc,lab.hrs,lab.rate,lab.strjobtype jobtype,lab.markupper markuppercent,lab.total,lab.remarks, "+
			" lab.jobid from ws_estlabour lab left join ws_jobtype t on lab.jobid=t.doc_no "+
			" left join ws_pkgcontractlabour pkglab on pkglab.doc_no=lab.contractdetdocno"+
	...
	public JSONArray getSparepartsData(String docno,String id) throws SQLException
	{
		...
		try{
			conn=objconn.getMyConnection();
			Statement stmt=conn.createStatement();
			String strsql="select sp.contractdetdocno,coalesce(pkgsp.pkgqty,0) pkgqty,sp.distsparediscount,sp.description, sp.rate sprate, sp.qty, "+
			" sp.genuinerate, sp.marketrate, sp.usedrate, sp.genuinetotal, sp.markettotal, sp.usedtotal,sp.approval, sp.approvedvalue,sp.psrno,"+
			" sp.spdiscount,sp.spvatpercent,sp.spvatamount,sp.spnetamount,sp.sptotal from ws_estspare sp "+
			" left join ws_pkgcontractspare pkgsp on sp.contractdetdocno=pkgsp.doc_no "+
			" where sp.addition=0 and sp.rdocno="+docno+" order by sp.srno";
			System.out.println("Spare==="+strsql);
			ResultSet rs=stmt.executeQuery(strsql);
			data=objcommon.convertToJSON(rs);
	...
	public int insert(String gatedocno, String sparepartstotal,
			...
			String sparetotal, String sparediscount, String netspare,String cmbentitytype,String distservicediscount,String distsparediscount,ClsEstimationV5Action masteraction) throws SQLException {
		...
		try{
			...
			if(docno<=0){
				...
				String packagecontractdocno=masteraction.getPkgcontractdocno();
				packagecontractdocno=packagecontractdocno==null || packagecontractdocno.trim().equalsIgnoreCase("") || packagecontractdocno.trim().equalsIgnoreCase("undefined")?"0":packagecontractdocno.trim();
				
				String cmbbillto=masteraction.getCmbbillto();
				cmbbillto=cmbbillto==null || cmbbillto.trim().equalsIgnoreCase("") || cmbbillto.trim().equalsIgnoreCase("undefined")?"0":cmbbillto.trim();
				
				String billtocldocno=masteraction.getBilltocldocno();
				billtocldocno=billtocldocno==null || billtocldocno.trim().equalsIgnoreCase("") || billtocldocno.trim().equalsIgnoreCase("undefined")?"0":billtocldocno.trim();
				
				String strupdateest="update ws_estm set billto="+cmbbillto+",billtocldocno="+billtocldocno+",pkgcontractdocno="+packagecontractdocno+",packagedocno="+packagedocno+",distservicediscount="+distservicediscount+",distsparediscount="+distsparediscount+",entitytype="+cmbentitytype+",sparetotal="+sparetotal+",sparediscount="+sparediscount+",sparenettotal="+netspare+",claimno='"+gipclaimno+"',gipdatetime='"+gipdatetime+"',estimatedays='"+estimatedays+"',header='"+header+"',notes='"+notes+"',internalremarks='"+internalremarks+"',chkservicelumsum="+hidchkservicelumsum+",servicelumsumamt="+servicelumsumamt+",chkrandomlumsum="+hidchkrandomlumsum+",randomlumsumamt="+randomlumsumamt+" where doc_no="+docno;
				...
				//Getting Gate Insur cldocno
				if(cmbbillto.equalsIgnoreCase("2")) {
					String strgateinsur="select insurancecomp from ws_gateinpass where doc_no="+gatedocno;
					ResultSet rsgateinsur=stmt.executeQuery(strgateinsur);
					int gateinsurstatus=0;
					while(rsgateinsur.next()) {
						gateinsurstatus=rsgateinsur.getInt("insurancecomp");
					}
					if(gateinsurstatus==0) {
						String strupdategate="update ws_gateinpass set insurancecomp=1,insurcldocno="+billtocldocno+" where doc_no="+gatedocno;
						int updategate=stmt.executeUpdate(strupdategate);
						if(updategate<=0) {
							System.out.println("GIP Insur Update Error");
							errorstatus=1;
							return 0;
						}
					}
				...
				for(int i=0;i<sparepartsarray.size();i++){
					...
					String contractdetdocno=temp[11].trim().equalsIgnoreCase("")||temp[11].trim().equalsIgnoreCase("undefined")||temp[11]==null||temp[11].isEmpty()?"0":temp[11].trim();
					if(!contractdetdocno.equalsIgnoreCase("0")){
						String strsql="insert into ws_estspare(rdocno, srno,description, qty, rate, approvedvalue, addition, confirmed, approved,sptotal,spdiscount,spvatpercent,spvatamount,spnetamount,psrno,distsparediscount,contractdetdocno,seccldocno)values("+
							""+docno+","+sparecount+",'"+temp[0]+"',"+temp[1]+","+temp[2]+","+temp[3]+",0,1,0,"+sptotal+","+spdiscount+","+spvatpercent+","+spvatamount+","+spnetamount+","+psrno+","+distsparediscountitem+","+contractdetdocno+","+billtocldocno+")";
						System.out.println(strsql);
						...
						//Updating in Contract Spare
						String strupdatespare="update ws_pkgcontractspare set pkgqty=(pkgqty-"+temp[1]+") where doc_no="+contractdetdocno;
						System.out.println(strupdatespare);
						int updatespare=stmt.executeUpdate(strupdatespare);
					}
					else{
						String strsql="insert into ws_estspare(rdocno, srno,description, qty, rate, approvedvalue, addition, confirmed, approved,sptotal,spdiscount,spvatpercent,spvatamount,spnetamount,psrno,distsparediscount,seccldocno)values("+
							""+docno+","+sparecount+",'"+temp[0]+"',"+temp[1]+","+temp[2]+","+temp[3]+",0,1,0,"+sptotal+","+spdiscount+","+spvatpercent+","+spvatamount+","+spnetamount+","+psrno+","+distsparediscountitem+","+billtocldocno+")";
						System.out.println(strsql);
						int gridinsert=stmt.executeUpdate(strsql);
						if(gridinsert<=0){
							errorstatus=1;
				...
				for(int i=0;i<labourcostarray.size();i++){
					...
					String contractdetdocno=temp[15].trim().equalsIgnoreCase("")||temp[15].trim().equalsIgnoreCase("undefined")||temp[15]==null||temp[15].isEmpty()?"0":temp[15].trim();
					...
					if(!contractdetdocno.equalsIgnoreCase("0")){
						String strsql="insert into ws_estlabour(rdocno, srno, jobid, hrs, rate, markupper, total, remarks, addition, confirmed, approved,strjobtype,strjobdesc,seqno,jobqty,jobdiscount,jobtotal,jobvatpercent,jobvatamount,jobnetamount,distservicediscount,contractdetdocno,seccldocno)values("+
						""+docno+","+labourcount+","+temp[0]+","+temp[1]+","+temp[2]+","+temp[3]+","+temp[4]+",'"+temp[5]+"',0,1,0,'"+strjobtype+"','"+strjobdesc+"',"+seqno+","+jobqty+","+jobdiscount+","+subtotal+","+jobvatpercent+","+jobvatamount+","+jobnetamount+","+distservicediscountitem+","+contractdetdocno+","+billtocldocno+")";
						System.out.println(strsql);
						int gridinsert=stmt.executeUpdate(strsql);
						if(gridinsert<=0){
							errorstatus=1;
						...
						//Updating in Contract Spare
						String strupdatelabour="update ws_pkgcontractlabour set pkgqty=(pkgqty-"+temp[1]+") where doc_no="+contractdetdocno;
						int updatelabour=stmt.executeUpdate(strupdatelabour);
					}
					else{
						String strsql="insert into ws_estlabour(rdocno, srno, jobid, hrs, rate, markupper, total, remarks, addition, confirmed, approved,strjobtype,strjobdesc,seqno,jobqty,jobdiscount,jobtotal,jobvatpercent,jobvatamount,jobnetamount,distservicediscount,seccldocno)values("+
						""+docno+","+labourcount+","+temp[0]+","+temp[1]+","+temp[2]+","+temp[3]+","+temp[4]+",'"+temp[5]+"',0,1,0,'"+strjobtype+"','"+strjobdesc+"',"+seqno+","+jobqty+","+jobdiscount+","+subtotal+","+jobvatpercent+","+jobvatamount+","+jobnetamount+","+distservicediscountitem+","+billtocldocno+")";
						System.out.println(strsql);
						int gridinsert=stmt.executeUpdate(strsql);
						if(gridinsert<=0){
							errorstatus=1;
			...
			try{
				...
				if(updateval<0){
					...
					for(int i=0;i<sparepartsarray.size();i++){
						...
						String distsparediscountitem=temp[10].trim().equalsIgnoreCase("")||temp[10].trim().equalsIgnoreCase("undefined")||temp[10]==null||temp[10].isEmpty()?"0":temp[10].trim();
						String contractdetdocno=temp[11].trim().equalsIgnoreCase("")||temp[11].trim().equalsIgnoreCase("undefined")||temp[11]==null||temp[11].isEmpty()?"0":temp[11].trim();
						String pkgqty=temp[12].trim().equalsIgnoreCase("")||temp[12].trim().equalsIgnoreCase("undefined")||temp[12]==null||temp[12].isEmpty()?"0":temp[12].trim();
						if(!contractdetdocno.equalsIgnoreCase("0")){
							String strsql="insert into ws_estspare(rdocno, srno,description, qty, rate, approvedvalue, addition, confirmed, approved,sptotal,spdiscount,spvatpercent,spvatamount,spnetamount,psrno,distsparediscount,contractdetdocno,seccldocno)values("+
								""+docno+","+sparecount+",'"+temp[0]+"',"+temp[1]+","+temp[2]+","+temp[3]+",0,1,0,"+sptotal+","+spdiscount+","+spvatpercent+","+spvatamount+","+spnetamount+","+psrno+","+distsparediscountitem+","+contractdetdocno+","+billtocldocno+")";
							System.out.println(strsql);
					...
					for(int i=0;i<labourcostarray.size();i++){
						...
						String distservicediscountitem=temp[14].trim().equalsIgnoreCase("")||temp[14].trim().equalsIgnoreCase("undefined")||temp[14]==null||temp[14].isEmpty()?"0.0":temp[14].trim();
						String contractdetdocno=temp[15].trim().equalsIgnoreCase("")||temp[15].trim().equalsIgnoreCase("undefined")||temp[15]==null||temp[15].isEmpty()?"0":temp[15].trim();
						String pkgqty=temp[16].trim().equalsIgnoreCase("")||temp[16].trim().equalsIgnoreCase("undefined")||temp[16]==null||temp[16].isEmpty()?"0":temp[16].trim();
			...
			String brhid,String regno) throws SQLException
	{
		...
		try{
			...
			" left join my_acbook billto on (m.billtocldocno=billto.cldocno and billto.dtype='CRM')"+
			" left join ws_packagem pkg on m.packagedocno=pkg.doc_no where m.status!=7 "+sqltest;
			ResultSet rs=stmt.executeQuery(strsql);
			data=objcommon.convertToJSON(rs);
		}
		catch(Exception e){
	...
	public ClsEstimationV5Bean viewdetails(int docno,int vocno) throws SQLException
	{
		ClsEstimationV5Bean been=new ClsEstimationV5Bean();
		Connection conn=null;
		...
		try{
			...
			" gate.yom=yom.doc_no left join my_acbook insur on (insur.cldocno=gate.insurcldocno and insur.dtype='CRM') "+
			" left join my_acbook billto on (m.billtocldocno=billto.cldocno and billto.dtype='CRM')"+
			" left join ws_packagem pk on m.packagedocno=pk.doc_no where m.status<>7 and m.doc_no="+docno;
			System.out.println("View SQL"+strsql);
			ResultSet rs=stmt.executeQuery(strsql);
			while(rs.next()) {
				been.setHidcmbbillto(rs.getString("billto"));
				been.setCmbbillto(rs.getString("billto"));
				been.setBilltocldocno(rs.getString("billtocldocno"));
				been.setBilltorefname(rs.getString("billtorefname"));
