...
public class ClsEstimationV5Action extends ActionSupport{

	ClsEstimationV5DAO estimationdao=new ClsEstimationV5DAO();
	...
	private String pkgcontractdocno;
	private String cmbbillto,billtorefname,billtocldocno,hidcmbbillto;
	
	
	
	public String getHidcmbbillto() {
		return hidcmbbillto;
	}
	public void setHidcmbbillto(String hidcmbbillto) {
		this.hidcmbbillto = hidcmbbillto;
	}
	public String getCmbbillto() {
		return cmbbillto;
	}
	public void setCmbbillto(String cmbbillto) {
		this.cmbbillto = cmbbillto;
	}
	public String getBilltorefname() {
		return billtorefname;
	}
	public void setBilltorefname(String billtorefname) {
		this.billtorefname = billtorefname;
	}
	public String getBilltocldocno() {
		return billtocldocno;
	}
	public void setBilltocldocno(String billtocldocno) {
		this.billtocldocno = billtocldocno;
	}
	public String getPkgcontractdocno() {
		return pkgcontractdocno;
	}
	public void setPkgcontractdocno(String pkgcontractdocno) {
		this.pkgcontractdocno = pkgcontractdocno;
	}
	...
	public void setData(String docno,String vocno,java.sql.Date sqldate){
		setCmbbillto(getCmbbillto());
		setBilltorefname(getBilltorefname());
		setBilltocldocno(getBilltocldocno());
		setHidcmbbillto(getCmbbillto());
		setDocno(docno);
		setVocno(vocno);
	...
	public String saveAction() throws ParseException, SQLException{
		...
		String mode=getMode();
		String formdocno=getDocno();
	    System.out.println(mode+"::"+formdocno);
		if(mode.equalsIgnoreCase("view")){
		String id=request.getParameter("id")==null?"0":request.getParameter("id");
		String modee=request.getParameter("mode")==null?"":request.getParameter("mode");
	    String docno=request.getParameter("docno")==null?"0":request.getParameter("docno");
	    String gipnoo=request.getParameter("gipnoo")==null?"0":request.getParameter("gipnoo");
	    
		if(!formdocno.equalsIgnoreCase("") && formdocno!=null) {
	    	docno=formdocno;
	    	id="2";
	    	modee=mode;
	    }
	    ...
	    int gip_no=(Integer.parseInt(gipnoo));
	 	been=estimationdao.viewdetails(doc_no,gip_no);
	 		setHidcmbbillto(been.getHidcmbbillto());
	 		setBilltorefname(been.getBilltorefname());
	 		setBilltocldocno(been.getBilltocldocno());
		...
		if(!mode.equalsIgnoreCase("view")){
			...
			if(mode.equalsIgnoreCase("A")){
				...
				ClsEstimationV5Action masteraction=new ClsEstimationV5Action();
				masteraction.setPackagedocno(getPackagedocno());
				masteraction.setPackagename(getPackagename());
				masteraction.setPkgcontractdocno(getPkgcontractdocno());
				masteraction.setCmbbillto(getCmbbillto());
				masteraction.setBilltocldocno(getBilltocldocno());
		...
		if(objcommon.getPrintPath("EST").contains("")==true){
	    	...
	    	Statement stmt = conn.createStatement();
	    	     ...
	    	     String strsql5=" select sum(total) subtotal, sum(vatamt) vatamt, sum(nettotal) nettotal  from (select sum(sptotal) total, sum(spvatamount) vatamt, sum(spnetamount) nettotal from ws_estspare where rdocno="+docno+" and addition="+addition+"  union all "
	    	    		 +" select sum(jobtotal) total, sum(jobvatamount) vatamt, sum(jobnetamount) nettotal from ws_estlabour where rdocno= "+docno+" and addition="+addition+"  and lumsumstatus=0  ) a " ;
...
//				    	    	 lumsumlab=rs5.getDouble("servicetotal"); 
	    	     ...
	    	     String strsqls1="select sum(amount) amount,sum(lab) lab,sum(flatserviceamt) flatserviceamt,sum(flatsubletamt) flatsubletamt from(select round(if(upper(lab.strjobtype)='SUBLET',lab.hrs*lab.rate,0),2) flatsubletamt,if(upper(lab.strjobtype)!='SUBLET',lab.hrs*lab.rate,0) flatserviceamt,upper(lab.strjobtype) type , if(upper(lab.strjobtype)='SUBLET',lab.total,0) amount , if(upper(lab.strjobtype)!='SUBLET',lab.total,0) lab,rdocno from ws_estlabour lab where lab.addition="+addition+" and lab.rdocno='"+docno+"' and lumsumstatus=0 )a group by rdocno";
