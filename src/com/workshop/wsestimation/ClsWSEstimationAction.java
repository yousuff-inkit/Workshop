...
public class ClsWSEstimationAction {

	ClsWSEstimationDAO estimationdao=new ClsWSEstimationDAO();
	...
	public String saveAction() throws ParseException, SQLException{
		...
		if(!mode.equalsIgnoreCase("view")){
			...
			for(int i=0;i<getSparepartsgridlength();i++){
			...
			if(mode.equalsIgnoreCase("A")){
				int insertval=estimationdao.insert(getGatedocno(),getSparepartstotal(),getLabourtotal(),getDiscount(),getEsttotal(),sqldate,
						sparepartsarray,labourcostarray,session,request,mode,getFormdetailcode(),getBrchName());
				if(insertval>0){
			...
			bean=estimationdao.getViewDetails(getDocno());
			setDocno(bean.getDocno());
			setVocno(bean.getVocno());
			setDate(bean.getDate());
			setSparepartstotal(bean.getSparepartstotal());
			setLabourtotal(bean.getLabourtotal());
			setDiscount(bean.getDiscount());
			setEsttotal(bean.getEsttotal());
			setGatevehicledetails(bean.getGatevehicledetails());
			setGatedocno(bean.getGatedocno());
			setGatevocno(bean.getGatevocno());
			setGateuserdetails(bean.getGateuserdetails());
