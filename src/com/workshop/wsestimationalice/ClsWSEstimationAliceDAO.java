...
public class ClsWSEstimationAliceDAO {
	...
	public JSONArray getSparepartsData(String docno,String id) throws SQLException
	{
		...
		try{
			conn=objconn.getMyConnection();
			Statement stmt=conn.createStatement();
			String strsql="select description, rate sprate, qty, genuinerate, marketrate, usedrate, genuinetotal, markettotal, usedtotal, approval, approvedvalue from ws_estspare where addition=0 and rdocno="+docno;
		...
		try{
			...
			if(docno<=0){
				...
				for(int i=0;i<labourcostarray.size();i++){
					...
					""+docno+","+labourcount+","+temp[0]+","+temp[1]+","+temp[2]+","+temp[3]+","+temp[4]+",'"+temp[5]+"',0,1,0)";
					System.out.println(strsql);
					int gridinsert=stmt.executeUpdate(strsql);
					if(gridinsert<=0){
						errorstatus=1;
	...
	public JSONArray getMasterSearch(String gatevocno,String cldocno,String clientname,String docno,String date,String id,String brhid) throws SQLException
	{
		JSONArray data=new JSONArray();
		if(!id.equalsIgnoreCase("1")){
			return data;
	...
	public ClsWSEstimationAliceBean viewdetails(int docno,int vocno) throws SQLException
	{
