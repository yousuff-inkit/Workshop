...
public class ClsRentalAgentAction extends ActionSupport{
	...
	public String saveAction() throws ParseException, SQLException{
		...
		if(mode.equalsIgnoreCase("A")){
						int val=rentalagentDAO.insert(getCode(),getName(),renatalAgentDate,getHidacno(),session,getMode(),getMail(),getMobile(),getFormdetailcode(),getCmbactive());
						if(val>0.0){
				...
				int Status=rentalagentDAO.edit(getCode(),getName(),renatalAgentDate,getHidacno(),session,getMode(),getDocno(),getMail(),getMobile(),getFormdetailcode(),getCmbactive());
				if(Status>0){
					setCode(getCode());
					setName(getName());
					setHidacno(getHidacno());
