...
//						System.out.println("Success");
			...
			if(getCmbtariftype().equalsIgnoreCase("Slab")){
				ArrayList<String> slabarray= new ArrayList<>();
				for(int i=0;i<getSlablength();i++){
					String tempslab=requestParams.get("txtslab"+i)[0];
					slabarray.add(tempslab);
				}
				
				int valslab=tarifDAO.insert_slab(slabarray,getTempgroup(),getDocno(),session,getTempstatus(),getHidgroupdoc());
				if(valslab>0){
					
					getValues();
					if(!(getCmbtariftype().equalsIgnoreCase("Client") || getCmbtariftype().equalsIgnoreCase("Corporate"))){
						setTxtclient("");
...
//					System.out.println("Success");
			...
			if(getCmbtariftype().equalsIgnoreCase("Package")){
				ArrayList<String> packagearray= new ArrayList<>();
						for(int i=0;i<getPackagelength();i++){
							String temppackage=requestParams.get("txtpackage"+i)[0];
							packagearray.add(temppackage);
							
						}
				
				int valpackage=tarifDAO.insert_package(packagearray,getTempgroup(),getDocno(),session,getTempstatus(),getHidgroupdoc());
				if(valpackage>0){
