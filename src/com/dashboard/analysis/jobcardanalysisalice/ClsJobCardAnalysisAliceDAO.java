...
public class ClsJobCardAnalysisAliceDAO {
	...
	public JSONArray invoiceStatusSearch() throws SQLException {
        JSONArray RESULTDATA=new JSONArray();
        Connection  conn = null;
		...
		try {
			...
			String strSql="select 1 docno ,'Invoiced' status union all select 2 docno ,'Pending' status";
	...
	public JSONArray getDetailData(String fromdate,String todate,String hidclient,String hidclientslm,String hidrepairtype,String hidinvoicestatus,
			String hidaccname,String id) throws SQLException {
        JSONArray RESULTDATA=new JSONArray();
        Connection  conn = null;
      ...
      //  System.out.println("hidrepairtype=="+hidrepairtype);
        
     //   System.out.println("hidinvoicestatus=="+hidinvoicestatus);
		...
		try {
			...
			String strSql="select model.vtype model,brand.brand_name brand,yom.yom,'View Attachments' btnattach,job.brhid,job.doc_no jobdocno,coalesce(lbr.spramt,0) actualspare,sm1.sal_name serviceadvisor,sm.sal_name estimator,cl.category clcategory,job.date,invm.voc_no invoiceno,job.voc_no jobno,invm.nettotal totalinv,invm.clienttotal,invm.excesstotal,ac.refname client,"
			+ "case when ((bac.refname=''  or bac.refname is null) and gate.cldocno=1) then 'ALICE RENT A CAR LLC' when bac.refname<>'' or bac.refname is null then coalesce(bac.refname,'NON INSURANCE JOBS') end   insurname,"+
			" concat(gate.regno,'-',gate.pltid) regno,usr.user_name,rtype.name repairtype,"+
			" coalesce(estl.total,0) labour, "+
			" slm.sal_name salesman,"+
			" h.description account,case when gate.processstatus=10 then 'Released' when gate.processstatus=7 then 'Invoiced' else 'Pending' end invstatus,coalesce(gate.mainremarks,'') maintenanceremarks from ws_jobcard job left join (select reftype,date,convert(group_concat(voc_no SEPARATOR ' ,'),char(50)) voc_no,refno,sum(nettotal) nettotal,sum(coalesce(nettotal,0))-coalesce(excess,0) clienttotal, coalesce(excess,0) excesstotal from ws_invm  "+
	...
	public JSONArray getSummaryData(String fromdate,String todate,String hidclient,String hidclientslm,String hidrepairtype,String hidinvoicestatus,
			String hidaccname,String id,String sumtype) throws SQLException {
        JSONArray RESULTDATA=new JSONArray();
        Connection  conn = null;
        if(!id.equalsIgnoreCase("1")){
        	return RESULTDATA;
        ...
        if(!hidaccname.equalsIgnoreCase("")){
			 sqltest+=" and bac.cldocno in ("+hidclient+")";
		 }
        if(!hidclientslm.equalsIgnoreCase("")){
			 sqltest+=" and slm.doc_no in ("+hidclientslm+")";
		 }
       ...
       if(!hidinvoicestatus.equalsIgnoreCase("") && hidinvoicestatus.equalsIgnoreCase("3")){
        ...
        if(sumtype.equalsIgnoreCase("clt")){
        ...
        else if(sumtype.equalsIgnoreCase("accname")){
			 sqlselect=" bac.cldocno,bac.refname,";
			 sqlgroup=" group by bac.cldocno"; 
		 ...
		 }else if(sumtype.equalsIgnoreCase("invs")){
			 
			 sqlselect=" if(invm.nettotal is not null,'Invoiced','Pending') refname,";
			 sqlgroup=" group by invstatus";
...
//			if(sum(coalesce(invm.nettotal,0)- coalesce(lbr.spramt,0))<0,0,sum(coalesce(invm.nettotal,0)- coalesce(lbr.spramt,0))) labour
			String strSql="select "+sqlselect+"if(invm.nettotal is not null,'Invoiced','Pending') invstatus,sum(coalesce(invm.nettotal,0)) totalinv,coalesce(estl.total,0) labour,coalesce(lbr.spramt,0) spares "
			+"  from ws_jobcard job left join (select reftype,date,voc_no,refno,sum(nettotal) nettotal from ws_invm   group by refno,doc_no) invm on (job.doc_no=invm.refno and invm.reftype='JC')"
			+" left join (select jobcarddocno,sum(coalesce(customeramt,0)) spramt from ws_jccspare group by jobcarddocno) lbr on lbr.jobcarddocno=job.doc_no"
			+" left join ws_estm est on (job.reftype='EST' and job.refno=est.doc_no) left join (select rdocno,total from ws_estlabour group by rdocno) estl on estl.rdocno=est.doc_no left join ws_gateinpass gate on (if(job.reftype='GIP',job.refno=gate.doc_no,est.gipno=gate.doc_no))"
			+" left join my_acbook ac on (gate.cldocno=ac.cldocno and ac.dtype='CRM')"
			+" left join my_acbook bac on (gate.insurcldocno=bac.cldocno and bac.dtype='CRM' and gate.insurancecomp=1)"
			+" left join my_salm slm on ac.sal_id=slm.doc_no"
			+" left join ws_gartype rtype on gate.repairtype=rtype.row_no"
			+" where 1=1 and job.date between '"+sqlfromdate+"' and '"+sqltodate+"' and job.status<>7 "+sqltest+sqlgroup;

			
		//	System.out.println("job card analysis summary---:"+strSql);
			ResultSet resultSet = detailstmt.executeQuery(strSql);
        	RESULTDATA=ClsCommon.convertToJSON(resultSet);
	...
	public JSONArray getDetailExportData(String fromdate,String todate,String hidclient,String hidclientslm,String hidrepairtype,
			String hidinvoicestatus,String hidaccname,String id)throws SQLException
	{
		JSONArray RESULTDATA=new JSONArray();
        Connection  conn = null;
        if(!id.equalsIgnoreCase("1")){
...
/*			invm.date,invm.voc_no invoiceno,invm.refno jobno,invm.nettotal totalinv,ac.refname client,"+
			...
			String strSql="select job.date 'Date',invm.voc_no 'Invoice No',ac.refname 'Client Name',cl.category 'Client Category',sm.sal_name 'Estimator',sm1.sal_name 'Service Advisor',h.description 'Account Name',job.voc_no 'Job No',concat(gate.regno,'-',gate.pltid) 'Reg No',CONVERT(coalesce(invm.nettotal,''),char) 'Total Inv Value',invm.clienttotal 'Clent',invm.excesstotal 'Ins. Co.',"
						+" convert(coalesce(format(coalesce(invm.nettotal,0)-coalesce(lbr.spramt,0),2),'0.00'),char) 'Labour',convert(coalesce(format(spr.spramt,2),'0.00'),char) 'Spares',coalesce(srv.nispareamt,0)+coalesce(spr.spramt,0)+coalesce(lub.spramt,0)+coalesce(cns.spramt,0)+coalesce(oth.spramt,0) 'Actual Spare',convert(coalesce(format(lub.spramt,2),'0.00'),char) 'Lubricants',convert(coalesce(format(cns.spramt,2),'0.00'),char) 'Consumables',convert(coalesce(format(oth.spramt,2),'0.00'),char) 'Others',coalesce(slm.sal_name,'') Salesman,coalesce(rtype.name,'') 'Repair Type' ,if(invm.nettotal is not null,'Invoiced','Pending') 'Invstatus' from ws_jobcard job inner join (select reftype,date,convert(group_concat(voc_no SEPARATOR ' ,'),char(50)) voc_no,refno,sum(nettotal) nettotal,sum(coalesce(nettotal,0))-coalesce(excess,0) clienttotal, coalesce(excess,0) excesstotal from ws_invm  "+
			...
			String strSql="select job.date 'Date',job.voc_no 'Jobno',ac.refname 'Client',case when ((bac.refname=''  or bac.refname is null) and gate.cldocno=1) then 'ALICE RENT A CAR LLC' when bac.refname<>'' or bac.refname is null then coalesce(bac.refname,'NON INSURANCE JOBS') end   Insurname,cl.category 'Client Category',sm.sal_name 'Estimator',sm1.sal_name 'Service Advisor',h.description 'Account Name',concat(gate.regno,'-',gate.pltid) 'Regno',coalesce(estl.total,0) 'EST Labour',coalesce(lbr.spramt,0) 'Actual Spare',slm.sal_name 'Salesman',model.vtype model,brand.brand_name brand,yom.yom,rtype.name 'Repair Type',invm.voc_no 'Invoice No',invm.nettotal 'Inv Value', "+
					" if(invm.nettotal is not null,'Invoiced','Pending') 'Inv Status',coalesce(gate.mainremarks,'') 'Maintenance Remarks' from ws_jobcard job left join (select reftype,date,convert(group_concat(voc_no SEPARATOR ' ,'),char(50)) voc_no,refno,sum(nettotal) nettotal,sum(coalesce(nettotal,0))-coalesce(excess,0) clienttotal, coalesce(excess,0) excesstotal from ws_invm  "+
					" group by refno) invm on (job.doc_no=invm.refno and invm.reftype='JC') left join my_user usr on job.userid=usr.doc_no left join (select jobcarddocno,sum(coalesce(customeramt,0)) spramt from ws_jccspare  group by jobcarddocno) lbr on lbr.jobcarddocno=job.doc_no "+
					" left join ws_estm est on (job.reftype='EST' and job.refno=est.doc_no) left join (select rdocno,sum(total) total from ws_estlabour group by rdocno) estl on estl.rdocno=est.doc_no "+
					" left join ws_gateinpass gate on (if(job.reftype='GIP',job.refno=gate.doc_no,est.gipno=gate.doc_no))"+
					" left join gl_yom yom on gate.yom=yom.doc_no left join gl_vehmodel model on gate.modid=model.doc_no left join gl_vehbrand brand on gate.brdid=brand.doc_no left join my_acbook ac on (gate.cldocno=ac.cldocno and ac.dtype='CRM')"+
		...
		//	System.out.println("jobcard analysis Export detail---:"+strSql);
	...
	public JSONArray accnameSearch() throws SQLException{
	...
	public  JSONArray attachGridReload(String docno,String dtype,String brchid) throws SQLException {
        JSONArray RESULTDATA=new JSONArray();
		...
		try {
		    ...
		    int gatedocno=0;
		    String  cpsql="";
		    if(dtype.equalsIgnoreCase("BWJA")){
		    	String strgetjobcard="select job.doc_no jobdocno,gate.doc_no gatedocno from ws_jobcard job left join ws_estm est on "+
		    	" job.refno=est.doc_no and job.reftype='EST' left join ws_gateinpass gate on est.gipno=gate.doc_no where job.doc_no="+docno;
		    	ResultSet rs=cpstmt.executeQuery(strgetjobcard);
		    	while(rs.next()){
		    		jobdocno=rs.getInt("jobdocno");
		    		gatedocno=rs.getInt("gatedocno");
		    	}
		    	cpsql="select sr_no,a.dtype extension,descpt description,filename,replace(path,'\\\\',';') path,coalesce(at.type_name,'') as type from my_fileattach a left join my_attach_type at on(at.doc_no=ref_id) where a.status=3 and a.doc_no="+jobdocno+" and a.dtype='JBC' and a.brhid='"+brchid+"' union all select sr_no,a.dtype extension,descpt description,filename,replace(path,'\\\\',';') path,coalesce(at.type_name,'') as type from my_fileattach a left join my_attach_type at on(at.doc_no=ref_id) where a.status=3 and a.doc_no="+gatedocno+" and a.dtype='GIP' and a.brhid='"+brchid+"'";
		    }
		    else{
		    	cpsql="select sr_no,a.dtype extension,descpt description,filename,replace(path,'\\\\',';') path,coalesce(at.type_name,'') as type from my_fileattach a left join my_attach_type at on(at.doc_no=ref_id) where a.status=3 and a.doc_no="+docno+" and a.dtype='"+dtype+"' and a.brhid='"+brchid+"' order by sr_no";
		    }
