...
public class ClsRevenueReportCarfareDAO {
...
*/ //Query changed	added estno AND esttotal	
			String strSql="select coalesce(spare.sparetotal,0.0) estapprvalue,convert(if(estadd.estaddition is not null, concat(est.voc_no,'-', estadd.estaddition),est.voc_no),char(100)) estall,coalesce(spare.sparetotal,0.0)+coalesce(labour.labourtotal,0.0) esttotalvalue,convert(lbr.estdocno,char(100)) estno,coalesce(lbr.esttotal,0) esttotal,srv.nispareamt actualspare,sm1.sal_name serviceadvisor,sm.sal_name estimator,cl.category clcategory,invm.date,invm.voc_no invoiceno,invm.refno jobno,invm.nettotal totalinv,invm.clienttotal,invm.excesstotal,ac.refname client,"+
					" concat(gate.regno,'-',gate.pltid) regno,usr.user_name,rtype.name repairtype,"+
					" coalesce(invm.nettotal,0)-coalesce(lbr.spramt,0) labour,coalesce(spr.spramt,0) "+
					" spares,coalesce(lub.spramt,0) lubricants,coalesce(cns.spramt,0) consumables,coalesce(oth.spramt,0) others,slm.sal_name salesman,"+
					" h.description account from ws_jobcard job inner join (select reftype,date,convert(group_concat(voc_no SEPARATOR ' ,'),char(50)) voc_no,refno,sum(nettotal) nettotal,sum(coalesce(nettotal,0))-coalesce(excess,0) clienttotal, coalesce(excess,0) excesstotal from ws_invm where date between '"+sqlfromdate+"' and '"+sqltodate+"' and status<>7 "+
...
//			if(sum(coalesce(invm.nettotal,0)- coalesce(lbr.spramt,0))<0,0,sum(coalesce(invm.nettotal,0)- coalesce(lbr.spramt,0))) labour
	...
	public JSONArray getDetailExportData(String fromdate,String todate,String hidclient,String hidclientslm,String hidrepairtype,String id,String hidserviceadvisor)throws SQLException
	{
		...
		try {
			conn=ClsConnection.getMyConnection();
			Statement detailstmt=conn.createStatement();
			/*,cl.category 'Client Category',convert(if(estadd.estaddition is not null, concat(est.voc_no,'-', estadd.estaddition),est.voc_no),char(100)) 'EST Docno(Additions)'
			 * ,srv.nispareamt 'Actual Spare',coalesce(lbr.esttotal,0) 'EST Total',coalesce(spare.sparetotal,0.0) 'EST Appr.Value',*/
			String strSql="select invm.date 'Date',invm.voc_no 'Invoice No',ac.refname 'Client Name',cl.category 'Client Category',coalesce(usr.user_name,'') 'Job Advisor',"+
			" coalesce(sm.sal_name,'') 'Estimator',coalesce(sm1.sal_name,'') 'Service Advisor',coalesce(h.description,'') 'Account Name',invm.refno 'Job No',concat(gate.regno,'-',gate.pltid) "+
			" 'Reg No',invm.clienttotal 'Client',invm.excesstotal 'Ins. Co.',CONVERT(coalesce(invm.nettotal,''),char) 'Total Inv Value',"+
			" coalesce(spare.sparetotal,0.0)+coalesce(labour.labourtotal,0.0) "+
			" 'EST total value',convert(coalesce(format(coalesce(invm.nettotal,0)-coalesce(lbr.spramt,0),2),'0.00'),char) 'Labour',"+
			" convert(coalesce(format(spr.spramt,2),'0.00'),char) 'Spares',convert(coalesce(format(lub.spramt,2),'0.00'),char) 'Lubricants',"+
			" convert(coalesce(format(cns.spramt,2),'0.00'),char) 'Consumables',convert(coalesce(format(oth.spramt,2),'0.00'),char) 'Others',"+
