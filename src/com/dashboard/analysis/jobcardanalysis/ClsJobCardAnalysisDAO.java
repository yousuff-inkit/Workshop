package com.dashboard.analysis.jobcardanalysis;
...
public class ClsJobCardAnalysisDAO {
	...
	public JSONArray getDetailData(String fromdate,String todate,String hidclient,String hidclientslm,String hidrepairtype,String hidinvoicestatus,String id) throws SQLException {
        JSONArray RESULTDATA=new JSONArray();
        Connection  conn = null;
     ...
     //   System.out.println("hidinvoicestatus=="+hidinvoicestatus);
		...
		try {
			...
			String strSql="select coalesce(lbr.spramt,0) actualspare,sm1.sal_name serviceadvisor,sm.sal_name estimator,cl.category clcategory,job.date,invm.voc_no invoiceno,job.voc_no jobno,invm.nettotal totalinv,invm.clienttotal,invm.excesstotal,ac.refname client,"+
			" concat(gate.regno,'-',gate.pltid) regno,usr.user_name,rtype.name repairtype,"+
			" coalesce(estl.total,0) labour, "+
			" slm.sal_name salesman,"+
			" h.description account,if(invm.nettotal is not null,'Invoiced','Pending') invstatus from ws_jobcard job left join (select reftype,date,convert(group_concat(voc_no SEPARATOR ' ,'),char(50)) voc_no,refno,sum(nettotal) nettotal,sum(coalesce(nettotal,0))-coalesce(excess,0) clienttotal, coalesce(excess,0) excesstotal from ws_invm  "+
	...
	public JSONArray getSummaryData(String fromdate,String todate,String hidclient,String hidclientslm,String hidrepairtype,String hidinvoicestatus,String id,String sumtype) throws SQLException {
        JSONArray RESULTDATA=new JSONArray();
        Connection  conn = null;
        if(!id.equalsIgnoreCase("1")){
        	return RESULTDATA;
...
//			if(sum(coalesce(invm.nettotal,0)- coalesce(lbr.spramt,0))<0,0,sum(coalesce(invm.nettotal,0)- coalesce(lbr.spramt,0))) labour
	...
	public JSONArray getDetailExportData(String fromdate,String todate,String hidclient,String hidclientslm,String hidrepairtype,String hidinvoicestatus,String id)throws SQLException
	{
		JSONArray RESULTDATA=new JSONArray();
        Connection  conn = null;
        if(!id.equalsIgnoreCase("1")){
...
/*			invm.date,invm.voc_no invoiceno,invm.refno jobno,invm.nettotal totalinv,ac.refname client,"+
			...
			String strSql="select job.date 'Date',job.voc_no 'Jobno',ac.refname 'Client',cl.category 'Client Category',sm.sal_name 'Estimator',sm1.sal_name 'Service Advisor',h.description 'Account Name',concat(gate.regno,'-',gate.pltid) 'Regno',coalesce(estl.total,0) 'EST Labour',coalesce(lbr.spramt,0) 'Actual Spare',slm.sal_name 'Salesman',rtype.name 'Repair Type',invm.nettotal 'Inv Value', "+
					" if(invm.nettotal is not null,'Invoiced','Pending') 'Inv Status' from ws_jobcard job left join (select reftype,date,convert(group_concat(voc_no SEPARATOR ' ,'),char(50)) voc_no,refno,sum(nettotal) nettotal,sum(coalesce(nettotal,0))-coalesce(excess,0) clienttotal, coalesce(excess,0) excesstotal from ws_invm  "+
					" group by refno) invm on (job.doc_no=invm.refno and invm.reftype='JC') left join my_user usr on job.userid=usr.doc_no left join (select jobcarddocno,sum(coalesce(customeramt,0)) spramt from ws_jccspare  group by jobcarddocno) lbr on lbr.jobcarddocno=job.doc_no "+
					" left join ws_estm est on (job.reftype='EST' and job.refno=est.doc_no) left join (select rdocno,sum(total) total from ws_estlabour group by rdocno) estl on estl.rdocno=est.doc_no "+
					" left join ws_gateinpass gate on (if(job.reftype='GIP',job.refno=gate.doc_no,est.gipno=gate.doc_no))"+
					" left join my_acbook ac on (gate.cldocno=ac.cldocno and ac.dtype='CRM')"+
