package com.dashboard.analysis.jobinprogress;
...
public class ClsJobInProgressDAO {
	...
	public JSONArray getDetailData(String todate,String hidclient,String hidclientslm,String hidrepairtype,String id,String hidserviceadvisor) throws SQLException {
        JSONArray RESULTDATA=new JSONArray();
        Connection  conn = null;
...
*/
			// removed all distribution kept only invoice expense and total
//			String strSql="select wrb.sal_name referredby,ins.sal_name insursurveyor, "
//			        + " case when coalesce(est.chkrandomlumsum,0)=1 then est.randomlumsumamt else if(est.chkservicelumsum=0,coalesce(lab.labourtotal,0),coalesce(est.servicelumsumamt,0))+ if(est.chklumsum=0,coalesce(amtspare.amt,0),coalesce(est.lumsumamount,0)) end esttotalvalue,"
//			        + " if(est.chkservicelumsum=0,coalesce(lab.labourtotal,0),coalesce(est.servicelumsumamt,0)) labour,if(est.chklumsum=0,coalesce(amtspare.amt,0),coalesce(est.lumsumamount,0)) spares, "
//			        + " coalesce(spare.sparetotal,0.0) estapprvalue, convert(if(estadd.estaddition is not null, concat(est.voc_no,'-', estadd.estaddition),est.voc_no),char(100)) estall,coalesce(spare.sparetotal,0.0)+coalesce(labour.labourtotal,0.0) esttotalvalue,sm1.sal_name serviceadvisor,sm.sal_name estimator,cl.category clcategory,invm.date,invm.voc_no invoiceno,job.voc_no jobno,ac.refname client, concat(gate.regno,'-',gate.pltid) regno,usr.user_name,rtype.name repairtype "
//			        + " , slm.sal_name salesman, h.description account,coalesce(jv.income,0) income,coalesce(jvex.expense,0) expense,coalesce(jv.income,0)-coalesce(jvex.expense,0) total from ws_jobcard job inner join "
//			        + "(select reftype,date,convert(group_concat(voc_no SEPARATOR ' ,'),char(50)) voc_no,refno,sum(nettotal) nettotal from ws_invm where status<>7 and date  between '"+sqlfromdate+"' and '"+sqltodate+"' group by refno) invm on (job.doc_no=invm.refno and invm.reftype='JC') left join my_user usr on job.userid=usr.doc_no   left join ws_estm est on (job.reftype='EST' and job.refno=est.doc_no)  "
...
//			        + " left join ws_gateinpass gate on (if(job.reftype='GIP',job.refno=gate.doc_no,est.gipno=gate.doc_no)) left join my_acbook ac on (gate.cldocno=ac.cldocno and ac.dtype='CRM') left join my_acbook bac on (gate.insurcldocno=bac.cldocno and bac.dtype='CRM' and insurancecomp=1) left join my_head h on (bac.acno=h.doc_no)  left join my_salesman wrb on (gate.referencedby=wrb.doc_no and wrb.sal_type='WRB')  left join my_salm slm on ac.sal_id=slm.doc_no left join ws_gartype rtype on gate.repairtype=rtype.row_no  left join my_clcatm cl on cl.doc_no=ac.catid left join my_salesman sm on (sm.doc_no=gate.marketingperson and sm.sal_type='WMP') left join my_salesman sm1 on (sm1.doc_no=gate.serviceadvisor and sm1.sal_type='WSA') left join my_salesman ins on (ins.doc_no=gate.insurancesurvivor and ins.sal_type='WIS') left join (select  sum(approvedvalue) sparetotal,rdocno from  ws_estspare where confirmed=1 and  approved=1 group by rdocno) spare on est.doc_no=spare.rdocno left join (select sum(total)  labourtotal,  rdocno from  ws_estlabour  where confirmed=1 and approved=1 group by rdocno) labour on  (est.doc_no=labour.rdocno) left join (select convert(group_concat(addition,'-'),char(100)) estaddition,  jobcarddocno jobdocno from ws_estmadd where status=3 group by jobcarddocno) estadd on  job.doc_no=estadd.jobdocno "
//			        + " left join (select  sum(income) income, costcode from (select if(h.gr_type=5,j.dramount*id,0) income ,h.description,costcode from my_jvtran j left join my_head h on j.acno=h.doc_no where h.gr_type in (5) and costtype=9 and j.date  between '"+sqlfromdate+"' and '"+sqltodate+"'  ) a group by costcode ) jv on jv.costcode=job.doc_no left join (select sum(expense) expense, costcode from (select if(h.gr_type=4,j.dramount,0) expense ,h.description,costcode from my_jvtran j left join my_head h on j.acno=h.doc_no where h.gr_type in (4) and costtype=9  ) a group by costcode ) jvex on jvex.costcode=job.doc_no  "
//			        + " where  1=1 "+sqltest+"  order by invm.voc_no " ;
			String strSql="select vm.vtype model,vb.brand_name brand,ym.yom yom,wrb.sal_name referredby,ins.sal_name insursurveyor,  case when coalesce(est.chkrandomlumsum,0)=1 then est.randomlumsumamt else if(est.chkservicelumsum=0,coalesce(lab.labourtotal,0),coalesce(est.servicelumsumamt,0))+ if(est.chklumsum=0,coalesce(amtspare.amt,0),coalesce(est.lumsumamount,0)) end esttotalvalue, if(est.chkservicelumsum=0,coalesce(lab.labourtotal,0),coalesce(est.servicelumsumamt,0)) labour,if(est.chklumsum=0,coalesce(amtspare.amt,0),coalesce(est.lumsumamount,0)) spares,  coalesce(spare.sparetotal,0.0) estapprvalue, convert(if(estadd.estaddition is not null, concat(est.voc_no,'-', estadd.estaddition),est.voc_no),char(100)) estall,coalesce(spare.sparetotal,0.0)+coalesce(labour.labourtotal,0.0) esttotalvalue,sm1.sal_name serviceadvisor,sm.sal_name estimator,cl.category clcategory,invm.date,invm.voc_no invoiceno,job.voc_no jobno,ac.refname client, concat(gate.regno,'-',gate.pltid) regno,usr.user_name,rtype.name repairtype  , slm.sal_name salesman, h.description account,coalesce(jv.income,0) income,coalesce(jvex.expense,0) expense,coalesce(jv.income,0)-coalesce(jvex.expense,0) total from ws_jobcard job left join (select reftype,date,convert(group_concat(voc_no SEPARATOR ' ,'),char(50)) voc_no,refno,sum(nettotal) nettotal from ws_invm where status<>7 and date  <= '"+sqltodate+"' group by refno) invm on (job.doc_no=invm.refno and invm.reftype='JC') left join my_user usr on job.userid=usr.doc_no   left join ws_estm est on (job.reftype='EST' and job.refno=est.doc_no)   left join (select sum(approvedvalue) amt,rdocno from ws_estspare group by rdocno) amtspare on est.doc_no=amtspare.rdocno left join (select sum(coalesce(invoiceamt,0)) labtot,rdocno,sum(coalesce(total,0)) labourtotal from ws_estlabour group by rdocno ) lab on est.doc_no=lab.rdocno left join ws_gateinpass gate on (if(job.reftype='GIP',job.refno=gate.doc_no,est.gipno=gate.doc_no)) left join my_acbook ac on (gate.cldocno=ac.cldocno and ac.dtype='CRM') left join my_acbook bac on (gate.insurcldocno=bac.cldocno and bac.dtype='CRM' and insurancecomp=1) left join my_head h on (bac.acno=h.doc_no)  left join my_salesman wrb on (gate.referencedby=wrb.doc_no and wrb.sal_type='WRB')  left join my_salm slm on ac.sal_id=slm.doc_no left join ws_gartype rtype on gate.repairtype=rtype.row_no  left join my_clcatm cl on cl.doc_no=ac.catid left join my_salesman sm on (sm.doc_no=gate.marketingperson and sm.sal_type='WMP') left join my_salesman sm1 on (sm1.doc_no=gate.serviceadvisor and sm1.sal_type='WSA') left join my_salesman ins on (ins.doc_no=gate.insurancesurvivor and ins.sal_type='WIS') left join (select  sum(approvedvalue) sparetotal,rdocno from  ws_estspare where confirmed=1 and  approved=1 group by rdocno) spare on est.doc_no=spare.rdocno left join (select sum(total)  labourtotal,  rdocno from  ws_estlabour  where confirmed=1 and approved=1 group by rdocno) labour on  (est.doc_no=labour.rdocno) left join (select convert(group_concat(addition,'-'),char(100)) estaddition,  jobcarddocno jobdocno from ws_estmadd where status=3 group by jobcarddocno) estadd on  job.doc_no=estadd.jobdocno  left join (select  sum(income) income, costcode from (select if(h.gr_type=5,j.dramount*id,0) income ,h.description,costcode from my_jvtran j left join my_head h on j.acno=h.doc_no where h.gr_type in (5) and costtype=9 ) a group by costcode ) jv on jv.costcode=job.doc_no left join (select sum(expense) expense, costcode from (select if(h.gr_type=4,j.dramount,0) expense ,h.description,costcode from my_jvtran j left join my_head h on j.acno=h.doc_no where h.gr_type in (4) and costtype=9  ) a group by costcode ) jvex on jvex.costcode=job.doc_no left join gl_vehmodel vm on gate.modid=vm.doc_no left join gl_vehbrand vb on gate.brdid=vb.doc_no left join gl_yom ym on gate.yom=ym.doc_no where  1=1 and invm.voc_no is  null "+sqltest+"   order by invm.voc_no" ;

			System.out.println("revenue detail---:"+strSql);
	...
	public JSONArray getSummaryData(String fromdate,String todate,String hidclient,String hidclientslm,String hidrepairtype,String id,String sumtype,String hidserviceadvisor) throws SQLException {
        JSONArray RESULTDATA=new JSONArray();
        Connection  conn = null;
        if(!id.equalsIgnoreCase("1")){
        	return RESULTDATA;
        ...
        if(sumtype.equalsIgnoreCase("clt")){
        	...
        	sqlselect=" job.date refname,";
			sqlgroup=" group by job.date";
		}
        else if(sumtype.equalsIgnoreCase("mly")){
			sqlselect=" month(job.date),CONVERT(concat(year(job.date),'-',monthname(job.date)),char) refname,";
			sqlgroup=" group by year(job.date),month(job.date)";
		}
        else if(sumtype.equalsIgnoreCase("yly")){
			sqlselect=" year(job.date) refname,";
			sqlgroup=" group by year(job.date)";
		}
		else{
			sqlselect="";
			sqlgroup="";
...
*/
			// income and expense shown
			 String strSql="select "+sqlselect+" sum(coalesce(jv.income,0)) income, sum(coalesce(jvex.expense,0)) expense, sum(coalesce(jv.income,0)-coalesce(jvex.expense,0)) total  from ws_jobcard job  inner join  (select reftype,date,convert(group_concat(voc_no SEPARATOR ' ,'),char(50)) voc_no,refno,sum(nettotal) nettotal from ws_invm where status<>7 and date  between '"+sqlfromdate+"' and '"+sqltodate+"' group by refno) invm on (job.doc_no=invm.refno and invm.reftype='JC') "
			            +" left join ws_estm est on (job.reftype='EST' and job.refno=est.doc_no) left join ws_gateinpass gate on (if(job.reftype='GIP',job.refno=gate.doc_no,est.gipno=gate.doc_no))"
			            +" left join my_acbook ac on (gate.cldocno=ac.cldocno and ac.dtype='CRM')"
			            +" left join my_clcatm cl on cl.doc_no=ac.catid "
			            +" left join my_salm slm on ac.sal_id=slm.doc_no"
	...
	public JSONArray getDetailExportData(String todate,String hidclient,String hidclientslm,String hidrepairtype,String id,String hidserviceadvisor)throws SQLException
	{
		JSONArray RESULTDATA=new JSONArray();
        Connection  conn = null;
        if(!id.equalsIgnoreCase("1")){
...
*/	//query changed- Added estno and esttotal
			String strSql="select invm.date 'Date',invm.voc_no 'Invoice No',ac.refname 'Client Name',cl.category 'Client Category',sm.sal_name 'Estimator',sm1.sal_name 'Service Advisor',h.description 'Account Name',invm.refno 'Job No',concat(gate.regno,'-',gate.pltid) 'Reg No',CONVERT(coalesce(invm.nettotal,''),char) 'Total Inv Value',invm.clienttotal 'Clent',convert(if(estadd.estaddition is not null, concat(est.voc_no,'-', estadd.estaddition),est.voc_no),char(100)) 'EST Docno(Additions)',coalesce(spare.sparetotal,0.0)+coalesce(labour.labourtotal,0.0) 'EST total value',invm.excesstotal 'Ins. Co.',"
				...
				" where status=3 group by jobcarddocno) estadd on job.doc_no=estadd.jobdocno where 1=1 "+sqltest+" order by invm.voc_no" ;
