package com.dashboard.procurment.detailstockreport;
...
public class ClsdetailStockReportDAO {
	...
	public   JSONArray stocklist(String branch,String status,String psrno,String load) throws SQLException {
      ...
      /*  
		...
		try {
				...
				String sql=" select b.name bin,r.name rack,f.name floor,cc.category,c.brhid,bd.brandname,c.qty,m.part_no productid,m.productname,c.amount,c.cost_price from (select  b.brhid,sum(b.qty) qty,b.prdid,sum(amount) amount,b.cost_price from (select i.cost_price,i.brhid,i.op_qty-coalesce(a.outqty,0) qty,i.prdid "
		...
		JSONArray RESULTDATA=new JSONArray();
	      /*  
			...
			try {
					...
					String sql=" select m.part_no 'Product Id',m.productname 'Product Name',bd.brandname 'Brand Name',cc.category 'Category',f.name 'Floor',b.name 'Bin',r.name 'Rack',c.qty 'Qty',c.cost_price 'Cost Price',c.amount 'Total' from (select  b.brhid,sum(b.qty) qty,b.prdid,sum(amount) amount,b.cost_price from (select i.cost_price,i.brhid,i.op_qty-coalesce(a.outqty,0) qty,i.prdid "
							    + "  ,i.cost_price*(i.op_qty-coalesce(a.outqty,0)) amount from my_prddin i "
								+ "  left join (select sum(qty)+sum(del_qty)+sum(rsv_qty) outqty, stockid ,prdid from my_prddout where 1=1  group by stockid) a on a.stockid=i.stockid "
								+ "  where 1=1  "+sqltest1+"  group by i.stockid) b group by b.prdid ) c left join my_main m on m.doc_no=c.prdId  "
								+ "left join my_desc de on(de.psrno=m.doc_no and c.brhid= de.brhid) "
      ...
      /*  
		...
		try {
				...
				*/
				...
				String sql="  select cc.category,bc.branchname,b.name bin,r.name rack,f.name floor,lc.loc_name,c.locid,c.brhid,bd.brandname,c.qty,m.part_no productid,m.productname,c.amount,c.cost_price from "
						 + "  (select  b.locid,b.brhid,b.qty qty,b.prdid,amount amount,b.cost_price from (select i.cost_price,i.locid,i.brhid,i.op_qty-coalesce(a.outqty,0) qty,i.prdid "
						 + "  ,i.cost_price*(i.op_qty-coalesce(a.outqty,0)) amount from my_prddin i "
	      ...
	      /*  
			...
			try {
					...
					*/
					...
					String sql="  select m.part_no 'Product Id',m.productname 'Product Name',bd.brandname 'Brand Name',cc.category 'Category',bc.branchname 'Branch',f.name 'Floor',b.name 'Bin',r.name 'Rack',lc.loc_name 'Location',c.qty 'Qty',c.cost_price 'Cost Price',c.amount 'Total' from "
							 + "  (select  b.locid,b.brhid,b.qty qty,b.prdid,amount amount,b.cost_price from (select i.cost_price,i.locid,i.brhid,i.op_qty-coalesce(a.outqty,0) qty,i.prdid "
							 + "  ,i.cost_price*(i.op_qty-coalesce(a.outqty,0)) amount from my_prddin i "
							 + "  left join (select sum(qty)+sum(del_qty)+sum(rsv_qty) outqty, stockid ,prdid from my_prddout group by stockid) a "
							 + "  on a.stockid=i.stockid   where 1=1 group by i.stockid) b  ) c left join my_main m on m.doc_no=c.prdId "
