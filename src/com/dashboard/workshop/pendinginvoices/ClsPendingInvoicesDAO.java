package com.dashboard.workshop.pendinginvoices;
...
public class ClsPendingInvoicesDAO {
	...
	public JSONArray getPendingData(String branch,String todate,String id) throws SQLException
	{
		...
		try{
			...
			String strsql="select coalesce(wsa.sal_name,'') serviceadvisor,coalesce(pend.remarks,'') remarks,job.doc_no jobcarddocno,job.voc_no jobcardvocno,job.date, concat(gate.pltid,' ',gate.regno) regno,concat(brd.brand_name,' ',model.vtype) flname,coalesce(sal.sal_name,'') sal_name, ac.refname,datediff(curdate(),gate.date) age,coalesce(labtot,0)+coalesce(sparestot,0) total from"
