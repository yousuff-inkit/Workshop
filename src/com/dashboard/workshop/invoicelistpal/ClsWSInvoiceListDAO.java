package com.dashboard.workshop.invoicelistpal;
...
public class ClsWSInvoiceListDAO {
	...
	public JSONArray getInvoiceData(String branch,String fromdate,String todate,String id,String acno,String regno) throws SQLException {
		...
		try {
			...
			" m.roundamt from ws_invm m left join ws_invcalctemp calc on m.doc_no=calc.invno left join ws_jobcard job on (m.reftype='JC' and m.refno=job.doc_no) left join ws_estm est on"+
			...
			" left join gl_yom ym on ym.doc_no=gate.yom "+
			"  left join (select sum(amount),invno,group_concat(distinct coalesce(ins.typename,'OTHER')) insurtypename from ws_invinsurtype i left join my_acinsurtype ins on i.insurtype=ins.rowno  group by invno) invest on (m.doc_no=invest.invno ) "
