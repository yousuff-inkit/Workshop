	 ...
	 public JSONArray regnodetails() throws SQLException {
		...
		public JSONArray getServices(String regno,String pltid,String id,String fromdate,String todate) throws SQLException{
			...
			try{
				...
				String strsql="select wj.voc_no jobvocno,wj.doc_no jobno,wj.date jdate,m.desc1 jobdesc,m.doc_no,m.date,lab.hrs,lab.rate,t.type jobtype,lab.markupper markuppercent,lab.total,lab.remarks,"+
						...
						"   left join ws_estm est on est.doc_no=lab.rdocno "
						+ " left join ws_jobcard wj on est.doc_no=wj.refno left join ws_gateinpass ws on ws.doc_no=est.gipno left join my_salesman wsa on"+
						" (ws.serviceadvisor=wsa.doc_no and wsa.sal_TYPE='WSA') left join ws_technician tech on (lab.technicianid=tech.doc_no)where m.status=3 and wj.complete=1 "+sqltest+" "+sql1+" order by wj.doc_no";
		...
		try {
			...
			String servicestr=" select coalesce(round(sum(invoiceamt),2),0) totamt from ws_gateinpass gate inner join ws_estm est on est.gipno=gate.doc_no "
							 ...
							 +" inner join ws_jobmaster jm on jm.doc_no=lab.jobid inner join ws_jobtype jt on jt.doc_no=jm.jobid where regno="+regno+" "+sqltest2+" group by regno";
