package com.dashboard.workshop.servicedesk;
...
public class ClsWSServiceDeskDAO {
	...
	public JSONArray getFloorMgmtData(String id,String brhid) throws SQLException{
		...
		try{
			...
			String strsql="select delbaymov.indate delindate,flr.esttotal,flr.jobdate,flr.deliverystatus,flr.regno,flr.rowno,case when flr.z1 like "+
			...
			" flr.referredby from ws_floormgmtdata flr left join (select jobcarddocno jobdocno,indate from ws_baymove where bayid=12 group by "+
			" jobcarddocno) delbaymov on flr.jobdocno=delbaymov.jobdocno where completestatus=0 and invstatus=0"+sqltest+" order by brhid,jobvocno";
