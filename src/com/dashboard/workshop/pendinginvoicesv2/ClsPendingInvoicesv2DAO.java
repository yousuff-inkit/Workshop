package com.dashboard.workshop.pendinginvoicesv2;
...
public class ClsPendingInvoicesv2DAO {
    ...
    public JSONArray getPendingData(String branch,String todate,String id) throws SQLException
    {
        ...
        try{
            ...
            " left join (select count(*) releasecount,jobcarddocno jobdocno from "+
        	" ws_vehrelease where clstatus=0 group by jobcarddocno) rls on job.doc_no=rls.jobdocno"+
            "  left join ws_invcalctemp calc on calc.jobdocno=job.doc_no"+
            " where job.complete=1 "+sqltest+" and "+
			" job.confirmstatus=0 and coalesce(rls.releasecount,0)=0 and gate.processstatus>=6 and coalesce(calc.invno,0)=0 group by job.doc_no,est.seccldocno"+
            ") x order by x.jobcardvocno";
            ...
            +" where inv.refno is null and gate.processstatus<7 and job.complete=0"+sqltest+"*/
