package com.dashboard.workshop.gipmgmt;
...
public class ClsGIPMgmtDAO {

	ClsConnection objconn=new ClsConnection();
	ClsCommon objcommon=new ClsCommon();
	
	public JSONArray getGIPMgmtData(String id,String brhid) throws SQLException{
		...
		try{
			...
			" left join my_salesman ms1 on ms1.doc_no=gip.referencedby and ms1.sal_type='WRB'left join my_salesman wsa on (gip.serviceadvisor=wsa.doc_no and wsa.sal_type='WSA' and wsa.status=3) where gip.status=3 and gip.processstatus<>8 "+sqltest+" group by gip.doc_no order by gip.doc_no desc";
	...
	public JSONArray getChecklistdata(String gatedocno)throws SQLException{
		JSONArray data=new JSONArray();
		...
		try{
			...
			{
			 strsql="select c.description,coalesce(answer,'') answer from checklistdata c  left join ws_gipchecklist g on c.description=g.description and  gipno='"+gatedocno+"' ";
			}
			else
			{
		   strsql="select description,'' answer from checklistdata where status=3 order by srno ";
