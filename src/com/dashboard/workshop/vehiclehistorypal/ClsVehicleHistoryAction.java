	    ...
	    HttpServletResponse response = ServletActionContext.getResponse();
		 ...
		 try {      
	       ...
	       //  System.out.println("product++++++++++"+productQuery);
	        ...
	        " select repairtype,(round(customeramt,2)) spareamount,0 labamount from ws_gateinpass gate inner join ws_estm est on "+
	        " est.gipno=gate.doc_no inner join ws_jobcard job on (job.reftype='EST' and job.refno=est.doc_no) inner join ws_jccspare sp on "+
	        " job.doc_no=sp.jobcarddocno where sp.chkcomplete=1 and regno='"+regno+"' and gate.date>='"+sqlFromDate+"' and gate.date<='"+sqlToDate+"' and gate.pltid='"+pltid+"' union all "+
	        " select repairtype,0,(round(invoiceamt,2)) labamount from ws_gateinpass gate inner join ws_estm est on est.gipno=gate.doc_no inner join "+
	        " ws_jobcard job on (job.reftype='EST' and job.refno=est.doc_no) inner join ws_estlabour lab on est.doc_no=lab.rdocno where "+
	        " lab.chkcomplete=1 and regno='"+regno+"' and gate.date>='"+sqlFromDate+"' and gate.date<='"+sqlToDate+"' and gate.pltid='"+pltid+"' union all"+
         ...
         JasperDesign design = JRXmlLoader.load(request.getSession().getServletContext().getRealPath("com/dashboard/workshop/vehiclehistorypal/vehicleHistoryRepairType.jrxml"));
			  ...
			  System.out.println("inside jrxml");
			    HttpServletResponse response = ServletActionContext.getResponse();
				 ...
				 try {      
			         ...
			         String imgpath=request.getSession().getServletContext().getRealPath("/icons/epic.jpg");
			          ...
			          if(vehval==1){
			        	   strSqldetail="select @i:=@i+1 srno,CONCAT('  ',a.date,'    JC No- ',a.jobcard,'    Milage-',a.kmin,' Service Advisor - ',a.sal_name) dtjobcardkmin,lbsrno,a.* from(select 1 onum,regno,pltid,date_format(job.date,'%d-%m-%Y') date,job.voc_no jobcard,kmin,'Complaint' type,coalesce(wsa.sal_name,'') sal_name,"
									...
									+" select 3 onum,regno,pltid,date_format(job.date,'%d-%m-%Y') date,job.voc_no jobcard,kmin,'Service' type,'' sal_name, concat(strjobtype ,' - ', strjobdesc)  desc1,hrs qty,"
									+ " round(invoiceamt,2) amt,format(round(invoiceamt,2),2) amount,lab.srno lbsrno from ws_gateinpass gate inner join ws_estm est on est.gipno=gate.doc_no "
									 +" inner join ws_jobcard job on (job.reftype='EST' and job.refno=est.doc_no) left join ws_estlabour lab on est.doc_no=lab.rdocno "
									 +" left join ws_jobmaster jm on jm.doc_no=lab.jobid left join ws_jobtype jt on jt.doc_no=jm.jobid where lab.chkcomplete=1 and regno="+regno+" "+sqltest+" union all"
			        	   ...
			        	   strSqldetail="select @i:=@i+1 srno,CONCAT('  ',a.date,'    JC No- ',a.jobcard,'    Milage-',a.kmin,' Service Advisor - ',a.sal_name) dtjobcardkmin,lbsrno,a.* from(select 1 onum,regno,pltid,date_format(job.date,'%d-%m-%Y') date,job.voc_no jobcard,kmin,'Complaint' type,coalesce(wsa.sal_name,'') sal_name,"
									...
									+ " round(invoiceamt,2) amt,format(round(invoiceamt,2),2) amount,lab.srno lbsrno from ws_gateinpass gate inner join ws_estm est on est.gipno=gate.doc_no "
									 +" inner join ws_jobcard job on (job.reftype='EST' and job.refno=est.doc_no) inner join ws_estlabour lab on est.doc_no=lab.rdocno "
									 +" inner join ws_jobmaster jm on jm.doc_no=lab.jobid inner join ws_jobtype jt on jt.doc_no=jm.jobid where lab.chkcomplete=1 and regno="+regno+" "+sqltest+" union all"+
									 " select 4 onum,regno,pltid,date_format(job.date,'%d-%m-%Y') date,job.voc_no jobcard,kmin,'Service' type,'' sal_name,d.desc1,d.qty,"+
									 " round(d.nettaxamount,2) amt,format(round(d.nettaxamount,2),2) amount,d.srno lbsrno from my_srvpurm m left join my_srvpurd d on m.doc_no=d.rdocno"+ 
									 " left join ws_jobcard job on (d.costtype=9 and d.costcode=job.doc_no)"+
									 " left join ws_estm est on job.refno=est.doc_no and job.reftype='EST'"+
			       ...
			       // System.out.println("desc"+bean.getLbldesc1()+"pay"+bean.getLblpaytems()+"paytrim"+bean.getLblpaytems()+"del"+ bean.getLbldelterms());
			     //   System.out.println("pathhhhhhhhhhhhhhhhhhh"+commDAO.getPrintPath(dtype));  
	            JasperDesign design = JRXmlLoader.load(request.getSession().getServletContext().getRealPath("com/dashboard/workshop/vehiclehistorypal/"+objcommon.getBIBPrintPath(dtype)));
			  ...
			  System.out.println("inside jrxml");
			    HttpServletResponse response = ServletActionContext.getResponse();
				 ...
				 try {      
				      ...
				      while(rs4.next()){ 
						vehval=rs4.getInt("method");
			        	   ...
			        	   strSqldetail="select @i:=@i+1 srno,h.regno,h.product,h.job,date_format(h.jdate,'%d-%m-%Y')date,h.kmin kms,if(coalesce((h.kmval-h.kmchk),0)*-1<0,0,coalesce((h.kmval-h.kmchk),0)*-1)kminterval,if(coalesce(DATEDIFF(h.dateval,h.datechk),0)*-1<0,0,coalesce(DATEDIFF(h.dateval,h.datechk),0)*-1) dateinterval"
                                      ...
                                      + "coalesce(@g:=(@g-@g)+a.jdate,0) as datechk from(select ws.date,m.psrno,ws.regno,m.part_no,case when coalesce(m.productname,'')='' and coalesce(m.part_no,'')='' then"+
                                      " sp.description else concat(coalesce(m.productname,''),'-',coalesce(m.part_no,'') , ' Qty - ',sp.qty) end product,coalesce(sp.jobcarddocno)job,"
                                      ...
                                      + "inner join ws_jccspare sp on wj.doc_no=sp.jobcarddocno left join my_main m on m.psrno=sp.psrno where sp.chkcomplete=1 and regno="+regno+" and ws.regno="+regno+"  "+sqltest+" union all"+
                                      " select ws.date,0 psrno,ws.regno,'' part_no,d.desc1 product,coalesce(job.doc_no)job,job.date"+
                                      " jdate,ws.kmin,ws.kmin km1 from my_srvpurm m left join my_srvpurd d on m.doc_no=d.rdocno left join ws_jobcard job on"+
                                      " (d.costtype=9 and d.costcode=job.doc_no) left join ws_estm est on (job.refno=est.doc_no and job.reftype='EST')"+
                                      " left join ws_gateinpass ws on est.gipno=ws.doc_no where d.costtype=9 and regno="+regno+" and ws.regno="+regno+"  "+sqltest+")as a,(select @j:=0) as j,(select @g:=0) as g,(select @i:=0) as i order by a.psrno,a.date)h";
			       ...
			       // System.out.println("desc"+bean.getLbldesc1()+"pay"+bean.getLblpaytems()+"paytrim"+bean.getLblpaytems()+"del"+ bean.getLbldelterms());
			     //   System.out.println("pathhhhhhhhhhhhhhhhhhh"+commDAO.getPrintPath(dtype));  
	            JasperDesign design = JRXmlLoader.load(request.getSession().getServletContext().getRealPath("com/dashboard/workshop/vehiclehistorypal/VehHisParts.jrxml"));
