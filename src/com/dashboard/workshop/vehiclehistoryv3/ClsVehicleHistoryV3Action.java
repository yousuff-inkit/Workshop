...
public class ClsVehicleHistoryV3Action extends ActionSupport{
	
	ClsCommon objcommon=new ClsCommon();
	ClsVehicleHistoryV3DAO vehhistoryDAO= new ClsVehicleHistoryV3DAO();
	ClsVehicleHistoryV3Bean vehhistoryBean;
	    ...
	    HttpServletResponse response = ServletActionContext.getResponse();
		 ...
		 try {      
	       ...
	       //  System.out.println("product++++++++++"+productQuery);
	         String imgpath=request.getSession().getServletContext().getRealPath("/icons/carfarehead1.jpg");
	          ...
	          if(vehval==1){
	        	   strSqldetail="select @i:=@i+1 srno,CONCAT('  ',a.date,'    JC No- ',a.jobcard,'    Milage-',a.kmin,' Service Advisor - ',a.sal_name) dtjobcardkmin,lbsrno,a.* from(select 1 onum,regno,pltid,date_format(job.date,'%d-%m-%Y') date,job.voc_no jobcard,kmin,'Complaint' type,coalesce(wsa.sal_name,'') sal_name,"
							...
							+" inner join ws_jobcard job on (job.reftype='EST' and job.refno=est.doc_no) "
							+"inner join ws_jccspare sp on job.doc_no=sp.jobcarddocno left join my_main m on m.psrno=sp.psrno where sp.chkcomplete=1 and regno="+regno+" "+sqltest+" "
							+" union all "
							+" select 3 onum,regno,pltid,date_format(job.date,'%d-%m-%Y') date,job.voc_no jobcard,kmin,'Service' type,'' sal_name, concat(strjobtype ,' - ', strjobdesc) desc1,hrs qty,"
							+ " round(invoiceamt,2) amt,format(round(invoiceamt,2),2) amount,lab.srno lbsrno from ws_gateinpass gate inner join ws_estm est on est.gipno=gate.doc_no "
							 ...
							 +" inner join ws_jobmaster jm on jm.doc_no=lab.jobid inner join ws_jobtype jt on jt.doc_no=jm.jobid where lab.chkcomplete=1 and regno="+regno+" "+sqltest+" ) a ,(select @i:=0) as i  order by jobcard,onum";
	        	   ...
	        	   strSqldetail="select @i:=@i+1 srno,CONCAT('  ',a.date,'    JC No- ',a.jobcard,'    Milage-',a.kmin,' Service Advisor - ',a.sal_name) dtjobcardkmin,lbsrno,a.* from(select 1 onum,regno,pltid,date_format(job.date,'%d-%m-%Y') date,job.voc_no jobcard,kmin,'Complaint' type,coalesce(wsa.sal_name,'') sal_name,"
							...
							+" from ws_gateinpass gate inner join ws_estm est on est.gipno=gate.doc_no "
							+" inner join ws_jobcard job on (job.reftype='EST' and job.refno=est.doc_no) "
							+"inner join ws_jccspare sp on job.doc_no=sp.jobcarddocno inner join my_main m on m.psrno=sp.psrno where sp.chkcomplete=1 and regno="+regno+" "+sqltest+" "
							+" union all "
							+" select 3 onum,regno,pltid,date_format(job.date,'%d-%m-%Y') date,job.voc_no jobcard,kmin,'Service' type,'' sal_name, concat(strjobtype ,' - ', strjobdesc)  desc1,hrs qty,"
							+ " round(invoiceamt,2) amt,format(round(invoiceamt,2),2) amount,lab.srno lbsrno from ws_gateinpass gate inner join ws_estm est on est.gipno=gate.doc_no "
							 +" inner join ws_jobcard job on (job.reftype='EST' and job.refno=est.doc_no) inner join ws_estlabour lab on est.doc_no=lab.rdocno "
	        ...
	        String sparesql="select r.name,round(sum(a.spareamount),2) spamt,round(sum(a.labamount),2) labamt,round(sum(a.spareamount)+sum(a.labamount),2) net from("+
	        " select repairtype,(round(netamount,2)) spareamount,0 labamount from ws_gateinpass gate inner join ws_estm est on "+
	        " est.gipno=gate.doc_no inner join ws_jobcard job on (job.reftype='EST' and job.refno=est.doc_no) inner join ws_investdetail det on job.doc_no=det.jobdocno where det.type='Parts' and regno='"+regno+"' and gate.date>='"+sqlFromDate+"' and gate.date<='"+sqlToDate+"' and gate.pltid='"+pltid+"' union all "+
	        " select repairtype,0,(round(netamount,2)) labamount from ws_gateinpass gate inner join ws_estm est on est.gipno=gate.doc_no inner join "+
	        " ws_jobcard job on (job.reftype='EST' and job.refno=est.doc_no) inner join ws_investdetail det on job.doc_no=det.jobdocno where det.type<>'Parts' and regno='"+regno+"' and gate.date>='"+sqlFromDate+"' and gate.date<='"+sqlToDate+"' and gate.pltid='"+pltid+"' union all"+
	        " select repairtype,(round(d.nettaxamount,2)) spareamount,0 labamount from my_srvpurm m left join my_srvpurd d on m.doc_no=d.rdocno left join ws_jobcard job on (d.costtype=9 and d.costcode=job.doc_no) left join ws_estm est on job.refno=est.doc_no and job.reftype='EST' left join ws_gateinpass gate on est.gipno=gate.doc_no where d.costtype=9 and regno='"+regno+"' and gate.date>='"+sqlFromDate+"' and gate.date<='"+sqlToDate+"' and gate.pltid='"+pltid+"') a left join ws_gartype r on r.row_no=a.repairtype group by r.name";	
...
//	          System.out.println("--"+getGrandtotal());
         ...
         JasperDesign design = JRXmlLoader.load(request.getSession().getServletContext().getRealPath("com/dashboard/workshop/vehiclehistoryv3/vehicleHistoryRepairType.jrxml"));
...
//		System.out.println("Inside V3 Print Action");
		HttpServletRequest request=ServletActionContext.getRequest();
		HttpSession session=request.getSession();;
		String branch = request.getParameter("branchval");
		String regno = request.getParameter("regno");
			         ...
			         String imgpath=request.getSession().getServletContext().getRealPath("/icons/epic.jpg");
			          ...
			          if(vehval==1){
			        	   strSqldetail="select @i:=@i+1 srno,CONCAT('  ',a.date,'    JC No- ',a.jobcard,'    Milage-',a.kmin,' Service Advisor - ',a.sal_name) dtjobcardkmin,lbsrno,a.* from(select 1 onum,regno,pltid,date_format(job.date,'%d-%m-%Y') date,job.voc_no jobcard,kmin,'Complaint' type,coalesce(wsa.sal_name,'') sal_name,"
									...
									+" inner join gl_complaint c on c.doc_no=d.complaintid where regno="+regno+" "+sqltest+" "
									+" union all "
									+" select 2 onum,regno,pltid,date_format(job.date,'%d-%m-%Y') date,job.voc_no jobcard,kmin,'Parts' type,'' sal_name,det.description desc1 ,qty , round(netamount,2) amt,format(round(netamount,2),2) amount,'' lbsrno "
									+" from ws_gateinpass gate inner join ws_estm est on est.gipno=gate.doc_no "
									+" inner join ws_jobcard job on (job.reftype='EST' and job.refno=est.doc_no) "
									+" inner join ws_investdetail det on job.doc_no=det.jobdocno where det.type='Parts' and regno="+regno+" "+sqltest+" "
									+" union all "
									+" select 3 onum,regno,pltid,date_format(job.date,'%d-%m-%Y') date,job.voc_no jobcard,kmin,'Service' type,'' sal_name, concat(strjobtype ,' - ', strjobdesc)  desc1,hrs qty,"
									+ " round(netamount,2) amt,format(round(netamount,2),2) amount,lab.srno lbsrno from ws_gateinpass gate inner join ws_estm est on est.gipno=gate.doc_no "
									 +" inner join ws_jobcard job on (job.reftype='EST' and job.refno=est.doc_no) inner join ws_investdetail det on job.doc_no=det.jobdocno where det.type<>'Parts' and regno="+regno+" "+sqltest+" union all"
									 + " select 4 onum,regno,pltid,date_format(job.date,'%d-%m-%Y') date,job.voc_no jobcard,kmin,'Service' type,'' sal_name,d.desc1,d.qty,"+
									 " round(d.nettaxamount,2) amt,format(round(d.nettaxamount,2),2) amount,d.srno lbsrno from my_srvpurm m left join my_srvpurd d on m.doc_no=d.rdocno"+ 
									 " left join ws_jobcard job on (d.costtype=9 and d.costcode=job.doc_no)"+
									 " left join ws_estm est on job.refno=est.doc_no and job.reftype='EST'"+
									 " left join ws_gateinpass gate on est.gipno=gate.doc_no "+
									 " where d.costtype=9 and regno="+regno+" "+sqltest+") a ,(select @i:=0) as i  order by jobcard,onum";

			          }else{
			        	   strSqldetail="select @i:=@i+1 srno,CONCAT('  ',a.date,'    JC No- ',a.jobcard,'    Milage-',a.kmin,' Service Advisor - ',coalesce(wsa.sal_name,'')) dtjobcardkmin,lbsrno,a.* from"
			        	   		+ "(select 1 onum,regno,pltid,date_format(job.date,'%d-%m-%Y') date,job.voc_no jobcard,kmin,'Complaint' type,'' sal_name,"
		        		            + " concat(c.compname,' - ',d.desc1) desc1 ,round(0,2) qty , 0 amt, 0 amount,'' lbsrno,gate.serviceadvisor from "
									 +" ws_gateinpass gate inner join ws_estm est on est.gipno=gate.doc_no "
									+"  "
									 +" inner join ws_jobcard job on (job.reftype='EST' and job.refno=est.doc_no) "
									 +" inner join ws_gateinpassd d on gate.doc_no=d.rdocno "
									+" inner join gl_complaint c on c.doc_no=d.complaintid where regno="+regno+" "+sqltest+" "
									+" union all "
									+" select 2 onum,regno,pltid,date_format(job.date,'%d-%m-%Y') date,job.voc_no jobcard,kmin,'Parts' type,'' sal_name, det.description desc1 ,round(qty,2) qty , round(netamount,2) amt,format(round(netamount,2),2) amount,'' lbsrno,gate.serviceadvisor "
									+" from ws_gateinpass gate inner join ws_estm est on est.gipno=gate.doc_no "
									+" inner join ws_jobcard job on (job.reftype='EST' and job.refno=est.doc_no) "
									+" inner join ws_investdetail det on job.doc_no=det.jobdocno where det.type='Parts' and regno="+regno+" "+sqltest+" "
									+" union all "
									+" select 3 onum,regno,pltid,date_format(job.date,'%d-%m-%Y') date,job.voc_no jobcard,kmin,'Service' type,'' sal_name, det.description  desc1,round(det.qty,2) qty,"
									+ " round(netamount,2) amt,format(round(netamount,2),2) amount,det.rowno lbsrno,gate.serviceadvisor from ws_gateinpass gate inner join ws_estm est on est.gipno=gate.doc_no "
									 +" inner join ws_jobcard job on (job.reftype='EST' and job.refno=est.doc_no) inner join ws_investdetail det on job.doc_no=det.jobdocno where det.type<>'Parts' and regno="+regno+" "+sqltest+" union all"+
									 " select 4 onum,regno,pltid,date_format(job.date,'%d-%m-%Y') date,job.voc_no jobcard,kmin,'Parts' type,'' sal_name,d.desc1,round(d.qty,2) qty,"+
									 " round(d.nettaxamount,2) amt,format(round(d.nettaxamount,2),2) amount,d.srno lbsrno,gate.serviceadvisor from my_srvpurm m left join my_srvpurd d on m.doc_no=d.rdocno"+ 
									 " left join ws_jobcard job on (d.costtype=9 and d.costcode=job.doc_no)"+
									 " left join ws_estm est on job.refno=est.doc_no and job.reftype='EST'"+
									 " left join ws_gateinpass gate on est.gipno=gate.doc_no "+
									 " where d.costtype=9 and regno="+regno+" "+sqltest+") a left join my_salesman wsa on (a.serviceadvisor=wsa.doc_no and wsa.sal_type='WSA' and wsa.status=3) ,(select @i:=0) as i  order by jobcard,onum";

			          }
			        String sparesql="select r.name,round(sum(a.spareamount),2) spamt,round(sum(a.labamount),2) labamt,round(sum(a.spareamount)+sum(a.labamount),2) net from(select repairtype,(round(customeramt,2)) spareamount,0 labamount from ws_gateinpass gate inner join ws_estm est on est.gipno=gate.doc_no inner join ws_jobcard job on (job.reftype='EST' and job.refno=est.doc_no) inner join ws_jccspare sp on job.doc_no=sp.jobcarddocno where sp.chkcomplete=1 and regno='"+regno+"' and gate.date>='"+sqlFromDate+"' and gate.date<='"+sqlToDate+"' and gate.pltid='"+pltid+"' "+strgridcondition+" union all select repairtype,0,(round(invoiceamt,2)) labamount from ws_gateinpass gate inner join ws_estm est on est.gipno=gate.doc_no inner join ws_jobcard job on (job.reftype='EST' and job.refno=est.doc_no) inner join ws_estlabour lab on est.doc_no=lab.rdocno where lab.chkcomplete=1 and regno='"+regno+"' and gate.date>='"+sqlFromDate+"' and gate.date<='"+sqlToDate+"' and gate.pltid='"+pltid+"' "+strgridcondition+" union all"+
			          " select repairtype,(round(d.nettaxamount,2)) spareamount,0 labamount from my_srvpurm m left join my_srvpurd d on m.doc_no=d.rdocno left join ws_jobcard job on (d.costtype=9 and d.costcode=job.doc_no) left join ws_estm est on job.refno=est.doc_no and job.reftype='EST' left join ws_gateinpass gate on est.gipno=gate.doc_no where d.costtype=9 and regno='"+regno+"' and gate.date>='"+sqlFromDate+"' and gate.date<='"+sqlToDate+"' and gate.pltid='"+pltid+"' "+strgridcondition+") a left join ws_gartype r on r.row_no=a.repairtype group by r.name";	
			             
//			        System.out.println("grid qry---------------"+sparesql);
//			        System.out.println("Vehicle SQL:"+strSqldetail);
			          param.put("sparesql",sparesql);   
			          param.put("compname", vehhistoryBean.getLblcompname());
			          param.put("compaddress", vehhistoryBean.getLblcompaddress());
			          param.put("printname", vehhistoryBean.getLblprintname());
...
//			          System.out.println("--"+getGrandtotal());
			       ...
			       // System.out.println("desc"+bean.getLbldesc1()+"pay"+bean.getLblpaytems()+"paytrim"+bean.getLblpaytems()+"del"+ bean.getLbldelterms());
			     //   System.out.println("pathhhhhhhhhhhhhhhhhhh"+commDAO.getPrintPath(dtype));  
	            JasperDesign design = JRXmlLoader.load(request.getSession().getServletContext().getRealPath(objcommon.getBIBPrintPath(dtype)));
