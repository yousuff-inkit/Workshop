	...
	public JSONArray getReturnData(String branch,String periodupto,String type,String id) throws SQLException{
			...
			try{
				...
				if(type.equalsIgnoreCase("1")){
					...
					strsql="select * from ("+
					" select 0 stockvehiclestatus,agmt.vehreturndate,agmt.status, cl.indate closedate,agmt.vehreturnstatus,agmt.clstatus,br.branchname,agmt.doc_no,agmt.voc_no,"+
					" agmt.brhid,agmt.date,agmt.cldocno,veh.fleet_no perfleet,agmt.outdate, if(agmt.per_name=1,date_add(agmt.outdate,interval"+
					" agmt.per_value*12 month),date_add(agmt.outdate,interval agmt.per_value month)) enddate,ac.refname, veh.reg_no,veh.flname from"+
					" gl_lagmt agmt left join my_acbook ac on (agmt.cldocno=ac.cldocno and ac.dtype='CRM') left join my_brch br on  (agmt.brhid=br.doc_no)"+
					" left join gl_vehmaster veh on (if(agmt.perfleet=0,agmt.tmpfleet,agmt.perfleet)=veh.fleet_no)  left join gl_lagmtclosem cl on"+
					" agmt.doc_no=cl.agmtno  and cl.status=3 where agmt.status=3 and agmt.clstatus=1 and agmt.vehreturnstatus=0 and agmt.vehreturndate"+
					" is null union all"+
					" select 1 stockvehiclestatus,stock.vehreturndate,stock.status,stock.todate indate,stock.vehreturnstatus,1 clstatus,br.branchname,stock.doc_no,stock.doc_no voc_no,"+
					" br.doc_no brhid,stock.date,0 cldocno,veh.fleet_no perfleet,stock.fromdate outdate,stock.todate enddate,'' refname,veh.reg_no,"+
					" veh.flname from gl_stockvehicles stock left join gl_vehmaster veh on stock.fleet_no=veh.fleet_no"+
					" left join my_brch br on veh.a_br=br.doc_no where stock.status=3 and stock.vehreturnstatus=0 and stock.vehreturndate is null) a where 1=1 "+sqltest;
				}
