...
public class ClsCommercialReceiptPostingAction {
	
	ClsCommon ClsCommon=new ClsCommon();
	ClsCommercialReceiptPostingDAO commercialReceiptPostingDAO=new ClsCommercialReceiptPostingDAO();
	...
	java.sql.Date dbCommercialReceiptPostingDate;
	...
	public String saveAction() throws ParseException, SQLException{
		...
		dbCommercialReceiptPostingDate = ClsCommon.changeStringtoSqlDate(getDate());
		...
		if(mode.equalsIgnoreCase("A")){
			...
			int val=commercialReceiptPostingDAO.insert(getHidchckibbranch(),getTxtibbranchid(),getCmbbranch(),getCmbtype(),dbCommercialReceiptPostingDate,getTxtdrtotal(),getTxtchequedescription(),getTxtselectedrno(),getTxtdocno(),tranarray,postingarray,ibpostingarray,session,request,mode);
			if(val>0.0){
				setDetail("Accounts");
				...
				setHiddate(dbCommercialReceiptPostingDate.toString());
