...
public class ClsReplacementAlFahimAction {
	ClsReplacementAlFahimDAO replacementDAO=new ClsReplacementAlFahimDAO();
	ClsReplacementAlFahimBean bean;
...
//		System.out.println("Date:"+date+"Date Out:"+dateout+"InCollectDate"+incollectdate);
					int val=replacementDAO.insert(date,getCmbrentaltype(),getRefno(),getTxtfleetno(),dateout,getTimeout(),getOutkm(),getCmbfuel(),getCmbtrreason(),getCmbreplacetype(),getMode(),
							...
							getFormdetailcode(),deliveryoutdate,getDeliveryouttime(),getDeliveryoutkm(),getCmbdeliveryoutfuel(),getInfleettrancode(),getHidcollectdriver(),getDescription());
					if(val>0.0){
...
//						System.out.println("Else Val"+val);
		...
		int value=replacementDAO.update(getDocno(),getHidchkdelivery(),getHidcollectdriver(),oncollectdate,getOncollecttime(),getOncollectkm(),getCmboncollectfuel(),
				...
				getOndeliverytime(),getOndeliverykm(),getCmbondeliveryfuel(),getTxtoutfleetno(),getHidoutbranch(),getHidoutlocation(),getCmbtrreason(),date,dateout,refdate,getInfleettrancode(),getCmbrentaltype());
			if(value>0){
				setHidchkcollection(getHidchkcollection());
