...
public class ClsLeaseCloseAlFahimAction {
	ClsLeaseCloseAlFahimDAO leasecloseDAO=new ClsLeaseCloseAlFahimDAO();
	ClsLeaseCloseAlFahimBean bean;
	private int docno;
...
//				System.out.println(calcarray.get(i));
			...
			int val=leasecloseDAO.insert(getAgreementno(),getClientid(),getHidchkcollection(),getCollectkm(),getCmbcollectfuel(),collectdate,getCollecttime(),getCmbcheckin(),
					...
					getHidrentalagent(),getCollectchg()==null?"0":getCollectchg(),getBrchName(),getCmbagmtbranch(),getCmbcloseloc(),getDescription(),alfahimstatus);
			if(val>0){
				setDocno(val);
