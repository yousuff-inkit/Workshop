...
public class ClsRentalCloseNewAction {
	ClsRentalCloseNewDAO rentalcloseDAO=new ClsRentalCloseNewDAO();
	ClsCommon objcommon=new ClsCommon();
	ClsRentalCloseNewBean bean;
	private int docno;
	...
	private String tarifnew;


	
	public String getTarifnew() {
		return tarifnew;
	}

	public void setTarifnew(String tarifnew) {
		this.tarifnew = tarifnew;
	}
...
//				System.out.println(calcarray.get(i));
			...
			int val=rentalcloseDAO.insert(getAgreementno(),getClientid(),getHidchkcollection(),getCollectkm(),getCmbcollectfuel()==null?"0":getCmbcollectfuel(),collectdate,getCollecttime(),getHidcheckin(),
					...
					getChaufferid(),calcarray,outdate,getClientacno(),getHidfleet(),getCreditnotesum(),request,getCollectchg()==null?"0":getCollectchg(),getBrchName(),getCmbagmtbranch(),getCmbcloseloc(),getDescription());
			if(val>0){
				setDocno(val);
